...
export async function runV6App(scene: V6AppSceneDefinition, ui: V6AppUiDefinition): Promise<void> {
  ...
  try {
    ...
    const hdr = await fetchBytes(`/fixtures/v6/environments/hdri/${scene.environment.file}`);
    ...
    const runtime: G3DV6Runtime = {
      ...
      runtime: {
        ...
        screenshotPath: `tests/reports/v6-app-suite/${scene.appId}.png`
      },
      metadata: pipeline.metadata,
      secondaryMetadata,
      frame,
...
function createImportedAssetMetadata(
  ...
  return {
    ...
    hdrEnvironmentUri: `/fixtures/v6/environments/hdri/${scene.environment.file}`
  };
}
...
function renderV6AnimationControls(runtime: G3DV6Runtime): string {
  ...
  return `
    <section class="metrics" data-v6-animation-controls="ready">
      <label>
        Clip
...
function resolveV6AssetUrl(file: string): string {
  ...
  if (file.includes("/")) return `/fixtures/v6/assets/${file}`;
  return `/fixtures/v6/assets/corpus/${file}`;
}
...
function createStudioStageItems(bounds: CameraFrameBounds): readonly RenderItem[] {
  ...
  const backdropMaterial = new PBRMaterial({
    name: "v6-studio-gradient-backdrop",
    baseColor: [0.075, 0.09, 0.12, 1],
    metallic: 0,
    roughness: 0.88,
    environmentIntensity: 0.58
  });
  const warmSoftbox = new UnlitMaterial({ name: "v6-studio-warm-softbox", color: [1, 0.82, 0.52, 1] });
  const coolSoftbox = new UnlitMaterial({ name: "v6-studio-cool-softbox", color: [0.38, 0.62, 1, 1] });
  return [
    {
      label: "v6-studio-floor",
      geometry: stageGeometry,
      material: floorMaterial,
      modelMatrix: composeMat4([centerX, floorY, centerZ + depth * 0.15], [0, 0, 0, 1], [width, 0.04, depth])
    },
    {
      label: "v6-studio-backdrop",
      geometry: stageGeometry,
      material: backdropMaterial,
      modelMatrix: composeMat4([centerX, centerY + height * 0.2, backZ], [0, 0, 0, 1], [width, height, 0.05])
    },
    {
      label: "v6-studio-warm-softbox",
