...
- `assets.rooftopDefender`: 1.8 metre low-poly practice standee driven by windup, contest, and recovery state.
...
The manifest retains durable source/download URLs, license, author, hash, role, suitability, orientation, and current hash-bound root-renderer probes for all five models. Audio is candidate-quality and is validated separately because model release-bounds checks do not apply to WAV files.
