...
interface V8WebGPUMaterialsRuntime {
  readonly appId: "v8-webgpu-materials";
...
const APP_ID = "v8-webgpu-materials" as const;
const SIZE = 640;
...
async function run(): Promise<void> {
  ...
  try {
    ...
    const target = renderer.device.createRenderTarget({ width: SIZE, height: SIZE, label: "v8-webgpu-materials-target", format: "rgba8", depth: "texture" });
...
function createRouteWebGPU(): WebGPULike {
  ...
  return {
    ...
    async requestAdapter(): Promise<WebGPUAdapterLike> {
      return {
        name: "v8-webgpu-materials-adapter",
