...
const APP_ID = "v8-animation-walk" as const;
const WIDTH = 1280;
const HEIGHT = 720;
const SOLDIER_URL = "/fixtures/v8/assets/character/soldier.glb";
...
async function run(): Promise<void> {
  ...
  try {
    ...
    const pipeline = await loadV6GLTFRenderPipeline({
      url: SOLDIER_URL,
      assetId: "v8-animation-walk-soldier",
...
function collectImportedItems(pipeline: Awaited<ReturnType<typeof loadV6GLTFRenderPipeline>>, placement: Mat4): readonly RenderItem[] {
  ...
  for (const { node, renderable } of pipeline.resources.scene.collectRenderables()) {
    ...
    const skinning = renderable.skinning
      ...
      : undefined;
    items.push({
      label: `v8-animation-walk:${node.name}`,
      geometry,
      material,
