...
export const DEMOS: readonly DemoDefinition[] = [
  ...
  },
  {
    ...
    subtitle: "Live Tokyo district plus batched traffic, cargo, sensors, facade windows, and data routes. Raise Object count to Extreme to stress instancing.",
    threeCategory: "Instancing / large-scene performance",
    reference: "Reference target: Three.js Littlest Tokyo/keyframe scene plus instancing and large-scene stress examples. Current state: a single authored Tokyo district GLB with live A3D traffic and data overlays kept outside the city keepout.",
    features: ["authored city GLTF", "batched traffic + cargo", "sensor/facade data", "flythrough", "performance telemetry"],
    systems: ["authored animated city district", "instanced traffic fleet", "instanced logistics cargo", "instanced sensor network", "district/facade data routes", "camera flythrough", "performance telemetry"],
    interactions: ["raise object-count load", "select district overlay", "traffic toggle", "wire/debug bounds", "flythrough/reset"],
    proves: ["A3D can keep an authored animated city readable while driving batched vehicles, cargo, sensors, facade windows, district routes, and live frame telemetry."],
    ...
    acceptance: ["Authored animated city scene dominates the screenshot", "A3D traffic/data overlays remain visible but secondary", "route-level traffic/flythrough motion and frame/load metrics stay explicit", "separate high-instance stress evidence is still required before this can claim large-scale instancing parity"],
    controls: [
