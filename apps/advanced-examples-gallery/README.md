...
| Smart City | Authored street grid, instancing, performance scenes | Aerial smart-city simulation with visible roads, districts, logistics yards, grounded traffic, intersection sensors, perimeter load telemetry, and optional flythrough motion. | Extra asset mash-ups, airborne cargo markers, vehicles leaving the road network, or a raw grid benchmark. |
