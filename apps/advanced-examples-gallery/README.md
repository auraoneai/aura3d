...
| Smart City | Accepted | Littlest Tokyo authored hero, restrained A3D traffic/data overlays, batching, and route instrumentation. Extra district GLBs are not added; tram/train/bus authored nodes are excluded. | It is an accepted route, not the entire large-scale instancing benchmark. |
...
| Smart City | Littlest Tokyo, instancing, performance scenes | Aerial Tokyo simulation with the single authored hero, restrained traffic/data pulses, flythrough motion, and keepout so traffic does not clip buildings. Extra district GLBs and tram/train/bus authored nodes are out of scope. | Extra district mash-up, vehicles running through buildings, or a raw grid benchmark. |
