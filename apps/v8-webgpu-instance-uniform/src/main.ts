...
interface V8WebGPUInstanceUniformRuntime {
  readonly appId: "v8-webgpu-instance-uniform";
...
const APP_ID = "v8-webgpu-instance-uniform" as const;
const SIZE = 640;
...
async function run(): Promise<void> {
  ...
  try {
    ...
    const target = renderer.device.createRenderTarget({
      width: SIZE,
      height: SIZE,
      label: "v8-webgpu-instance-uniform-target",
      format: "rgba8",
      depth: "texture"
    });
...
function createRouteWebGPU(): WebGPULike {
  ...
  return {
    ...
    async requestAdapter(): Promise<WebGPUAdapterLike> {
      return {
        name: "v8-webgpu-instance-uniform-adapter",
