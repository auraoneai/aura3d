...
createAuraApp("#app", {
  ...
  scene: scene()
    ...
    .add(primitives.sphere({ name: "neutral emissive sample", material: material.emissive({ color: "#e8edf2", emissive: "#e8edf2", emissiveIntensity: 0.42 }) }).position(-1.18, 0.44, -0.76).scale(0.5))
    .add(primitives.sphere({ material: material.pbr({ color: "#dce8f4", roughness: 0.18, metallic: 0.94 }) }).position(-0.38, 0.42, -0.82).scale(0.38))
    .add(primitives.sphere({ name: "magenta emissive sample", material: material.emissive({ color: "#ff00b8", emissive: "#ff00b8", emissiveIntensity: 0.5 }) }).position(0.4, 0.44, -0.78).scale(0.46))
