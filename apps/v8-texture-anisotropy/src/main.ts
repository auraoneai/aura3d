...
interface V8TextureAnisotropyRuntime {
  readonly appId: "v8-texture-anisotropy";
...
const APP_ID = "v8-texture-anisotropy" as const;
const WIDTH = 1280;
const HEIGHT = 720;
...
async function run(): Promise<void> {
  ...
  try {
    ...
    const shader = device.createShaderProgram({
      label: "v8-texture-anisotropy-shader",
      marker: "@galileo3d-shader:v8-texture-anisotropy-v1",
      vertex: `#version 300 es
// @galileo3d-shader:v8-texture-anisotropy-v1
    ...
    const render = (now: number): void => {
      try {
        ...
        device.draw(pipeline.createDrawCommand({
          label: "v8-texture-anisotropy-draw",
          vertexBuffer,
