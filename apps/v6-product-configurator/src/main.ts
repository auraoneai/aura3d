  ...
  try {
    ...
    const [asset, environments] = await Promise.all([
      ...
      Promise.all(environmentOptions.map((environment) => loadHdrEnvironment({
        url: environment.url ?? `/fixtures/v6/environments/hdri/${environment.file}`,
        id: environment.id,
...
function mountProductConfigurator(root: HTMLElement, runtime: G3DV6Runtime, viewer?: G3DProductViewer): void {
  ...
  root.innerHTML = `
    ...
    <section class="metrics">
      ...
      <span>${runtime.sdkSurface ?? "@galileo3d/engine/v6"}</span>
    </section>
    ...
    <section class="diagnostics">
      ...
      <a href="/tests/reports/v7/product-viewer/product-viewer-report.json">Three.js comparison report</a>
    </section>
  `;
...
function assetUrl(asset: { readonly file: string; readonly url?: string }): string {
  return asset.url ?? `/fixtures/v6/assets/corpus/${asset.file}`;
}
