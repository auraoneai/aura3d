...
createAuraApp("#app", {
  ...
  scene: scene()
    ...
    .add(primitives.box({ name: "amber studio light bar", material: material.emissive({ color: "#ff9f00", emissive: "#ffb000", emissiveIntensity: 0.55 }) }).position(1.25, 0.72, -0.84).scale([0.1, 0.7, 0.1]))
    ...
    .add(primitives.sphere({ name: "amber typed-asset accent", material: material.emissive({ color: "#ff9f00", emissive: "#ffb000", emissiveIntensity: 0.55 }) }).position(0.96, 0.72, -0.72).scale(0.22))
