...
import {
  ...
  type AnimationPose,
  type CartoonAnimationTimelineBinding
} from "../../../packages/animation/src";

type CartoonTestClipId = "Talk" | "Idle" | "Wave" | "Walk" | "Reach";

describe("cartoon animation runtime helpers", () => {
  it("binds speak, listen, gesture, walk, and action timeline states to clips", () => {
    const registry = new AnimationClipRegistry<CartoonTestClipId, never, AnimationPose>([
    ...
    const bindings: readonly CartoonAnimationTimelineBinding<CartoonTestClipId>[] = [
    ...
    expect(controller.bindCartoonTimelineAction("speak", bindings, 0.2)).toMatchObject({ action: "speak", clipId: "Talk" });
    expect(controller.bindCartoonTimelineAction("listen", bindings, 0.4)).toMatchObject({ action: "listen", clipId: "Idle" });
    expect(controller.bindCartoonTimelineAction("gesture", bindings, 0.6)).toMatchObject({ action: "gesture", clipId: "Wave" });
    expect(controller.bindCartoonTimelineAction("walk", bindings, 0.8)).toMatchObject({ action: "walk", clipId: "Walk" });
    expect(controller.bindCartoonTimelineAction("action", bindings, 1)).toMatchObject({ action: "action", clipId: "Reach" });
  });

  it("validates cartoon clip maps and requires segmented fallback for missing clips", () => {
    ...
    expect(validateCartoonClipMap(registry, {
  ...
  it("rejects static pose samples masquerading as cartoon animation", () => {
    const staticReport = summarizeCartoonAnimationMotion([
    ...
    const animatedReport = summarizeCartoonAnimationMotion([
