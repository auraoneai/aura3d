...
describe("TransformControls bounded Three.js parity", () => {
  it("applies translate, rotate, and scale deltas to an attached object like Three.Object3D transform mutation", () => {
    ...
    expect(a3dObject.position.x).toBeCloseTo(threeObject.position.x);
    expect(a3dObject.position.y).toBeCloseTo(threeObject.position.y);
    expect(a3dObject.position.z).toBeCloseTo(threeObject.position.z);
    expect(a3dObject.rotation.y).toBeCloseTo(threeObject.rotation.y);
    expect(a3dObject.scale.x).toBeCloseTo(threeObject.scale.x);
    expect(a3dObject.scale.y).toBeCloseTo(threeObject.scale.y);
    expect(a3dObject.scale.z).toBeCloseTo(threeObject.scale.z);

    controls.detach();
    controls.apply(new ControlVector3(1, 1, 1));
    expect(a3dObject.position.x).toBeCloseTo(0.25);
  });
});
