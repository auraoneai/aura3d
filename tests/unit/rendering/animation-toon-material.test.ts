...
import {
  AnimationToonMaterial,
  ANIMATION_TOON_SHADER_NAME,
  ANIMATION_TOON_SHADER_MARKER,
  applyAnimationRenderPreset,
...
describe("animation toon ramp/quantize math", () => {
...
describe("AnimationToonMaterial integration", () => {
  it("targets the animation toon shader and seeds toon uniforms", () => {
    const material = new AnimationToonMaterial({ bands: 5, shadowFloor: 0.4 });
    expect(material.shaderKey).toBe(ANIMATION_TOON_SHADER_NAME);
  ...
  it("rejects out-of-range band counts", () => {
    expect(() => new AnimationToonMaterial({ bands: 1 })).toThrow();
    expect(() => new AnimationToonMaterial({ bands: 17 })).toThrow();
    expect(() => new AnimationToonMaterial({ bands: 3.5 })).toThrow();
  });

  it("registers a compilable animation shader program in the default library", () => {
    const library = createDefaultShaderLibrary();
    expect(library.names()).toContain(ANIMATION_TOON_SHADER_NAME);
    const compiled = library.compileSource(ANIMATION_TOON_SHADER_NAME);
    expect(compiled.fragment).toContain(ANIMATION_TOON_SHADER_MARKER);
    expect(compiled.fragment).toContain("a3dToonQuantize");
  });
});

describe("applyAnimationRenderPreset", () => {
  it("builds a real toon material from the preset and enables the outline pass", () => {
    const preset = createAnimationRenderPreset({ materialStyle: { treatment: "cel" } });
    const width = 8;
    const height = 8;
    ...
    const result = applyAnimationRenderPreset(preset, { frame: { pixels, width, height } });

    expect(result.material.shaderKey).toBe(ANIMATION_TOON_SHADER_NAME);
  ...
  it("skips the outline pass when the style does not request outlines", () => {
    const preset = createAnimationRenderPreset({ materialStyle: { treatment: "preserve-pbr" } });
    const result = applyAnimationRenderPreset(preset);
