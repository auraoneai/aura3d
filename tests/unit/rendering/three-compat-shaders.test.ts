...
import { CustomShaderMaterialCompat, NodeMaterialCompat, RawShaderMaterialCompat, SHADER_CHUNKS_THREE_COMPAT, diagnoseThreeCompatShader } from "../../../packages/three-compat/src";

describe("ThreeCompat shader authoring", () => {
  it("supports custom shader materials, raw shader materials, uniforms, nodes, chunks, and diagnostics", () => {
    ...
    const bad = diagnoseThreeCompatShader(vertex, "precision highp float;");
    ...
    expect(SHADER_CHUNKS_THREE_COMPAT.pbrLighting).toContain("applyPbrLight");
