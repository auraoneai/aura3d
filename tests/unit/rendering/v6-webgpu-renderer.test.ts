...
describe("V6 WebGPU report", () => {
  ...
  it("publishes V7 WebGPU production SDK readiness without keeping the SDK backend blocked", async () => {
    ...
    expect(report.safetyChecks.find((item) => item.id === "renderer-v6-webgpu-uses-production-webgpu-path")?.status).toBe("ready");
  ...
  it("routes backend='webgpu' to ProductionWebGPURenderer instead of silently falling back to WebGL2", () => {
    const source = readFileSync(resolve("packages/rendering/src/v6/RendererV6.ts"), "utf8");
  ...
  it("keeps scene-color transmission capture shared across WebGL2 and WebGPU production renderers", () => {
    const helper = readFileSync(resolve("packages/rendering/src/v6/TransmissionBackdropCapture.ts"), "utf8");
    const webgl2 = readFileSync(resolve("packages/rendering/src/v6/ProductionWebGL2Renderer.ts"), "utf8");
    const webgpu = readFileSync(resolve("packages/rendering/src/v6/ProductionWebGPURenderer.ts"), "utf8");
