...
describe("ShaderLibrary", () => {
  ...
  it("uses the same configurable linear/display output encode across PBR shader paths", () => {
    ...
    for (const compiled of pbrShaders) {
      ...
      expect(compiled.fragment).toMatch(/a3d(TexturedPbrEncode)?LinearToSrgb/);
      expect(compiled.fragment).toContain("vec3(0.0031308)");
      expect(compiled.fragment).not.toContain("vec3(1.0 / 2.2)");
  ...
  it("preserves textured PBR HDR reflections while retaining material specular ownership", () => {
    const library = createDefaultShaderLibrary();
    ...
    expect(textured.fragment).toContain("float proceduralSpecularScale = materialFiniteSpecularScale;");
    expect(textured.fragment).toContain("float sampledSpecularScale = materialEnvironmentSpecularScale;");
    expect(textured.fragment).toContain("return clamp(radiance, vec3(0.0), vec3(65504.0));");
    expect(textured.fragment).not.toContain("float faceOnSpecularGate = smoothstep(0.1, 0.42, nDotV);");
    expect(textured.fragment).not.toContain("maxChannel > 1.35 ? softKnee * (1.35 / maxChannel) : softKnee;");
  ...
  it("keeps the packaged direct-PBR shader files synchronized with the default shader library", () => {
    ...
    expect(fragmentSource).toContain("sampledSpecular *= u_environmentMapTextureSpecularIntensity * u_materialEnvironmentIntensity * sampledEnvironmentWeight * mix(1.1, 0.65, roughness);");
  ...
  it("lights textured PBR material extensions as separate direct and environment lobes", () => {
    ...
    expect(compiled.fragment).toContain("float proceduralSpecularScale = materialFiniteSpecularScale;");
    expect(compiled.fragment).toContain("float sampledSpecularScale = materialEnvironmentSpecularScale;");
    expect(compiled.fragment).not.toContain("float faceOnSpecularGate = smoothstep(0.1, 0.42, nDotV);");
    ...
    expect(compiled.fragment).toContain("vec3 clearcoatLobe = clearcoatF * clearcoatD * clearcoatG * clamp(clearcoat, 0.0, 1.0);");
    ...
    expect(compiled.fragment).toContain("anisotropyRotation += atan(anisotropyDirection.y, anisotropyDirection.x);");
    ...
    expect(compiled.fragment).toContain("vec3 a3dTexturedPbrThinFilmFresnel(");
    expect(compiled.fragment).toContain("vec3 iridescenceLobe = (filmF - a3dFresnelSchlick(substrateF0, vDotH)) * clamp(iridescence, 0.0, 1.0) * filmD * filmG;");
    ...
    expect(compiled.fragment).toContain("vec3 clearcoatLobe = specularRadiance * clearcoatF * clamp(clearcoat, 0.0, 1.0);");
    expect(compiled.fragment).toContain("vec3 iridescenceLobe = substrateSpecularRadiance * (filmF - a3dFresnelSchlick(substrateF0, filmNdotV)) * clamp(iridescence, 0.0, 1.0);");
    expect(compiled.fragment).toContain("sampledSpecular *= u_environmentMapTextureSpecularIntensity * u_materialEnvironmentIntensity * sampledEnvironmentWeight * mix(1.1, 0.65, clampedRoughness);");
    ...
    expect(compiled.fragment).toContain("return clamp(radiance, vec3(0.0), vec3(65504.0));");
    expect(compiled.fragment).not.toContain("maxChannel > 1.35 ? softKnee * (1.35 / maxChannel) : softKnee;");
