...
import {
  ...
  type CartoonRouteProofAsset,
  type CartoonRouteProofGesture,
  type CartoonRouteProofShot,
  type CartoonRouteProofViseme
} from "../../../packages/engine/src";

const generatedAt = "2026-06-06T00:00:00.000Z";

describe("cartoon route proof", () => {
  it("creates a stable passing proof for a typed rendered route", () => {
    const proof = cartoon.routeProof({
    ...
    expect(proof).toMatchObject({
      ...
      schemaVersion: "aura3d-cartoon-route-proof/v1",
      status: "pass"
    });
    expect(proof.checks.every((check) => check.passed)).toBe(true);
    expect(validateCartoonRouteProof(proof)).toHaveLength(0);
  });

  it("fails source-only/notTrue3D routes and missing rendered mouth proof", () => {
    const proof = createCartoonRouteProof({
      episodeId: "moon-garden-001",
      route: "/cartoon/image-puppet",
      duration: 12,
      frameRate: 30,
      assets: readyAssets(),
      shots: readyShots(),
    ...
    expect(proof.issues.map((issue) => issue.code)).toEqual(
      expect.arrayContaining(["cartoon-route-visemes-rendered", "cartoon-route-not-source-only"])
    );
    expect(validateCartoonRouteProof(proof).map((issue) => issue.code)).toContain("cartoon-route-proof-status-fail");
  });

  it("fails when expected characters are not visible in shots", () => {
    const shots: CartoonRouteProofShot[] = [{
      ...readyShots()[0],
    ...
    expect(proof.status).toBe("fail");
    expect(proof.issues.map((issue) => issue.code)).toContain("cartoon-route-shot-characters-visible");
  });
});

function readyAssets(): readonly CartoonRouteProofAsset[] {
  return [
...
function readyShots(): readonly CartoonRouteProofShot[] {
  return [{
    id: "shot-1",
    startTime: 0,
    endTime: 4,
...
function readyVisemes(): readonly CartoonRouteProofViseme[] {
  return [{
    id: "viseme-1",
    characterId: "miko",
    startTime: 1,
...
function readyGestures(): readonly CartoonRouteProofGesture[] {
  return [{
    id: "wave-1",
    characterId: "luma",
    state: "gesture-wave",
