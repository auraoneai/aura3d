...
function outputPackage(): AnimationRenderOutputPackageMetadata {
  return {
    ...
    youtubeDraft: {
      ...
      description: "A short Aura3D animation package.",
...
function mp4OutputPackage(): AnimationRenderOutputPackageMetadata {
  return {
    ...outputPackage(),
    outputs: {
...
function runtime() {
  return {
    seek: () => undefined,
    step: () => undefined,
    captureFrame: (item: AnimationRenderQueueArtifact["items"][number]) => ({
      item,
...
describe("animation video export adapters", () => {
