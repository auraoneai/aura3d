...
import {
  ...
  type AnimationMotionFrameSample,
  type AnimationMotionSegmentInput
} from "../../../packages/engine/src";

const generatedAt = "2026-06-06T00:00:00.000Z";

describe("animation motion quality", () => {
  it("rejects global-only still-image motion", () => {
    const report = createAnimationMotionQualityReport({
    ...
    expect(report.issues.map((issue) => issue.code)).toContain("animation-motion-global-only");
    expect(report.segments[0]?.issues.map((issue) => issue.code)).toContain("animation-motion-segment-global-only");
    expect(validateAnimationMotionQuality(report).map((issue) => issue.code)).toContain("animation-motion-global-only");
  });

  it("passes independent character and mouth motion", () => {
    const report = animation.motionQuality({
      episodeId: "moon-garden-001",
      frameRate: 30,
      generatedAt,
    ...
    expect(validateAnimationMotionQuality(report)).toHaveLength(0);
  });
  ...
  it("requires mouth motion during dialogue beats", () => {
    ...
    const segment: AnimationMotionSegmentInput = { id: "dialogue-1", kind: "dialogue", startFrame: 0, endFrame: 3 };
    const report = createAnimationMotionQualityReport({
      episodeId: "moon-garden-001",
      frameRate: 30,
      frames,
    ...
    expect(report.status).toBe("fail");
    expect(report.issues.map((issue) => issue.code)).toContain("animation-motion-mouth-missing");
  });
});

function globalOnlyFrames(): readonly AnimationMotionFrameSample[] {
...
function independentMotionFrames(): readonly AnimationMotionFrameSample[] {
  const dialogueFrames: AnimationMotionFrameSample[] = Array.from({ length: 4 }, (_, frame) => ({
    frame,
    time: frame / 30,
  ...
  const actionFrames: AnimationMotionFrameSample[] = Array.from({ length: 4 }, (_, index) => {
    const frame = index + 4;
    return {
      frame,
      time: frame / 30,
