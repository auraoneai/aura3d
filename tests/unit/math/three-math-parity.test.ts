...
describe("Three.js math parity surface", () => {
  it("keeps Vector3-compatible mutation semantics alongside immutable A3D math", () => {
    const compat = new Vector3Compat(1, 2, 3).add(new Vector3Compat(4, 5, 6)).sub(new Vector3Compat(1, 1, 1));
    const a3d = new Vector3(1, 2, 3).add(new Vector3(4, 5, 6)).subtract(new Vector3(1, 1, 1));

    expect([compat.x, compat.y, compat.z]).toEqual(a3d.toArray());
