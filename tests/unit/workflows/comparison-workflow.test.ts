...
import { createComparisonWorkflow } from "@galileo3d/workflows";

describe("createComparisonWorkflow", () => {
  it("creates a renderable G3D/Three.js comparison workflow", async () => {
    ...
    expect(workflow.comparison.g3dSteps.length).toBeGreaterThan(0);
    expect(workflow.comparison.threeJsSteps.length).toBeGreaterThan(0);
    expect(workflow.comparison.migrationNotes.join(" ")).toContain("@galileo3d/three-compat");
