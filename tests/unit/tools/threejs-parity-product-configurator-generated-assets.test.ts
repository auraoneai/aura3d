...
describe("threejsParity Product Configurator generated/support asset manifests", () => {
  it("keeps the generated studio GLB support-only with zero texture-backed hero evidence", () => {
    ...
    expect(manifest.source.sourceScript).toBe("tools/advanced-gallery-assets/generate-product-configurator-studio-blender.py");
    expect(manifest.source.inputAssets).toEqual([]);
    expect(manifest.source.derivativeOfExternalAsset).toBe(false);

    expect(glb.path).toBe("fixtures/advanced-gallery/assets/product-configurator-studio-blender/product-configurator-studio-blender.glb");
    ...
    expect(manifest.supportTruth?.cannotReplace).toContain("fixtures/threejs-parity/assets/vehicles/car-concept.glb");
  ...
  it("keeps the batched car GLB a derivative that cannot replace the original Product hero", () => {
    const manifest = readJson<GeneratedAssetManifest>("fixtures/advanced-gallery/assets/product-configurator-car-batched/manifest.json");
    const glb = manifest.outputs.glb;
    ...
    expect(manifest.source.sourceScript).toBe("tools/advanced-gallery-assets/optimize-product-car-blender.py");
    expect(manifest.source.inputAssets).toEqual(["fixtures/threejs-parity/assets/vehicles/car-concept.glb"]);
    ...
    expect(sourceAsset.path).toBe("fixtures/threejs-parity/assets/vehicles/car-concept.glb");
    ...
    expect(glb.path).toBe("fixtures/advanced-gallery/assets/product-configurator-car-batched/car-concept-batched.glb");
    expect(existsSync(resolve(glb.path))).toBe(true);
    expect(glb.byteSize).toBe(statSync(resolve(glb.path)).size);
    expect(glb.sha256).toBe(sha256(glb.path));
    ...
    expect(manifest.supportTruth?.cannotReplace).toContain("fixtures/threejs-parity/assets/vehicles/car-concept.glb");
    expect(manifest.acceptanceBoundary).toContain("cannot replace fixtures/threejs-parity/assets/vehicles/car-concept.glb");
