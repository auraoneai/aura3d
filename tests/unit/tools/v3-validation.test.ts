...
import { validateV3ClaimGates } from "../../../tools/v3-claim-gates/index.js";
import { validateV3ReportFreshness, writeJson, baseReport } from "../../../tools/v3-reporting/index.js";
...
`);
  ...
  it("detects stale v3 reports by source hash", () => {
    ...
    writeJson(root, "tests/reports/v3-current-capability.json", report);
    writeFileSync(join(root, "docs", "v3", "README.md"), "after\n");

    const issues = validateV3ReportFreshness(root, ["tests/reports/v3-current-capability.json"]);

    expect(issues).toEqual([
      expect.objectContaining({ path: "tests/reports/v3-current-capability.json", message: expect.stringContaining("Freshness source changed") }),
    ]);
  });
});
