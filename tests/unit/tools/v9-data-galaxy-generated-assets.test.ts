...
describe("v9 Data Galaxy generated asset manifests", () => {
  it("keeps the Blender data core manifest support-only with generated texture-backed provenance and no hero claim", () => {
    const manifest = readJson<GeneratedAssetManifest>("fixtures/v9/assets/data-galaxy-core-blender/manifest.json");
    ...
    expect(manifest.source.sourceScript).toBe("tools/v9-advanced-gallery-assets/generate-data-galaxy-core-blender.py");
    expect(manifest.source.inputAssets).toEqual([]);
    expect(manifest.source.derivativeOfExternalAsset).toBe(false);

    expect(glb.path).toBe("fixtures/v9/assets/data-galaxy-core-blender/data-galaxy-core-blender.glb");
    ...
    expect(blend.path).toBe("fixtures/v9/assets/data-galaxy-core-blender/data-galaxy-core-blender.blend");
  ...
  it("keeps the deep-space HDR manifest scoped to background evidence only", () => {
    const manifest = readJson<GeneratedAssetManifest>("fixtures/v9/environments/hdri/data_galaxy_deep_space_1k.manifest.json");
    ...
    expect(manifest.source.sourceScript).toBe("tools/v9-advanced-gallery-assets/generate-data-galaxy-deep-space-hdr.mjs");
    expect(manifest.source.inputAssets).toEqual([]);
    expect(manifest.source.derivativeOfExternalAsset).toBe(false);

    expect(hdr.path).toBe("fixtures/v9/environments/hdri/data_galaxy_deep_space_1k.hdr");
