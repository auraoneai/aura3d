...
import { validateV4ClaimGates } from "../../../tools/external-parity-claim-gates/index.js";
import { createV4BroadParityReadinessReport } from "../../../tools/external-parity-broad-parity-readiness/index.js";
import { createV4ProductionReadinessReport, validatePublicDemoDeploymentSmokeEvidence } from "../../../tools/external-parity-production-readiness/index.js";
import { createV4PbrGltfReadinessReport } from "../../../tools/external-parity-pbr-gltf-readiness/index.js";
import { collectPbrVisualEvidencePaths } from "../../../tools/external-parity-pbr-visual-parity/index.js";
import { collectProductVisualEvidencePaths, validateExternalProductVisualBaseline } from "../../../tools/external-parity-product-visual-parity/index.js";
import { collectShadowVisualEvidencePaths } from "../../../tools/external-parity-shadow-visual-parity/index.js";
import { collectHdrVisualEvidencePaths } from "../../../tools/external-parity-hdr-visual-parity/index.js";
import { collectPostprocessSuiteEvidencePaths } from "../../../tools/external-parity-postprocess-suite/index.js";
...
import { createV4UnityUnrealParityReport } from "../../../tools/external-parity-unity-unreal-parity/index.js";
import { createV4ExternalEngineBaselineKit } from "../../../tools/external-parity-external-engine-baselines/index.js";
import { createV4ExternalEvidenceReadinessReport } from "../../../tools/external-parity-external-evidence-readiness/index.js";
import { createV4CompletionAuditReport } from "../../../tools/external-parity-completion-audit/index.js";
import { createV4ParityStatusSummary } from "../../../tools/external-parity-parity-status/index.js";
import { createV4LocalPortStatusSummary } from "../../../tools/external-parity-local-port-status/index.js";
import { createV4ExternalHostDoctorReport } from "../../../tools/external-parity-external-host-doctor/index.js";
import { createV4ExternalHostRunnerReport } from "../../../tools/external-parity-external-host-runner/index.js";
import { createV4GithubExternalReadinessReport, type CommandResult, type CommandRunner } from "../../../tools/external-parity-github-external-readiness/index.js";
import { createV4ExternalEvidenceHandoffReport, verifyAndRecordV4ExternalEvidenceHandoffPackage, verifyV4ExternalEvidenceHandoffPackage } from "../../../tools/external-parity-external-evidence-handoff/index.js";
import { createV4HdrRenderTargetReadinessReport } from "../../../tools/external-parity-hdr-render-target-readiness/index.js";
import { createV4PbrReferenceReadinessReport } from "../../../tools/external-parity-pbr-reference-readiness/index.js";
import { createV4ShadowMapReadinessReport } from "../../../tools/external-parity-shadow-map-readiness/index.js";
import { evaluateV4CodebaseRootReadinessGate, shouldV4CodebaseRootReadinessCliPass } from "../../../tools/external-parity-codebase-root-readiness/index.js";
import { baseReport } from "../../../tools/external-parity-reporting/index.js";
import { validateV4ReportFreshness } from "../../../tools/external-parity-reporting/index.js";
...
jobs:
  ...
  deploy:
    ...
    steps:
      ...
      - run: pnpm verify:external-parity-report-freshness
...
describe("v4 validation tools", () => {
  ...
  it("detects stale v4 reports by source hash", () => {
    ...
    expect(issues).toEqual([
      expect.objectContaining({ path: "tests/reports/external-parity-current-capability.json", message: expect.stringContaining("Freshness source changed") }),
    ]);
  });
  ...
  it("rejects known-bad v4 visual manifest entries before manual review can override them", () => {
    const screenshot = {
      id: "game-slice",
      path: "tests/reports/external-parity-example-screenshots/game-slice.png",
    ...
    expect(validateVisualGateEntry({ ...good, screenshotPath: "tests/reports/external-parity-example-screenshots/old.png" }, screenshot)).toMatchObject({ passed: false, stale: true });
  ...
  it("keeps v4 visual approval blocked without an explicit current review artifact", () => {
    const required = ["product-configurator", "architecture-viewer", "game-slice"];

    expect(validateManualVisualApproval(null, required)).toMatchObject({
      passed: false,
      missingIds: required,
      reason: expect.stringContaining("is missing"),
    });
    expect(validateManualVisualApproval({
      status: "approved",
      reviewer: "visual-review",
      approvedAt: "2026-05-13T00:00:00.000Z",
      entries: [
        { id: "product-configurator", status: "approved" },
        { id: "architecture-viewer", status: "rejected" },
      ],
    }, required)).toMatchObject({
      passed: false,
      rejectedIds: ["architecture-viewer"],
      missingIds: ["architecture-viewer", "game-slice"],
    });
    expect(validateManualVisualApproval({
      status: "approved",
      reviewer: "visual-review",
      approvedAt: "2026-05-13T00:00:00.000Z",
      entries: required.map((id) => ({ id, status: "approved" })),
    }, required)).toMatchObject({
      passed: true,
      approvedIds: required,
      missingIds: [],
    });
  });

  it("keeps root readiness false while current visual quality is failing", () => {
    expect(evaluateV4CodebaseRootReadinessGate({
      rootBlockers: [],
      completionBlockers: [],
      broadParityBlockers: [],
      externalEvidenceBlockers: [],
      visualQualityOk: false,
    })).toMatchObject({
      rootContractsReady: true,
      localVisualQualityReady: false,
      exampleImplementationMayResume: false,
      rootReady: false,
      examplesAllowedToResume: false,
      violations: ["visual-quality: tests/reports/external-parity-visual-quality.json is missing or failing"],
      examplesResumeBlockers: ["visual-quality: tests/reports/external-parity-visual-quality.json is missing or failing"],
    });
    ...
    expect(evaluateV4CodebaseRootReadinessGate({
      ...
      broadParityBlockers: [],
      externalEvidenceBlockers: [],
      visualQualityOk: true,
    })).toMatchObject({
      rootContractsReady: true,
      localVisualQualityReady: true,
      exampleImplementationMayResume: true,
      rootReady: true,
      examplesAllowedToResume: true,
      violations: [],
      examplesResumeBlockers: [],
    });
  });

  it("allows a contracts-only root readiness exit without approving full root readiness", () => {
    const locallyReadyButStrictlyBlocked = {
      ok: false,
      rootContractsReady: true,
      allCodebaseObjectiveRequirementsCovered: true,
    };

    expect(shouldV4CodebaseRootReadinessCliPass(locallyReadyButStrictlyBlocked, "strict")).toBe(false);
    expect(shouldV4CodebaseRootReadinessCliPass(locallyReadyButStrictlyBlocked, "contracts-only")).toBe(true);
    expect(shouldV4CodebaseRootReadinessCliPass({
      ...locallyReadyButStrictlyBlocked,
      allCodebaseObjectiveRequirementsCovered: false,
    }, "contracts-only")).toBe(false);
  });
  ...
  it("breaks full glTF parity into explicit remaining evidence dimensions", () => {
    ...
    writeFileSync(join(root, "tests", "reports", "external-parity-pbr-visual-parity.json"), JSON.stringify({ ok: false }, null, 2));
    writeFileSync(join(root, "tests", "reports", "external-parity-pbr-reference-readiness.json"), JSON.stringify({ ok: false }, null, 2));
    writeFileSync(join(root, "tests", "reports", "external-parity-gltf-loader-visual-parity.json"), JSON.stringify({
      ok: true,
      boundedGltfLoaderVisualParity: { threejs: true, babylon: true },
  ...
  it("collects product visual render and diff artifacts into screenshot evidence paths", () => {
    const paths = collectProductVisualEvidencePaths({
      renders: [
        {
          ...
          metrics: {
            ...
            nonBlankPixels: 1,
            salientRatio: 0.2,
            occupiedAreaRatio: 0.2,
            occupiedQuadrants: 3,
            meanLuma: 32,
            darkPixelRatio: 0.4,
            colorBuckets: 1,
            dominantBucketRatio: 0.5,
            edgePixelRatio: 0.02,
            drawCalls: 1,
            materialCount: 1,
            productParts: 1,
        ...
        {
          engine: "threejs",
          screenshotPath: "tests/reports/external-parity-product-visual-parity/threejs-product.png",
          bundleBytes: 1,
          metrics: {
            width: 720,
            height: 480,
      ...
      externalBaselines: {
        unity: {
          engine: "unity",
          reportPath: "tests/reports/external-parity-unity-product-visual-baseline.json",
          ok: true,
          present: true,
          requiredSceneDescriptorId: "v4-deterministic-product-visual-parity",
          requiredSceneDescriptorVersion: "external-parity-product-visual-parity-scene-v1",
          screenshot: {
            ok: true,
            path: "tests/reports/external-parity-product-visual-parity/unity-product.png",
          },
          runnerEvidence: {
            ok: true,
            violations: [],
        ...
        unreal: {
          engine: "unreal",
          reportPath: "tests/reports/external-parity-unreal-product-visual-baseline.json",
          ok: false,
          present: false,
          requiredSceneDescriptorId: "v4-deterministic-product-visual-parity",
          requiredSceneDescriptorVersion: "external-parity-product-visual-parity-scene-v1",
    ...
    const paths = collectPostprocessSuiteEvidencePaths({
      rendering: {
        screenshotPaths: [
          ...
          "tests/reports/external-parity-example-screenshots/shadow-lab.png",
        ],
        validations: [
          {
            name: "postprocess-lab-runtime-color-management-controls",
            screenshotPath: "tests/reports/external-parity-example-screenshots/postprocess-lab-color-controls.png",
          },
          {
            name: "shadow-lab-pcf",
      ...
      hdrBrowser: {
        screenshotPaths: ["tests/reports/external-parity-hdr-render-target-postprocess.png"],
      },
    });
  ...
  it("records audited blockers for production and Unity/Unreal parity without marking them ready", () => {
    ...
    expect(unityUnreal.violations).toEqual(expect.arrayContaining([
      ...
      expect.stringContaining("tests/reports/external-parity-unity-baseline-render.json is missing"),
      expect.stringContaining("No Unity/Unreal rendered output"),
      expect.stringContaining("tests/reports/external-parity-unreal-baseline-render.json is missing"),
    ]));
  });
  ...
  it("summarizes browser runtime evidence without clearing native Unity/Unreal runtime parity", () => {
    const root = fixtureRoot();
    mkdirSync(join(root, "tests", "reports"), { recursive: true });
    writeFileSync(join(root, "tests", "reports", "external-parity-runtime.json"), JSON.stringify({
      ok: true,
  ...
  it("summarizes browser editor workflow evidence without clearing Unity/Unreal editor parity", () => {
    const root = fixtureRoot();
    mkdirSync(join(root, "tests", "reports"), { recursive: true });
    writeFileSync(join(root, "tests", "reports", "external-parity-editor-authoring.json"), JSON.stringify({
      ok: true,
      authoredWorkflow: {
    ...
    writeFileSync(join(root, "tests", "reports", "external-parity-editor-prefab-workflow.json"), JSON.stringify({
      ok: true,
  ...
  it("summarizes local rendering evidence without clearing Unity/Unreal rendered parity", () => {
    const root = fixtureRoot();
    mkdirSync(join(root, "tests", "reports"), { recursive: true });
    writeFileSync(join(root, "tests", "reports", "external-parity-rendering.json"), JSON.stringify({
      ok: true,
    ...
    writeFileSync(join(root, "tests", "reports", "external-parity-pbr-visual-parity.json"), JSON.stringify({ ok: true, boundedPbrVisualParity: { threejs: true, babylon: true } }, null, 2));
    writeFileSync(join(root, "tests", "reports", "external-parity-shadow-visual-parity.json"), JSON.stringify({ ok: true, boundedShadowVisualParity: { threejs: true, babylon: true } }, null, 2));
    writeFileSync(join(root, "tests", "reports", "external-parity-hdr-visual-parity.json"), JSON.stringify({ ok: true, boundedHdrRenderTargetParity: { threejs: true, babylon: true } }, null, 2));
  ...
  it("persists external evidence readiness summaries and next actions", () => {
    ...
    writePublicDeploymentFixture(root);
    writeFileSync(
      join(root, ".github", "workflows", "external-parity-external-engine-baselines.yml"),
      readFileSync(join(process.cwd(), ".github", "workflows", "external-parity-external-engine-baselines.yml"), "utf8")
    );
    writeFileSync(
    ...
    writeFileSync(join(root, "tests", "reports", "external-parity-external-engine-baselines.json"), JSON.stringify({
      ok: true,
      sceneSlots: [
        { baselineKind: "product-visual", targetReports: { unity: "tests/reports/external-parity-unity-product-visual-baseline.json", unreal: "tests/reports/external-parity-unreal-product-visual-baseline.json" } },
        { baselineKind: "pbr-visual", targetReports: { unity: "tests/reports/external-parity-unity-pbr-visual-baseline.json", unreal: "tests/reports/external-parity-unreal-pbr-visual-baseline.json" } },
        { baselineKind: "shadow-visual", targetReports: { unity: "tests/reports/external-parity-unity-shadow-visual-baseline.json", unreal: "tests/reports/external-parity-unreal-shadow-visual-baseline.json" } },
        { baselineKind: "hdr-render-target", targetReports: { unity: "tests/reports/external-parity-unity-hdr-render-target-baseline.json", unreal: "tests/reports/external-parity-unreal-hdr-render-target-baseline.json" } },
        { baselineKind: "postprocess-suite", targetReports: { unity: "tests/reports/external-parity-unity-postprocess-suite-baseline.json", unreal: "tests/reports/external-parity-unreal-postprocess-suite-baseline.json" } },
      ],
    }, null, 2));
    writeFileSync(join(root, "tests", "reports", "external-parity-unity-unreal-parity.json"), JSON.stringify({
      unityParity: false,
      unrealParity: false,
      replacement: false,
      editorEvidence: {
    ...
    writeFileSync(join(root, "tests", "reports", "external-parity-production-readiness.json"), JSON.stringify({ productionReady: false }, null, 2));
    writeFileSync(join(root, "tests", "reports", "external-parity-pbr-gltf-readiness.json"), JSON.stringify({
      pbrParity: false,
      gltfParity: false,
    ...
    expect(report.missingArtifactRunbookPath).toBe("tests/reports/external-parity-external-evidence-missing-artifacts.md");
    expect(report.localPreflight).toMatchObject({
      ...
      unity: {
        ...
        cliSmokeOptIn: false,
        smokeReportPath: "tests/reports/external-parity-unity-editor-cli-smoke.json",
      },
      unreal: {
        engine: "unreal",
    ...
    expect(report.areas).toEqual(expect.arrayContaining([
      expect.objectContaining({
        ...
        blockers: ["tests/reports/external-parity-github-external-readiness.json is missing; run pnpm audit:external-parity-github-external-readiness."],
      }),
    ]));
    ...
    expect(report.sourceFileHashes).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ path: "tools/external-parity-external-host-doctor/index.ts" }),
      expect.objectContaining({ path: "tools/external-parity-external-host-runner/index.ts" }),
      expect.objectContaining({ path: "tools/external-parity-external-evidence-handoff/index.ts" }),
      expect.objectContaining({ path: "tools/external-parity-external-evidence-readiness/index.ts" }),
    ]));
    ...
    expect(report.artifactChecklist).toEqual(expect.arrayContaining([
      expect.objectContaining({
        ...
        kind: "ci-workflow",
        ready: true,
        path: ".github/workflows/external-parity-external-engine-baselines.yml",
      }),
      expect.objectContaining({
      ...
      expect.objectContaining({
        ...
        requiredExternalEvidence: expect.arrayContaining([
          "Run the unity editor CLI smoke against a real Unity editor executable and write tests/reports/external-parity-unity-editor-cli-smoke.json with ok=true.",
        ]),
      }),
      expect.objectContaining({
        areaId: "unity-external-baselines",
      ...
      expect.objectContaining({
        ...
        ready: false,
        path: "tests/reports/external-parity-unreal-baseline-render.json",
        localEvidence: expect.arrayContaining([
          "unreal render workflow report command and baseline kit slots are prepared locally.",
        ]),
        requiredExternalEvidence: expect.arrayContaining([
          "Open/build/render the current baseline kit in a real Unreal editor workflow and write tests/reports/external-parity-unreal-baseline-render.json with ok=true.",
        ]),
      }),
      expect.objectContaining({
        areaId: "unity-external-baselines",
      ...
      expect.objectContaining({
        ...
        kind: "asset-import-workflow-report",
        ready: false,
        path: "tests/reports/external-parity-unreal-asset-import-workflow.json",
        expectedRunnerEvidencePath: "tests/reports/external-parity-unreal-asset-import-workflow.evidence.json",
      }),
      expect.objectContaining({
        ...
        ready: false,
        path: "tests/reports/external-parity-production-readiness.json",
        requiredExternalEvidence: expect.arrayContaining([
    ...
    expect(report.violations).toEqual(expect.arrayContaining([
      expect.stringContaining("unity-external-baselines/unity:editor-cli-smoke: tests/reports/external-parity-unity-editor-cli-smoke.json is missing"),
      expect.stringContaining("unity-external-baselines/unity:product-visual: tests/reports/external-parity-unity-product-visual-baseline.json is missing"),
    ]));
    ...
    expect(runbook).toContain("Target path: `tests/reports/external-parity-unity-editor-cli-smoke.json`");
    expect(runbook).toContain("- [ ] `unity:product-visual`");
    expect(runbook).toContain("Target path: `tests/reports/external-parity-unity-product-visual-baseline.json`");
  ...
  it("keeps the external evidence preflight wired through the runnable Unity and Unreal dry-run helpers", () => {
    ...
    expect(v4Readme).toContain("pnpm verify:external-parity-external-evidence-handoff");
    ...
    expect(v4Readme).toContain("tests/reports/external-parity-external-host-doctor.json");
    expect(v4Readme).toContain("tests/reports/external-parity-external-host-runner.json");
  ...
  it("audits GitHub external evidence prerequisites without dispatching workflows", () => {
    ...
    expect(readFileSync(join(root, "tests", "reports", "external-parity-github-external-readiness.json"), "utf8")).toContain("\"githubExternalReady\": true");
  });
  ...
  it("summarizes the current V4 parity status without mutating reports", () => {
    ...
    writeFileSync(join(root, "tests", "reports", "external-parity-external-evidence-readiness.json"), JSON.stringify({
      ...
      artifactChecklist: [{
        ...
        blockers: ["tests/reports/external-parity-unity-editor-cli-smoke.json is missing"],
      }],
    }, null, 2));
  ...
  it("writes an external evidence handoff inventory without clearing blocked parity artifacts", () => {
    ...
    writeFileSync(join(root, ".github", "workflows", "external-parity-external-engine-baselines.yml"), "name: external\n");
    ...
    writeFileSync(join(root, "release-artifacts", "v4-external-evidence-operator-runbook.md"), [
      ...
      "cp /path/to/external-parity-external-evidence-handoff/release-artifacts/v4-current-handoff-supplement.patch release-artifacts/",
      "```",
      "",
    ...
    writeFileSync(join(root, "release-artifacts", "v4-current-handoff-supplement.patch"), [
      "diff --git a/tools/external-parity-external-evidence-handoff/index.ts b/tools/external-parity-external-evidence-handoff/index.ts",
      "restorePreflight",
      "verificationScope",
    ...
    writeFileSync(join(root, "package.json"), readFileSync(join(process.cwd(), "package.json"), "utf8"));
      ...
      "tools/external-parity-current-capability/index.ts",
	      ...
	      "tools/external-parity-parity-status/index.ts",
      "tools/external-parity-local-port-status/index.ts",
      "tools/external-parity-external-host-runner/index.ts",
      "tools/external-parity-external-host-doctor/index.ts",
      ...
      "tools/external-parity-external-evidence-readiness/index.ts",
	      ...
	      "tools/external-parity-report-freshness/index.ts",
    ...
    writeFileSync(join(root, "tests", "reports", "external-parity-external-evidence-readiness.json"), JSON.stringify({
      ...
      artifactChecklist: [
        ...
        { areaId: "external-baseline-ci-workflow", id: "github-actions:self-hosted-unity-unreal-baselines", kind: "ci-workflow", ready: true, path: ".github/workflows/external-parity-external-engine-baselines.yml", blockers: [] },
      ],
    }, null, 2));
    ...
    const runbook = readFileSync(join(root, "tests", "reports", "external-parity-external-evidence-handoff.md"), "utf8");

    expect(report.ok).toBe(true);
    expect(report.handoffFilesReady).toBe(true);
    expect(report.packagedFilesReady).toBe(true);
    expect(report.packageDir).toBe("release-artifacts/external-parity-external-evidence-handoff");
    expect(report.packageArchivePath).toBe("release-artifacts/external-parity-external-evidence-handoff.tar.gz");
    expect(report.packageArchiveSha256Path).toBe("release-artifacts/external-parity-external-evidence-handoff.tar.gz.sha256");
    expect(report.transferManifestPath).toBe("release-artifacts/external-parity-external-evidence-handoff.transfer.json");
    expect(report.packageManifestPath).toBe("release-artifacts/external-parity-external-evidence-handoff/manifest.json");
    expect(report.packageReadmePath).toBe("release-artifacts/external-parity-external-evidence-handoff/START_HERE.md");
    expect(report.packageStandaloneVerifyScriptPath).toBe("release-artifacts/external-parity-external-evidence-handoff/VERIFY_PACKAGE_INTEGRITY.mjs");
    expect(report.packageRestoreScriptPath).toBe("release-artifacts/external-parity-external-evidence-handoff/RESTORE_INTO_CHECKOUT.mjs");
    expect(report.packageExternalHostScriptPath).toBe("release-artifacts/external-parity-external-evidence-handoff/RUN_EXTERNAL_HOST_PREFLIGHT.mjs");
    expect(report.commands.localRefresh).toContain("pnpm verify:external-parity-external-evidence-handoff");
    ...
    expect(report.files).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ path: "tests/reports/external-parity-external-host-doctor.json", exists: true, kind: "runbook" }),
      expect.objectContaining({ path: "tests/reports/external-parity-external-host-runner.json", exists: true, kind: "runbook" }),
      expect.objectContaining({ path: "tests/reports/external-parity-github-external-readiness.json", exists: true, kind: "runbook" }),
      ...
      expect.objectContaining({ path: "tools/external-parity-examples/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-claim-gates/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-assets/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-current-capability/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-reporting/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-parity-status/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-local-port-status/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-external-host-runner/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-github-external-readiness/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-external-engine-baselines/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-report-freshness/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-pbr-reference-readiness/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-shadow-map-readiness/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-hdr-render-target-readiness/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-production-readiness/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-pbr-gltf-readiness/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-ecosystem-readiness/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-broad-parity-readiness/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-completion-audit/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-product-visual-parity/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-product-visual-parity/productScene.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-pbr-visual-parity/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-shadow-visual-parity/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-hdr-visual-parity/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-postprocess-suite/index.ts", exists: true, kind: "tooling" }),
      expect.objectContaining({ path: "tools/external-parity-unity-unreal-parity/index.ts", exists: true, kind: "tooling" }),
    ...
    expect(report.packagedFiles).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({
        ...
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/release-artifacts/external-demos/0.1.0-alpha.0/public-deployment-manifest.json",
        copied: true,
      }),
      expect.objectContaining({
        path: "tests/reports/external-parity-product-visual-parity/threejs-product-diff.png",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tests/reports/external-parity-product-visual-parity/threejs-product-diff.png",
        copied: true,
      }),
      expect.objectContaining({
        path: "tests/reports/external-parity-external-host-runner.json",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tests/reports/external-parity-external-host-runner.json",
        copied: true,
      }),
      expect.objectContaining({
        path: "docs/project/v4-parity-execution-prompt.md",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/docs/project/v4-parity-execution-prompt.md",
        copied: true,
      }),
      expect.objectContaining({
        path: "release-artifacts/v4-current-handoff-supplement.patch",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/release-artifacts/v4-current-handoff-supplement.patch",
        copied: true,
      }),
      expect.objectContaining({
        path: "tests/unit/tools/external-parity-validation.test.ts",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tests/unit/tools/external-parity-validation.test.ts",
        copied: true,
      }),
      expect.objectContaining({
        path: "tools/external-parity-production-readiness/index.ts",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tools/external-parity-production-readiness/index.ts",
        copied: true,
      }),
      expect.objectContaining({
        path: "tools/external-parity-broad-parity-readiness/index.ts",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tools/external-parity-broad-parity-readiness/index.ts",
        copied: true,
      }),
      expect.objectContaining({
        path: "tools/external-parity-completion-audit/index.ts",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tools/external-parity-completion-audit/index.ts",
        copied: true,
      }),
      expect.objectContaining({
        path: "tools/external-parity-parity-status/index.ts",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tools/external-parity-parity-status/index.ts",
        copied: true,
      }),
      expect.objectContaining({
        path: "tools/external-parity-local-port-status/index.ts",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tools/external-parity-local-port-status/index.ts",
        copied: true,
      }),
      expect.objectContaining({
        path: "tools/external-parity-external-engine-baselines/index.ts",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tools/external-parity-external-engine-baselines/index.ts",
        copied: true,
      }),
      expect.objectContaining({
        path: "tools/external-parity-report-freshness/index.ts",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tools/external-parity-report-freshness/index.ts",
        copied: true,
      }),
      expect.objectContaining({
        path: "tools/external-parity-product-visual-parity/index.ts",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tools/external-parity-product-visual-parity/index.ts",
        copied: true,
      }),
      expect.objectContaining({
        path: "tools/external-parity-product-visual-parity/productScene.ts",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tools/external-parity-product-visual-parity/productScene.ts",
        copied: true,
      }),
      expect.objectContaining({
        path: "tools/external-parity-pbr-visual-parity/index.ts",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tools/external-parity-pbr-visual-parity/index.ts",
        copied: true,
      }),
      expect.objectContaining({
        path: "tools/external-parity-shadow-visual-parity/index.ts",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tools/external-parity-shadow-visual-parity/index.ts",
        copied: true,
      }),
      expect.objectContaining({
        path: "tools/external-parity-hdr-visual-parity/index.ts",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tools/external-parity-hdr-visual-parity/index.ts",
        copied: true,
      }),
      expect.objectContaining({
        path: "tools/external-parity-postprocess-suite/index.ts",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tools/external-parity-postprocess-suite/index.ts",
        copied: true,
      }),
      expect.objectContaining({
        path: "tools/external-parity-unity-unreal-parity/index.ts",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tools/external-parity-unity-unreal-parity/index.ts",
        copied: true,
      }),
      expect.objectContaining({
        path: "tools/static-demo-server-smoke/index.ts",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tools/static-demo-server-smoke/index.ts",
        copied: true,
      }),
      expect.objectContaining({
        path: "tools/package-provenance/index.ts",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tools/package-provenance/index.ts",
        copied: true,
      }),
      expect.objectContaining({
        path: "tools/compare-engines/index.ts",
        packagePath: "release-artifacts/external-parity-external-evidence-handoff/tools/compare-engines/index.ts",
        copied: true,
      }),
    ]));
    ...
    expect(verified).toMatchObject({
      ...
      packageManifestPath: "release-artifacts/external-parity-external-evidence-handoff/manifest.json",
      packageArchivePath: "release-artifacts/external-parity-external-evidence-handoff.tar.gz",
      packageArchiveSha256Path: "release-artifacts/external-parity-external-evidence-handoff.tar.gz.sha256",
      transferManifestPath: "release-artifacts/external-parity-external-evidence-handoff.transfer.json",
      violations: [],
    });
    const recordedReport = JSON.parse(readFileSync(join(root, "tests", "reports", "external-parity-external-evidence-handoff.json"), "utf8")) as {
    ...
    const recordedTransferManifest = JSON.parse(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff.transfer.json"), "utf8")) as {
      readonly packageVerification?: {
        readonly ok?: boolean;
        readonly verificationScope?: {
          readonly packageInternalEntries?: boolean;
    ...
    const packageReadme = readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "START_HERE.md"), "utf8");
    ...
    const operatorRunbook = readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "release-artifacts", "v4-external-evidence-operator-runbook.md"), "utf8");
    expect(operatorRunbook).toContain("mkdir -p release-artifacts");
    expect(operatorRunbook).toContain("cp /path/to/external-parity-external-evidence-handoff/release-artifacts/v4-current-handoff-supplement.patch release-artifacts/");
    ...
    const packagedPrBody = readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "release-artifacts", "v4-parity-external-evidence-pr.md"), "utf8");
    ...
    const restoreScript = readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "RESTORE_INTO_CHECKOUT.mjs"), "utf8");
    ...
    expect(restoreScript).toContain("\"tools/external-parity-reporting\"");
    expect(restoreScript).toContain("\"tools/external-parity-parity-status\"");
    expect(restoreScript).toContain("\"tools/external-parity-local-port-status\"");
    expect(restoreScript).toContain("\"tools/external-parity-github-external-readiness\"");
    expect(restoreScript).toContain("\"tools/external-parity-external-engine-baselines\"");
    expect(restoreScript).toContain("\"tools/external-parity-product-visual-parity\"");
    expect(restoreScript).toContain("\"tools/external-parity-postprocess-suite\"");
    expect(restoreScript).toContain("\"tools/compare-engines\"");
    expect(restoreScript).toContain("\"pnpm status:v4-local-port\"");
    const runnerReportPath = join(root, "tests", "reports", "external-parity-external-host-runner.json");
    ...
    try {
      ...
      expect(blockedRunner).toMatchObject({
        ...
        reportPath: "tests/reports/external-parity-external-host-runner.json",
      });
      ...
      expect(readFileSync(join(root, "tests", "reports", "external-parity-external-host-doctor.json"), "utf8")).toContain("\"externalHostReady\": true");
      expect(readFileSync(join(root, "tests", "reports", "external-parity-external-host-doctor.json"), "utf8")).toContain("\"externalEvidenceReady\": false");
      expect(readFileSync(join(root, "tests", "reports", "external-parity-external-host-doctor.json"), "utf8")).toContain("\"firstBlockedArtifact\": \"unity:editor-cli-smoke\"");
      expect(readFileSync(join(root, "tests", "reports", "external-parity-external-host-doctor.json"), "utf8")).toContain("\"firstBlockedArtifactDetails\"");
      ...
      expect(readFileSync(join(root, "tests", "reports", "external-parity-external-host-runner.json"), "utf8")).toContain("\"readyToExecute\": true");
      expect(readFileSync(join(root, "tests", "reports", "external-parity-external-host-runner.json"), "utf8")).toContain("\"expectedEvidencePaths\"");
      expect(readFileSync(join(root, "tests", "reports", "external-parity-external-host-runner.json"), "utf8")).toContain("\"firstBlockedArtifact\": \"unity:editor-cli-smoke\"");
    ...
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff.tar.gz")).byteLength).toBeGreaterThan(1024);
    const archiveShaSidecar = readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff.tar.gz.sha256"), "utf8");
    expect(archiveShaSidecar).toContain("release-artifacts/external-parity-external-evidence-handoff.tar.gz");
    expect(archiveShaSidecar.trim().split(/\s+/u)[0]).toHaveLength(64);
    const transferManifest = JSON.parse(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff.transfer.json"), "utf8")) as {
    ...
    expect(transferManifest.packageArchiveSha256Path).toBe("release-artifacts/external-parity-external-evidence-handoff.tar.gz.sha256");
    expect(transferManifest.packageManifestPath).toBe("release-artifacts/external-parity-external-evidence-handoff/manifest.json");
    ...
    expect(transferManifest.packageEntryPoints).toMatchObject({
      standaloneVerify: "release-artifacts/external-parity-external-evidence-handoff/VERIFY_PACKAGE_INTEGRITY.mjs",
      restoreIntoCheckout: "release-artifacts/external-parity-external-evidence-handoff/RESTORE_INTO_CHECKOUT.mjs",
      externalHostPreflight: "release-artifacts/external-parity-external-evidence-handoff/RUN_EXTERNAL_HOST_PREFLIGHT.mjs",
    });
    ...
    expect(restoreDryRun.restored).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ entry: "tools/external-parity-external-host-runner", kind: "directory" }),
      expect.objectContaining({ entry: "tools/external-parity-external-engine-baselines", kind: "directory" }),
      expect.objectContaining({ entry: "tools/external-parity-product-visual-parity", kind: "directory" }),
      expect.objectContaining({ entry: "tools/external-parity-postprocess-suite", kind: "directory" }),
    ...
    writeFileSync(join(root, "tests", "reports", "external-parity-external-host-doctor.json"), JSON.stringify({
      externalHostReady: false,
      handoffPackageReady: true,
      externalEvidenceReady: false,
      firstMissingCapability: "unity-editor-executable",
    ...
    const standaloneVerifyScript = join(root, "release-artifacts", "external-parity-external-evidence-handoff", "VERIFY_PACKAGE_INTEGRITY.mjs");
    ...
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "START_HERE.md"), "utf8")).toContain("## Transfer Manifest Verification");
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "START_HERE.md"), "utf8")).toContain("release-artifacts/external-parity-external-evidence-handoff.transfer.json.packageVerification");
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "START_HERE.md"), "utf8")).toContain("checks package-internal files only");
    ...
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "RESTORE_INTO_CHECKOUT.mjs"), "utf8")).toContain("dryRun");
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "RUN_EXTERNAL_HOST_PREFLIGHT.mjs"), "utf8")).toContain("RUN_EXTERNAL_HOST_PREFLIGHT");
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "RUN_EXTERNAL_HOST_PREFLIGHT.mjs"), "utf8")).toContain("pnpm");
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "RUN_EXTERNAL_HOST_PREFLIGHT.mjs"), "utf8")).toContain("doctor:v4-external-host:strict");
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "tests", "reports", "external-parity-external-evidence-handoff.json"), "utf8")).toContain("\"claimBoundary\"");
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "tests", "reports", "external-parity-external-evidence-handoff.md"), "utf8")).toContain("# V4 External Evidence Handoff");
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "docs/project/v4-parity-execution-prompt.md"), "utf8")).toContain("Goal 12: Full glTF Parity");
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "docs/project/v4-parity-execution-prompt.md"), "utf8")).toContain("Goal 13: Full WebGPU Parity");
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "release-artifacts", "v4-external-evidence-operator-runbook.md"), "utf8")).toContain("External Evidence Operator Runbook");
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "release-artifacts", "v4-external-evidence-operator-runbook.md"), "utf8")).toContain("Handoff integrity is now split by scope");
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "release-artifacts", "v4-external-evidence-operator-runbook.md"), "utf8")).toContain("\"restorePreflight\"");
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "release-artifacts", "v4-parity-external-evidence-pr.md"), "utf8")).toContain("V4 Parity Execution");
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "release-artifacts", "v4-parity-external-evidence-pr.md"), "utf8")).toContain("Repo-side handoff verification scope");
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "release-artifacts", "v4-parity-external-evidence-pr.md"), "utf8")).toContain("Standalone operator package verification also passed");
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "release-artifacts", "codingrelated-completion-audit.md"), "utf8")).toContain("Codingrelated Completion Audit");
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "release-artifacts", "codingrelated-completion-audit.md"), "utf8")).toContain("Standalone package verification scope");
    expect(readFileSync(join(root, "release-artifacts", "external-parity-external-evidence-handoff", "release-artifacts", "codingrelated-completion-audit.md"), "utf8")).toContain("current handoff set");
    ...
    expect(readFileSync(join(root, "tests", "reports", "external-parity-external-evidence-handoff.json"), "utf8")).toContain("\"claimBoundary\"");
    expect(runbook).toContain("# V4 External Evidence Handoff");
    expect(runbook).toContain("Package directory: `release-artifacts/external-parity-external-evidence-handoff`");
    expect(runbook).toContain("Package archive: `release-artifacts/external-parity-external-evidence-handoff.tar.gz`");
    expect(runbook).toContain("Package archive checksum: `release-artifacts/external-parity-external-evidence-handoff.tar.gz.sha256`");
    expect(runbook).toContain("Transfer manifest: `release-artifacts/external-parity-external-evidence-handoff.transfer.json`");
    expect(runbook).toContain("Package readme: `release-artifacts/external-parity-external-evidence-handoff/START_HERE.md`");
    expect(runbook).toContain("Package standalone integrity script: `release-artifacts/external-parity-external-evidence-handoff/VERIFY_PACKAGE_INTEGRITY.mjs`");
    expect(runbook).toContain("Package restore script: `release-artifacts/external-parity-external-evidence-handoff/RESTORE_INTO_CHECKOUT.mjs`");
    expect(runbook).toContain("Package external-host preflight script: `release-artifacts/external-parity-external-evidence-handoff/RUN_EXTERNAL_HOST_PREFLIGHT.mjs`");
    expect(runbook).toContain("unity-external-baselines/unity:editor-cli-smoke");
    expect(runbook).toContain("pnpm run:v4-external-host-evidence");
    expect(runbook).toContain("pnpm preflight:v4-parity:after-external-evidence");

    const supplementPatchPath = join(root, "release-artifacts", "external-parity-external-evidence-handoff", "release-artifacts", "v4-current-handoff-supplement.patch");
  ...
  it("does not count ok-only external scene reports as ready without runner sidecar evidence", () => {
    ...
    writeFileSync(join(root, "tests", "reports", "external-parity-external-engine-baselines.json"), JSON.stringify({
      ...
      sceneSlots: [{
        ...
        targetReports: {
          unity: "tests/reports/external-parity-unity-pbr-visual-baseline.json"
        }
      }],
    }, null, 2));
    writeFileSync(join(root, "tests", "reports", "v4-unity-pbr-visual-baseline.json"), JSON.stringify({
      ...
      screenshotPath: "tests/reports/external-parity-pbr-visual/unity-pbr-visual-baseline.png",
      metrics: {
        width: 960,
        height: 540,
    ...
    writeFileSync(join(root, "tests", "reports", "external-parity-external-engine-baselines.json"), JSON.stringify({
      ...
      sceneSlots: [{
        ...
        targetReports: {
          ...
          unreal: "tests/reports/external-parity-unreal-product-visual-baseline.json"
        }
      }]
    }, null, 2));
  ...
  it("packages external baseline report writer and current Galileo reference screenshots", () => {
    ...
    expect(baselineSchema.allOf).toEqual(expect.arrayContaining([
      expect.objectContaining({
        ...
        then: expect.objectContaining({
          properties: expect.objectContaining({
            ...
            sceneDescriptorVersion: { const: "external-parity-product-visual-parity-scene-v1" },
      ...
      expect.objectContaining({
        ...
        then: expect.objectContaining({
          properties: expect.objectContaining({
            sceneDescriptorId: { const: "external-parity-pbr-visual-parity-scene" },
            metrics: expect.objectContaining({
    ...
    expect(commandPlan.captures).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({
        engine: "unreal",
        baselineKind: "postprocess-suite",
        targetReportPath: "tests/reports/external-parity-unreal-postprocess-suite-baseline.json",
      }),
    ]));
    expect(commandPlan.blockedUntilAllCapturesPass).toContain("Unity/Unreal replacement language");
    expect(baselineRunbook).toContain("pnpm audit:external-parity-external-evidence-readiness");
    ...
    expect(baselineRunbook).toContain("tests/reports/external-parity-external-evidence-missing-artifacts.md");
    ...
    expect(JSON.stringify(ingestDryRun)).toContain("tests/reports/external-parity-unity-editor-cli-smoke.json");
    expect(JSON.stringify(ingestDryRun)).toContain("tests/reports/external-parity-unreal-editor-cli-smoke.json");
    expect(JSON.stringify(ingestDryRun)).toContain("tests/reports/external-parity-unity-asset-import-workflow.json");
    expect(JSON.stringify(ingestDryRun)).toContain("tests/reports/external-parity-unity-asset-import-workflow.evidence.json");
    expect(JSON.stringify(ingestDryRun)).toContain("tests/reports/external-parity-unreal-asset-import-workflow.json");
    expect(JSON.stringify(ingestDryRun)).toContain("tests/reports/external-parity-unreal-asset-import-workflow.evidence.json");
    expect(JSON.stringify(ingestDryRun)).toContain("tests/reports/external-parity-product-visual/unity-product-visual-baseline.png.evidence.json");
  }, 15_000);

  it("provides a manual self-hosted workflow for external Unity and Unreal baselines", () => {
    const workflow = readFileSync(join(process.cwd(), ".github", "workflows", "external-parity-external-engine-baselines.yml"), "utf8");
    ...
    expect(workflow).toContain("merge-multiple: true");
	    ...
	    expect(workflow).toContain("pnpm audit:external-parity-pbr-reference-readiness || true");
	    expect(workflow).toContain("pnpm audit:external-parity-pbr-gltf-readiness || true");
	    expect(workflow).toContain("pnpm audit:external-parity-shadow-map-readiness || true");
	    expect(workflow).toContain("pnpm audit:external-parity-hdr-render-target-readiness || true");
	    expect(workflow).toContain("pnpm audit:external-parity-postprocess-suite || true");
	    expect(workflow).toContain("v4-external-baseline-final-audits");
	    expect(workflow).toContain("actions/upload-artifact@v4");
    expect(workflow).toContain("tests/reports/external-parity-unity-asset-import-workflow.json");
    expect(workflow).toContain("tests/reports/external-parity-unity-asset-import-workflow.evidence.json");
    expect(workflow).toContain("tests/reports/external-parity-unity-product-visual-baseline.json");
    expect(workflow).toContain("tests/reports/external-parity-unity-pbr-visual-baseline.json");
    expect(workflow).toContain("tests/reports/external-parity-unity-shadow-visual-baseline.json");
    expect(workflow).toContain("tests/reports/external-parity-unity-hdr-render-target-baseline.json");
    expect(workflow).toContain("tests/reports/external-parity-unity-postprocess-suite-baseline.json");
    expect(workflow).toContain("tests/reports/external-parity-unreal-asset-import-workflow.json");
    expect(workflow).toContain("tests/reports/external-parity-unreal-asset-import-workflow.evidence.json");
    expect(workflow).toContain("tests/reports/external-parity-unreal-product-visual-baseline.json");
    expect(workflow).toContain("tests/reports/external-parity-unreal-pbr-visual-baseline.json");
    expect(workflow).toContain("tests/reports/external-parity-unreal-shadow-visual-baseline.json");
    expect(workflow).toContain("tests/reports/external-parity-unreal-hdr-render-target-baseline.json");
	    expect(workflow).toContain("tests/reports/external-parity-unreal-postprocess-suite-baseline.json");
	    expect(workflow).toContain("tests/reports/external-parity-pbr-reference-readiness.json");
	    expect(workflow).toContain("tests/reports/external-parity-pbr-gltf-readiness.json");
	    expect(workflow).toContain("tests/reports/external-parity-shadow-map-readiness.json");
	    expect(workflow).toContain("tests/reports/external-parity-hdr-render-target-readiness.json");
	    expect(workflow).toContain("tests/reports/external-parity-postprocess-suite.json");
	    expect(workflow).toContain("tests/reports/external-parity-external-evidence-missing-artifacts.md");
    expect(workflow).toContain("tests/reports/external-parity-completion-audit-runbook.md");
    expect(workflow).toContain("pnpm audit:external-parity-unity-unreal-parity || true");
  });
  ...
  it("ingests downloaded public demo deployment report artifacts without accepting unrelated files", () => {
    ...
    writeFileSync(join(artifactRoot, "external-parity-production-readiness.json"), JSON.stringify({ productionReady: true }, null, 2));
    ...
    expect(dryRun.copied).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ path: "tests/reports/external-parity-production-readiness.json" }),
    ]));
    ...
    expect(readFileSync(join(root, "tests", "reports", "external-parity-production-readiness.json"), "utf8")).toContain("\"productionReady\": true");
  ...
  it("does not count full WebGPU completion from a bare boolean without the evidence matrix", () => {
    const root = fixtureRoot();
    mkdirSync(join(root, "tests", "reports"), { recursive: true });
    writeFileSync(join(root, "tests", "reports", "external-parity-broad-parity-readiness.json"), JSON.stringify({ claims: [] }, null, 2));
    ...
    writeFileSync(join(root, "tests", "reports", "external-parity-hdr-render-target-readiness.json"), JSON.stringify({}, null, 2));
    writeFileSync(join(root, "tests", "reports", "external-parity-shadow-map-readiness.json"), JSON.stringify({}, null, 2));
    writeFileSync(join(root, "tests", "reports", "external-parity-postprocess-suite.json"), JSON.stringify({}, null, 2));
    writeFileSync(join(root, "tests", "reports", "external-parity-product-visual-parity.json"), JSON.stringify({}, null, 2));

    const report = createV4CompletionAuditReport(root);
    const webgpu = report.criteria.find((entry) => entry.id === "full-webgpu-parity");
    const runbook = readFileSync(join(root, "tests", "reports", "external-parity-completion-audit-runbook.md"), "utf8");

    expect(report.completionRunbookPath).toBe("tests/reports/external-parity-completion-audit-runbook.md");
    expect(report.externalEvidenceRunbookPath).toBe("tests/reports/external-parity-external-evidence-missing-artifacts.md");
    ...
    expect(runbook).toContain("pnpm preflight:v4-parity");
    expect(runbook).toContain("pnpm refresh:v4-readiness-reports");
    expect(runbook).toContain("tests/reports/external-parity-external-evidence-missing-artifacts.md");
  });
