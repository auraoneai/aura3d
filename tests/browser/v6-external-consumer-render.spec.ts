...
test.describe("V6 external consumer render", () => {
  ...
  test("renders the built external Vite consumer from the packed package", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/tests/reports/v6-external-consumer-preview/index.html`, { waitUntil: "domcontentloaded" });
    ...
    mkdirSync(resolve("tests/reports/v6-external-consumer"), { recursive: true });
    const screenshot = "tests/reports/v6-external-consumer/external-consumer-render.png";
    await page.locator("#viewport").screenshot({ path: screenshot });
    writeFileSync(resolve("tests/reports/v6-external-consumer-render.json"), `${JSON.stringify({
      schema: "g3d-v6-external-consumer-render/v1",
