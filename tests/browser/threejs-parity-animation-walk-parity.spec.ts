...
test.describe("Three.js parity animation walk same-asset Three.js parity", () => {
  ...
  test("samples Soldier Walk through A3D and actual Three.js AnimationMixer", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/tools/threejs-parity-animation-walk-parity/index.html`, { waitUntil: "domcontentloaded" });
    await page.waitForFunction(
      () => {
        const result = window.__THREEJS_PARITY_ANIMATION_WALK_PARITY__ as { readonly status?: string } | undefined;
    ...
    const result = await page.evaluate(() => window.__THREEJS_PARITY_ANIMATION_WALK_PARITY__) as AnimationWalkParityResult;
    ...
    expect(result.schema).toBe("a3d-threejs-parity-animation-walk-parity");
...
type AnimationWalkParityResult =
  ...
  | { readonly status: "error"; readonly schema: "a3d-threejs-parity-animation-walk-parity"; readonly error: string };
