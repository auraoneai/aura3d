...
test("Animation Studio Pro has real app screenshot evidence and runtime state", async () => {
  const source = JSON.parse(readFileSync("tests/reports/v4-character-viewer-browser.json", "utf8")) as {
    ok?: boolean;
  ...
  writeFileSync(join(process.cwd(), "tests/reports/v4-animation-studio-pro-browser.json"), `${JSON.stringify({ ok, sourceReport: "tests/reports/v4-character-viewer-browser.json" }, null, 2)}\n`);
  expect(ok).toBe(true);
});
