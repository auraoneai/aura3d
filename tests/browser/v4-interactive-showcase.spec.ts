...
const reportPath = "tests/reports/v4-interactive-showcase-browser.json";
const screenshotDir = "tests/reports/v4-gallery/interactive";
...
test.describe("V4 interactive showcase", () => {
  ...
  test("supports camera, selection, and variant interaction in example and app", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/examples/interactive-showcase-v4/index.html`, { waitUntil: "domcontentloaded" });
    const example = await waitForState(page, "interactive-showcase-v4");
    await page.locator("[data-testid='hr4-interactive-canvas']").screenshot({ path: `${screenshotDir}/interactive-showcase-v4.png` });
    ...
    await page.locator("[data-testid='hr4-interactive-canvas']").screenshot({ path: `${screenshotDir}/interactive-showcase-v4-interacted.png` });
    ...
    const report = { ok: errors.length === 0 && passes(example, "interactive-showcase-v4") && passes(interacted, "interactive-showcase-v4") && passes(app, "interactive-showcase-pro") && interacted.variant === "emissive" && interacted.cameraOrbitDegrees === 42, generatedAt: new Date().toISOString(), screenshots: [`${screenshotDir}/interactive-showcase-v4.png`, `${screenshotDir}/interactive-showcase-v4-interacted.png`, `${screenshotDir}/interactive-showcase-pro.png`], productBoundary: "Milestone 12 proves interaction product state. Full V4 release still requires production 3D interaction parity, packaged API proof, and same-scene Three.js comparison.", errors, states: { example, interacted, app } };
