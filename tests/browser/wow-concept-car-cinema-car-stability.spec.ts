...
test.describe("wow neon city car delayed environment stability", () => {
  ...
  test("captures early and late car frames to isolate delayed HDR speckle source", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/apps/wow-concept-car-cinema/`, { waitUntil: "domcontentloaded" });
    ...
    const earlyPath = join(REPORT_DIR, "wow-concept-car-cinema-car-early.png");
    ...
    const latePath = join(REPORT_DIR, "wow-concept-car-cinema-car-late.png");
    ...
    const report = {
      schema: "a3d-wow-concept-car-cinema-car-delayed-environment-diagnostic",
      source: "tests/browser/wow-concept-car-cinema-car-stability.spec.ts",
      route: "apps/wow-concept-car-cinema",
      asset: "car-concept",
    ...
    writeFileSync(join(REPORT_DIR, "wow-concept-car-cinema-car-stability.json"), `${JSON.stringify(report, null, 2)}\n`);
  });
});
