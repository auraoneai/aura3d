...
test.describe("V6 WebGPU report", () => {
  ...
  test("reports real browser WebGPU availability or explicit hardware-unavailable status", async ({ page }) => {
    ...
    const report = await page.evaluate(async () => {
      const module = await import("/packages/rendering/src/v6/index.js");
    ...
    expect(report.schema).toBe("g3d-v6-webgpu-report/v1");
    ...
    const reportPath = "tests/reports/v6-webgpu-browser-report.json";
    ...
    expect(readiness.safetyChecks.find((item) => item.id === "renderer-v6-webgpu-uses-production-webgpu-path")?.status).toBe("ready");
    ...
    const readinessPath = "tests/reports/v7/webgpu-readiness.json";
