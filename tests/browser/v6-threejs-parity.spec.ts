...
test.describe("V6 same-scene Three.js parity", () => {
  ...
  test("renders 12 real GLB scenes through G3D and Three.js with diff metrics", async ({ page }) => {
    ...
    mkdirSync(resolve("tests/reports/v6-threejs-parity"), { recursive: true });
    mkdirSync(resolve("tests/reports/v6-gallery/threejs-comparison"), { recursive: true });
    for (const scene of result.results ?? []) {
      await saveCanvasPng(page, `${scene.id}-g3d`, `tests/reports/v6-threejs-parity/${scene.id}-g3d.png`);
      await saveCanvasPng(page, `${scene.id}-threejs`, `tests/reports/v6-threejs-parity/${scene.id}-threejs.png`);
      await saveCanvasPng(page, `${scene.id}-diff`, `tests/reports/v6-threejs-parity/${scene.id}-diff.png`);
    }
    const galleryPairs = [
    ...
    for (const item of galleryPairs) {
      await saveCanvasPng(page, `${item.sceneId}-g3d`, `tests/reports/v6-gallery/threejs-comparison/${item.galleryId}-g3d.png`);
      await saveCanvasPng(page, `${item.sceneId}-threejs`, `tests/reports/v6-gallery/threejs-comparison/${item.galleryId}-threejs.png`);
      await saveCanvasPng(page, `${item.sceneId}-diff`, `tests/reports/v6-gallery/threejs-comparison/${item.galleryId}-diff.png`);
    }
    writeFileSync(resolve("tests/reports/v6-threejs-parity/browser-report.json"), `${JSON.stringify({
      schema: "g3d-v6-threejs-parity-browser/v1",
      generatedAt: new Date().toISOString(),
      screenshots: (result.results ?? []).flatMap((scene) => [
        `tests/reports/v6-threejs-parity/${scene.id}-g3d.png`,
        `tests/reports/v6-threejs-parity/${scene.id}-threejs.png`,
        `tests/reports/v6-threejs-parity/${scene.id}-diff.png`
      ]),
      ...result
    }, null, 2)}\n`);
  });
