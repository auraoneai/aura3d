...
const reportPath = "tests/reports/v4-gltf-visual-corpus-browser.json";
...
test.describe("V4 glTF visual corpus browser evidence", () => {
  ...
  test("loads corpus manifest in browser and renders a human-inspectable coverage board", async ({ page }) => {
    ...
    const screenshotPath = "tests/reports/v4-gallery/assets/gltf-corpus-coverage.png";
    mkdirSync(join(process.cwd(), "tests/reports/v4-gallery/assets"), { recursive: true });
