...
const report: V4MaterialFidelityReport = {
  ...
  command: "pnpm exec playwright test tests/browser/asset-material-fidelity-v4.spec.ts",
...
test.describe("V4 asset material fidelity report", () => {
  ...
  test.afterAll(async () => {
    ...
    report.completedTaskEvidence = [{
      task: "`tests/reports/v4-asset-material-fidelity.json` passes.",
      evidence: [
    ...
    const reportPath = resolve("tests/reports/v4-asset-material-fidelity.json");
