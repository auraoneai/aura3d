...
test.describe("rendering WebGL2 device", () => {
  ...
  test("renders a public Renderer triangle, line segments, points, unlit cube, PBR sphere, lit cube, textured cube, optional texture fallback, normal map, scene morph target, GPU morph target, instanced unlit and PBR triangles, emissive/environment materials, point/spot light attenuation, GPU buffer readback, and render target readback", async ({ page }) => {
    ...
    expect(result?.pbrDiagnostics?.lastError).toBeNull();
    ...
    expect(sr).toBeGreaterThan(80);
    expect(sg).toBeGreaterThan(60);
    expect(sb).toBeGreaterThan(25);
    expect(sa).toBe(255);
    expect(sr + sg + sb).toBeGreaterThan(rr + rg + rb + 80);
    expect(ra).toBe(255);

    const [cbr = 0, cbg = 0, cbb = 0, cba = 0] = result?.litCubePixel ?? [];
    expect(cbr).toBeGreaterThan(40);
    expect(cbg).toBeGreaterThan(80);
    expect(cbb).toBeGreaterThan(110);
    expect(cba).toBe(255);

    const [tr = 0, tg = 0, tb = 0, ta = 0] = result?.texturedCubePixel ?? [];
    expect(tr).toBeLessThan(80);
    expect(tg).toBeGreaterThan(180);
    expect(tb).toBeGreaterThan(80);
    expect(ta).toBe(255);
    ...
    expect(ipr).toBeLessThan(90);
    expect(ipg).toBeGreaterThan(130);
    expect(ipb).toBeGreaterThan(150);
    expect(ipa).toBe(255);
    expect(jpr).toBeLessThan(90);
    expect(jpg).toBeGreaterThan(130);
    expect(jpb).toBeGreaterThan(150);
    expect(jpa).toBe(255);

    const [er = 0, eg = 0, eb = 0, ea = 0] = result?.emissivePixel ?? [];
    expect(eg).toBeGreaterThan(er + 60);
