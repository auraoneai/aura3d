...
test("V5 loader corpus browser proof renders loader diagnostics", async ({ page }) => {
  const diagnostics = [
    new GLTFLoaderV5().load("fixtures/v5/assets/corpus/damaged-helmet.glb").diagnostic,
    new OBJLoaderV5().load("fixtures/v5/loaders/sample.obj").diagnostic,
    new HDRLoaderV5().load("fixtures/v5/environments/hdri/studio_small_08_1k.hdr"),
    new KTX2LoaderV5().load("tests/assets/corpus/ktx2/Rib_N.ktx2"),
    new TextureLoaderV5().load("tests/reports/v4-hdr-visual-parity/galileo-hdr.png")
  ];
