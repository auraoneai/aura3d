...
test.describe("decals same-scene Three.js parity", () => {
  ...
  test("captures A3D projected decals against actual Three.js DecalGeometry", async ({ page }) => {
    ...
    await page.waitForFunction(
      () => {
        const result = window.__THREEJS_PARITY_DECALS_PARITY__ as { readonly status?: string } | undefined;
    ...
    const result = await page.evaluate(() => window.__THREEJS_PARITY_DECALS_PARITY__) as DecalsParityResult;
    const report = {
    ...
    expect(result.schema).toBe("a3d-threejs-parity-decals-parity");
