...
test("Interactive Showcase Pro has real app screenshot evidence and runtime state", async () => {
  const source = JSON.parse(readFileSync("tests/reports/v4-interactive-showcase-browser.json", "utf8")) as {
    ok?: boolean;
  ...
  writeFileSync(join(process.cwd(), "tests/reports/v4-interactive-showcase-pro-browser.json"), `${JSON.stringify({ ok, sourceReport: "tests/reports/v4-interactive-showcase-browser.json" }, null, 2)}\n`);
  expect(ok).toBe(true);
});
