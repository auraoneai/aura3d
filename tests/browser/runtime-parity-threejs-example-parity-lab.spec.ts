...
test.describe("V7 Three.js example parity lab", () => {
  ...
  test("renders the first G3D V7 parity lab for animation, IK, decals, stereo, and physics", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/apps/example-parity-lab/`, { waitUntil: "domcontentloaded" });
    try {
    ...
    expect(runtime.appId).toBe("example-parity-lab");
    ...
    const reportDir = "tests/reports/runtime-parity/threejs-example-parity-lab";
    mkdirSync(resolve(reportDir), { recursive: true });
    const screenshotPath = `${reportDir}/example-parity-lab.png`;
