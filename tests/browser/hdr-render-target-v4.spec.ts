...
const reportPath = "tests/reports/v4-hdr-render-target-browser.json";
  ...
  test("creates an rgba32f WebGL2 render target and reads overbright float pixels", async ({ page }) => {
    ...
    const report = {
      ok: errors.length === 0 &&
        ...
        state.featureEvidence.browserFloatFramebuffer === true &&
    ...
    expect(Number(state?.metrics.sampleG ?? 0)).toBeCloseTo(0.5, 2);
    ...
    expect(Number(state?.metrics.textureBytes ?? 0)).toBe(16 * 16 * 16);
    expect(Number(state?.diagnostics.drawCalls ?? 0)).toBe(1);
  });
});
