...
test.describe("V7 contact shadow parity artifact", () => {
  ...
  test("captures a same-scene G3D vs Three.js contact-shadow delta", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/tests/browser/v7-contact-shadow-parity.html`, { waitUntil: "domcontentloaded" });
    try {
    ...
    const reportDir = "tests/reports/v7/contact-shadow-parity";
