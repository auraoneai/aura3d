...
test.describe("V6 glTF render pipeline", () => {
  ...
  test("renders multiple imported corpus assets with PBR/HDR renderer metadata", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/tests/browser/production-runtime-gltf-render-pipeline.html`, { waitUntil: "domcontentloaded" });
    try {
    ...
    mkdirSync(resolve("tests/reports/production-runtime-gltf-render"), { recursive: true });
    await page.locator("#damaged-helmet").screenshot({ path: "tests/reports/production-runtime-gltf-render/damaged-helmet.png" });
