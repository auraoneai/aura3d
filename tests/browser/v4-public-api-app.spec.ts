...
const reportPath = "tests/reports/v4-public-api-app-browser.json";
...
test.describe("V4 public API browser app", () => {
  ...
  test("renders through createG3DApp and reports diagnostics", async ({ page }) => {
    const errors = captureErrors(page);
    await page.goto(`${server.origin}/tests/browser/fixtures/v4-public-api-app/index.html`, { waitUntil: "domcontentloaded" });
    await page.waitForFunction(() => window.__G3D_V4_PUBLIC_API_APP__?.status === "ready", undefined, { timeout: 60_000 });
    const screenshotPath = "tests/reports/v4-gallery/api/public-api-app.png";
    mkdirSync(join(process.cwd(), "tests/reports/v4-gallery/api"), { recursive: true });
    await page.locator("[data-testid='hr4-public-api-canvas']").screenshot({ path: screenshotPath });
    const state = await page.evaluate(() => window.__G3D_V4_PUBLIC_API_APP__);
    const report = {
      ok: errors.length === 0 &&
