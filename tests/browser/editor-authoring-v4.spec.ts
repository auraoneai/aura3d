...
const reportPath = "tests/reports/v4-editor-authoring.json";
...
test.describe("V4 editor authoring workflow", () => {
  ...
  test("authors, saves, reloads, plays, exports, and smoke-tests a V4 app", async ({ page }) => {
    ...
    writeFileSync(reportPath, JSON.stringify({
      schemaVersion: "g3d-v4-editor-authoring-report-v1",
      ok: true,
      generatedAt: new Date().toISOString(),
      command: "pnpm exec playwright test tests/browser/editor-authoring-v4.spec.ts",
  ...
  test("runs the checked-in V4 editor-authored app without editor code", async ({ page }) => {
    mkdirSync(screenshotDir, { recursive: true });
    await page.goto(`${server.origin}/examples/editor-authored-v4-app/index.html`, { waitUntil: "domcontentloaded" });
    await page.waitForFunction(() => (window as any).__GALILEO3D_EXPORTED_PROJECT__?.status === "ready");
    await page.waitForFunction(() => ((window as any).__GALILEO3D_EXPORTED_PROJECT__?.diagnostics.scriptTickCount ?? 0) > 2);
