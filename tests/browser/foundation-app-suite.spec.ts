...
test.describe("Foundation real app suite", () => {
  ...
  test.afterAll(async () => {
    ...
    writeFileSync(join(reportDir, "manifest.json"), `${JSON.stringify({
      schema: "a3d-foundation-app-suite-browser",
  ...
  for (const app of apps) {
    test(`${app.id} renders a real workflow-backed application`, async ({ page }) => {
      ...
      await page.evaluate(async (scenarioId) => {
        await (window as any).__A3D_FOUNDATION_APP__?.loadScenario?.(scenarioId);
...
async function readState(page: import("@playwright/test").Page): Promise<AppState | undefined> {
  return page.evaluate(() => {
    const state = (window as any).__A3D_FOUNDATION_APP__?.captureState?.() ?? (window as any).__A3D_FOUNDATION_APP__;
