...
const screenshotDir = resolve(process.cwd(), "tests/reports/aura3d11/animation-studio-visual-regression");

test.describe("Aura3D Animation Studio visual regression frames", () => {
  ...
  test("captures first, dialogue, action, and final browser frames without route chrome", async ({ page }) => {
    ...
    for (const frame of frames) {
      await page.evaluate((time) => {
        ...
        global.__AURA3D_ANIMATION_EPISODE_SEEK__?.(time);
      }, frame.time);
    ...
    const proof = await page.evaluate(() => {
      ...
      const proof = global.__AURA3D_ANIMATION_EPISODE_PROOF__;
      return {
        ...
        studioPanelVisible: Boolean(document.querySelector("#animation-studio-panel"))
      };
    });
