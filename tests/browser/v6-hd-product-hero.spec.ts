...
test.describe("V6 HD product hero renderer", () => {
  ...
  test("renders a close 2560x1440 imported PBR asset hero under real HDR", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/tests/browser/v6-hd-product-hero.html`, { waitUntil: "domcontentloaded" });
    try {
    ...
    const screenshotPath = "tests/reports/v6-hd-product-hero/damaged-helmet-hero.png";
    mkdirSync(dirname(resolve(screenshotPath)), { recursive: true });
    await page.locator("#v6-hd-product-hero").screenshot({ path: screenshotPath });
    ...
    const reportPath = "tests/reports/v6-hd-product-hero.json";
    writeFileSync(resolve(reportPath), `${JSON.stringify({
      schema: "g3d-v6-hd-product-hero/v1",
