...
const REPORT_PATH = "tests/reports/v9/morphtargets-parity.json";
const ARTIFACTS = {
  g3d: "tests/reports/v9/morphtargets-parity/g3d-morphtargets.png",
  threejs: "tests/reports/v9/morphtargets-parity/threejs-morphtargets.png",
  sideBySide: "tests/reports/v9/morphtargets-parity/side-by-side.png"
} as const;

test.describe("V9 morph targets same-asset Three.js parity", () => {
  ...
  test("applies Robot Expressive head morph weights through G3D and actual Three.js morphTargetInfluences", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/tools/v9-morphtargets-parity/index.html`, { waitUntil: "domcontentloaded" });
    await page.waitForFunction(
      () => {
        const result = window.__V9_MORPHTARGETS_PARITY__ as { readonly status?: string } | undefined;
    ...
    const result = await page.evaluate(() => window.__V9_MORPHTARGETS_PARITY__) as MorphTargetsParityResult;
    ...
    expect(result.schema).toBe("g3d-v9-morphtargets-parity/v1");
...
function assertNoThreeJsInG3DMorphRuntimeSource(): void {
  ...
  for (const sourcePath of [
    "apps/v8-skinning-morph/src/main.ts",
...
type MorphTargetsParityResult =
  | {
      ...
      readonly schema: "g3d-v9-morphtargets-parity/v1";
      readonly g3d: {
  ...
  | { readonly status: "error"; readonly schema: "g3d-v9-morphtargets-parity/v1"; readonly error: string };
