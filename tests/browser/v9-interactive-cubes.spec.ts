...
test.describe("V9 interactive cubes parity evidence", () => {
  ...
  test("route reports live pointer-hover cube hits through public picking rays", async ({ page }) => {
    ...
    const response = await page.goto(`${server.origin}/apps/v8-interactive-picking/`, { waitUntil: "domcontentloaded" });
    expect(response?.status()).toBe(200);
    await page.waitForFunction(() => {
      const runtime = window.__g3dV8InteractivePicking as { readonly status?: string; readonly cubePickHits?: number } | undefined;
    ...
    for (const xRatio of [0.28, 0.38, 0.5, 0.62, 0.74]) {
      for (const yRatio of [0.34, 0.46, 0.58, 0.7]) {
        ...
        const hit = await page.evaluate(() => {
          const runtime = window.__g3dV8InteractivePicking as {
    ...
    const runtime = await page.evaluate(() => window.__g3dV8InteractivePicking);
