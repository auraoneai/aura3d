...
const REPORT_PATH = "tests/reports/v9/animation-walk-parity.json";
const ARTIFACTS = {
  g3d: "tests/reports/v9/animation-walk-parity/g3d-animation-walk.png",
  threejs: "tests/reports/v9/animation-walk-parity/threejs-animation-walk.png",
  sideBySide: "tests/reports/v9/animation-walk-parity/side-by-side.png"
} as const;

test.describe("V9 animation walk same-asset Three.js parity", () => {
  ...
  test("samples Soldier Walk through G3D and actual Three.js AnimationMixer", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/tools/v9-animation-walk-parity/index.html`, { waitUntil: "domcontentloaded" });
    await page.waitForFunction(
      () => {
        const result = window.__V9_ANIMATION_WALK_PARITY__ as { readonly status?: string } | undefined;
    ...
    const result = await page.evaluate(() => window.__V9_ANIMATION_WALK_PARITY__) as AnimationWalkParityResult;
    ...
    expect(result.schema).toBe("g3d-v9-animation-walk-parity/v1");
...
function assertNoThreeJsInG3DWalkRuntimeSource(): void {
  ...
  for (const sourcePath of [
    "apps/v8-animation-walk/src/main.ts",
...
function stripDataUrls(result: Extract<AnimationWalkParityResult, { readonly status: "ready" }>): Omit<typeof result, "dataUrls"> {
...
type AnimationWalkParityResult =
  | {
      ...
      readonly schema: "g3d-v9-animation-walk-parity/v1";
      readonly g3d: {
      ...
      readonly threejs: {
        readonly renderer: { readonly drawCalls: number };
        readonly animation: { readonly clipName: string; readonly skinnedMeshCount: number };
        readonly pixels: { readonly uniqueColorBuckets: number };
      };
  ...
  | { readonly status: "error"; readonly schema: "g3d-v9-animation-walk-parity/v1"; readonly error: string };
