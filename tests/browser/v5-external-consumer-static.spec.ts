...
test("V5 external consumer static preview renders built output", async ({ page }) => {
  await page.goto(`file://${process.cwd()}/tests/reports/v5-external-consumer/static-preview/index.html`);
  await expect.poll(async () => page.evaluate(() => window.__g3dStaticPreview)).toBe(true);
  await page.screenshot({ path: "tests/reports/v5-external-consumer/static-preview.png" });
