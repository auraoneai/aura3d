...
test.describe("V8 flagship viewer", () => {
  ...
  test("renders first frame, drives controls, changes environment/exposure, and captures a screenshot report", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/apps/v8-flagship-viewer/`, { waitUntil: "domcontentloaded" });
    ...
    const reportDir = "tests/reports/v8-flagship-viewer";
    ...
    const report = {
      schema: "g3d-v8-flagship-viewer/v1",
      generatedAt: new Date().toISOString(),
      url: `${server.origin}/apps/v8-flagship-viewer/`,
    ...
    writeFileSync(resolve("tests/reports/v8-flagship-viewer.json"), `${JSON.stringify(report, null, 2)}\n`);
