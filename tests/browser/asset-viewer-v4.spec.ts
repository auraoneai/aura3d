...
const screenshotRoot = resolve("tests/reports/v4-example-screenshots/asset-viewer-v4");
const reportPath = resolve("tests/reports/v4-asset-viewer-browser.json");

const report: AssetViewerV4Report = {
  ok: false,
  generatedAt: new Date().toISOString(),
  command: "pnpm exec playwright test tests/browser/asset-viewer-v4.spec.ts",
  ...
  test.afterAll(async () => {
    ...
    report.completedTaskEvidence = [{
      ...
      evidence: [
        ...
        "tests/reports/v4-example-screenshots/asset-viewer-v4/*.png",
      ],
    }, {
...
async function validateAsset(page: Page, origin: string, asset: V4AssetManifestEntry): Promise<AssetViewerV4Validation> {
  ...
  const screenshot = `tests/reports/v4-example-screenshots/asset-viewer-v4/${asset.id}.png`;
  await page.getByTestId("asset-viewer-canvas").screenshot({ path: resolve(screenshot) });
  if (asset.id === "v4-material-fidelity-card") {
    await page.screenshot({ path: resolve("tests/reports/v4-example-screenshots/asset-viewer.png"), fullPage: true });
  }
...
function assertCoreRenderEvidence(
  ...
  expect(result?.screenshotPath).toBe("tests/reports/v4-example-screenshots/asset-viewer.png");
  ...
  expect(result?.v4RenderPreset?.presetId).toBe("galileo3d-v4-visual-quality-preset");
