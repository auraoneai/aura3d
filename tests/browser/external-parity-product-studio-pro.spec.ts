...
test("Product Studio Pro has real app screenshot evidence and runtime state", async () => {
  const source = JSON.parse(readFileSync("tests/reports/external-parity-product-configurator-browser.json", "utf8")) as {
    ok?: boolean;
  ...
  writeFileSync(join(process.cwd(), "tests/reports/external-parity-product-studio-pro-browser.json"), `${JSON.stringify({ ok, sourceReport: "tests/reports/external-parity-product-configurator-browser.json" }, null, 2)}\n`);
  expect(ok).toBe(true);
});
