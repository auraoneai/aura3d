...
test.describe("v3 renderer examples", () => {
  ...
  const reportRunId = `v3-rendering-${Date.now()}`;
  const report: V3RenderingReport = {
    ok: false,
    generatedAt: new Date().toISOString(),
    command: "pnpm exec playwright test tests/browser/rendering-v3-labs.spec.ts --grep \"shadow lab\"",
    run: {
      id: reportRunId,
      agent: "renderer-debug-timing",
      startedAt: new Date().toISOString(),
      command: "pnpm exec playwright test tests/browser/rendering-v3-labs.spec.ts"
    },
    evidence: {},
    validations: [],
  ...
  test.afterAll(async () => {
    ...
    report.completedTaskEvidence = [
      {
        ...
        evidence: ["examples/material-showroom/main.ts", "tests/browser/rendering-v3-labs.spec.ts"]
      },
      {
        task: "examples/renderer-stress-lab allows object/material/light count changes.",
        evidence: ["examples/renderer-stress-lab/main.ts", "tests/browser/rendering-v3-labs.spec.ts"]
      },
      {
        task: "Reports include draw calls, visible objects, culled objects, frame time, and memory estimates.",
        evidence: ["tests/reports/v3-rendering.json"]
      },
      {
        task: "Known-limits output lists unsupported PBR features for each loaded material.",
        evidence: ["examples/material-showroom/main.ts", "tests/browser/rendering-v3-labs.spec.ts", "tests/reports/v3-rendering.json"]
      },
      {
        task: "Shadow lab visual test verifies cascade metadata and projected shadow pixels.",
        evidence: ["examples/shadow-lab/main.ts", "tests/browser/rendering-v3-labs.spec.ts", "tests/reports/v3-rendering.json"]
      },
      {
        task: "Stable shadow camera/projection fitting.",
        evidence: ["packages/rendering/src/CascadedShadowMaps.ts", "examples/shadow-lab/main.ts", "tests/browser/rendering-v3-labs.spec.ts", "tests/reports/v3-rendering.json"]
      },
      {
      ...
      },
      {
        ...
        evidence: ["packages/rendering/src/ShadowMap.ts", "packages/rendering/src/CascadedShadowMaps.ts", "examples/shadow-lab/main.ts", "tests/unit/rendering/shadow-pass.test.ts", "tests/browser/rendering-v3-labs.spec.ts", "tests/reports/v3-rendering.json"]
      },
      {
        task: "Postprocess lab before/after visual test verifies tone mapping, bloom, FXAA, and presentation pixels.",
        evidence: ["examples/postprocess-lab/main.ts", "tests/browser/rendering-v3-labs.spec.ts", "tests/reports/v3-rendering.json"]
      },
      {
        task: "Depth texture plumbing is backed by a RenderGraph depth resource and browser-read depth visualization metrics.",
        evidence: ["packages/rendering/src/PostProcessPass.ts", "examples/postprocess-lab/main.ts", "tests/browser/rendering-v3-labs.spec.ts", "tests/reports/v3-rendering.json"]
      },
      {
      ...
      },
      {
        ...
        evidence: ["packages/rendering/src/PostProcessPass.ts", "examples/material-showroom/main.ts", "tests/browser/rendering-v3-labs.spec.ts", "tests/reports/v3-rendering.json"]
      },
      {
        ...
        evidence: ["packages/rendering/src/EnvironmentMapResources.ts", "tests/unit/rendering/environment-map-resources.test.ts", "examples/material-showroom/main.ts", "tests/browser/rendering-v3-labs.spec.ts", "tests/reports/v3-rendering.json"]
      },
      {
        task: "Visible sampled environment reflection affects bounded metallic/rough material-showroom objects.",
        evidence: ["packages/rendering/src/ShaderLibrary.ts", "packages/rendering/src/ForwardPass.ts", "examples/material-showroom/main.ts", "tests/browser/rendering-v3-labs.spec.ts", "tests/reports/v3-rendering.json"]
      },
      {
        task: "Implement render-pass and shader error overlays for developer debugging",
        evidence: ["packages/rendering/src/RendererDebugOverlay.ts", "examples/postprocess-lab/main.ts", "tests/unit/rendering/renderer-debug-overlay.test.ts", "tests/browser/rendering-v3-labs.spec.ts", "tests/reports/v3-rendering.json"]
      },
      {
        task: "Implement GPU timing where available with fallback CPU timing",
        evidence: ["packages/rendering/src/RendererTiming.ts", "examples/postprocess-lab/main.ts", "examples/renderer-stress-lab/main.ts", "tests/unit/rendering/renderer-timing.test.ts", "tests/browser/rendering-v3-labs.spec.ts", "tests/reports/v3-rendering.json"]
      },
      {
        task: "V3 visual tests verify expected color/material/geometry regions instead of accepting nonblank pixels.",
        evidence: ["tests/browser/rendering-v3-labs.spec.ts", "tests/reports/v3-rendering.json"]
      },
      {
        task: "WebGPU examples fail gracefully when unavailable.",
        evidence: ["examples/webgpu-capability/main.ts", "tests/browser/rendering-v3-labs.spec.ts"]
      }
    ];
    report.blockedTasks = [
    ...
    const reportPath = resolve("tests/reports/v3-rendering.json");
  ...
  test("shadow lab validates cascade metadata and projected shadow regions", async ({ page }, testInfo) => {
    ...
    const shadowScreenshotPath = resolve("tests/reports/v3-shadow-lab-debug.png");
