...
test("V5 BVH raycast acceleration reports large-geometry speedup", async ({ page }) => {
  ...
  await page.setContent(`<html><body><script>window.__g3dRaycast=${JSON.stringify(raycast)}</script></body></html>`);
  await expect.poll(async () => page.evaluate(() => window.__g3dRaycast.speedup)).toBeGreaterThan(100);
});
