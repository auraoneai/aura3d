...
const catalog = JSON.parse(readFileSync(resolve("examples/v6/catalog.json"), "utf8")) as {
...
test.describe("V6 real renderer examples", () => {
  ...
  for (const example of catalog.examples.filter((item) => item.browserTested)) {
    test(`renders ${example.slug} through the V6 public workflow API`, async ({ page }) => {
      ...
      await page.goto(`${server.origin}/examples/v6/${example.slug}/`, { waitUntil: "domcontentloaded" });
      ...
      await page.locator("#g3d-v6-action").click();
      ...
      mkdirSync(resolve("tests/reports/v6-examples"), { recursive: true });
      const screenshot = `tests/reports/v6-examples/${example.slug}.png`;
      await page.locator("#viewport").screenshot({ path: screenshot });
      writeFileSync(resolve(`tests/reports/v6-examples/${example.slug}.json`), `${JSON.stringify({
        schema: "g3d-v6-example-runtime/v1",
  ...
  test("lists the V6 example catalog", async ({ page }) => {
    await page.goto(`${server.origin}/examples/v6/`, { waitUntil: "domcontentloaded" });
