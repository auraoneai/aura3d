...
test.describe("scripting browser runtime", () => {
  ...
  test("runs a simple behavior demo that moves a scene node", async ({ page }) => {
    ...
    await page.waitForFunction(
      () => window.__GALILEO3D_SCRIPTING_BROWSER_TEST__?.status === "ready" || window.__GALILEO3D_SCRIPTING_BROWSER_TEST__?.status === "error",
      undefined,
      { timeout: 10_000 }
    );

    const result = await page.evaluate(() => window.__GALILEO3D_SCRIPTING_BROWSER_TEST__);
...
declare global {
  interface Window {
    __GALILEO3D_SCRIPTING_BROWSER_TEST__?: {
