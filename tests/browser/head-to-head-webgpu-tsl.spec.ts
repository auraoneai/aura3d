...
test.describe("current head-to-head WebGPU / TSL", () => {
  ...
  test("renders the frozen product through native Aura WebGPU/WGSL and Three WebGPU/TSL", async ({ page }) => {
    test.setTimeout(300_000); const errors: string[] = []; const knownSwiftShaderScopeTeardownErrors: string[] = [];
    page.on("pageerror", (error) => {
      if (error.message === "Instance dropped in popErrorScope" && !error.stack) {
        knownSwiftShaderScopeTeardownErrors.push(error.message);
        return;
      }
    ...
    expect(await page.evaluate(() => window.__AURA_THREE_HEAD_TO_HEAD_WEBGPU_TSL_ERROR__) ?? errors.join(" | ")).toBeFalsy();
    expect(knownSwiftShaderScopeTeardownErrors.length).toBeLessThanOrEqual(4);
    const before = await page.evaluate(() => window.__AURA_THREE_HEAD_TO_HEAD_WEBGPU_TSL__);
    ...
    await page.locator("#advance").click(); await page.waitForFunction(() => Boolean(window.__AURA_THREE_HEAD_TO_HEAD_WEBGPU_TSL__?.after), undefined, { timeout: 120_000 }); await page.evaluate(() => new Promise<void>((resolve) => requestAnimationFrame(() => requestAnimationFrame(() => resolve())))); const after = await page.evaluate(() => window.__AURA_THREE_HEAD_TO_HEAD_WEBGPU_TSL__); const afterHashes = await capturePair(page, "after");
    expect(after.after.aura.materialStateHash).not.toBe(before.before.aura.materialStateHash); expect(afterHashes.aura).not.toBe(beforeHashes.aura); expect(afterHashes.three).not.toBe(beforeHashes.three); expect(after.after.aura.drawCalls).toBeGreaterThan(0); expect(after.after.three.drawCalls).toBeGreaterThan(0);
    ...
    writeFileSync(resolve(REPORT_DIRECTORY, "report.json"), `${JSON.stringify({ schema: "aura3d.current-head-to-head-workload/1.0", generatedAt: new Date().toISOString(), pass: true, assetSha256: assetHash, captureHashes: { before: beforeHashes, after: afterHashes }, before: before.before, after: after.after, lifecycle }, null, 2)}\n`);
  });
