...
async function run(): Promise<void> {
  ...
  const threeEnvironment = await loadThreeEnvironment(`${location.origin}/fixtures/v6/environments/hdri/studio_small_08_1k.hdr`);
...
async function renderG3D(scene: ParityScene, canvas: HTMLCanvasElement, hdrPipeline: ReturnType<typeof createV6PbrHdrPipelineFromRadiance>): Promise<ParityResult["g3d"]> {
  ...
  const pipeline = await loadV6GLTFRenderPipeline({
    url: `${location.origin}/fixtures/v6/assets/corpus/${scene.file}`,
...
async function renderThree(scene: ParityScene, canvas: HTMLCanvasElement, environment: THREE.Texture): Promise<ParityResult["threejs"]> {
  ...
  const gltf = await new Promise<{ scene: THREE.Object3D }>((resolve, reject) => {
    new GLTFLoader().load(`${location.origin}/fixtures/v6/assets/corpus/${scene.file}`, resolve, undefined, reject);
  });
