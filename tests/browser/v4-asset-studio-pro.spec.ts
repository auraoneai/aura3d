...
const reportPath = "tests/reports/v4-asset-studio-pro-browser.json";
const screenshotDir = "tests/reports/v4-gallery/assets";
...
test.describe("V4 Asset Studio Pro", () => {
  ...
  test("renders the corpus browser and asset diagnostics in the example and app", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/examples/asset-gallery-v4/index.html`, { waitUntil: "domcontentloaded" });
    const exampleState = await waitForAssetState(page, "asset-gallery-v4");
    await page.locator("[data-testid='hr4-asset-canvas']").screenshot({ path: `${screenshotDir}/asset-gallery-v4.png` });
    ...
    await page.locator("[data-testid='hr4-asset-canvas']").screenshot({ path: `${screenshotDir}/asset-gallery-v4-cesium-man.png` });
    ...
    const report = {
      ok: errors.length === 0 &&
        statePasses(exampleState, "asset-gallery-v4") &&
        statePasses(selectedState, "asset-gallery-v4") &&
        statePasses(appState, "asset-studio-pro") &&
        selectedState.selectedAsset?.id === "cesium-man",
      generatedAt: new Date().toISOString(),
      screenshots: [
        `${screenshotDir}/asset-gallery-v4.png`,
        `${screenshotDir}/asset-gallery-v4-cesium-man.png`,
    ...
    expect(statePasses(exampleState, "asset-gallery-v4")).toBe(true);
    expect(statePasses(selectedState, "asset-gallery-v4")).toBe(true);
