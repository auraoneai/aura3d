...
test.describe("Production HD PBR material renderer", () => {
  ...
  test("renders PBR material extension assets at 1920x1080 with HDR evidence", async ({ page }) => {
    ...
    try {
      await page.waitForFunction(
        () => {
          const result = window.__PRODUCTION_HD_MATERIALS__ as { status?: string } | undefined;
      ...
      throw new Error(`Production HD materials harness did not report ready/error. Page errors:\n${pageErrors.join("\n") || "(none captured)"}`, { cause: error });
    }

    const result = await page.evaluate(() => window.__PRODUCTION_HD_MATERIALS__) as {
      status: "ready" | "error";
      error?: string;
    ...
    expect(result.assetIds).toEqual(["damaged-helmet", "clear-coat-test", "sheen-test-grid", "specular-glossiness-card"]);
    ...
    expect(result.proof?.importedAsset.environmentId).toBe("kloppenheim-puresky");
    ...
    writeFileSync(resolve(reportPath), `${JSON.stringify({
      schema: "a3d-production-runtime-hd-materials",
