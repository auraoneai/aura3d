...
test("Material Studio alias report maps the V4 requested filename to Material Studio Pro proof", async () => {
  ...
  writeFileSync(join(process.cwd(), "tests/reports/external-parity-material-studio-browser.json"), `${JSON.stringify({ ok, sourceReport: "tests/reports/external-parity-material-studio-pro-browser.json" }, null, 2)}\n`);
  expect(ok).toBe(true);
});
