...
const REPORT_PATH = "tests/reports/v9/unreal-bloom-parity.json";
const ARTIFACTS = {
  g3d: "tests/reports/v9/unreal-bloom-parity/g3d-unreal-bloom.png",
  threejs: "tests/reports/v9/unreal-bloom-parity/threejs-unreal-bloom.png",
  sideBySide: "tests/reports/v9/unreal-bloom-parity/side-by-side.png"
} as const;
...
test.describe("V9 UnrealBloomPass same-scene parity", () => {
  ...
  test("captures G3D bloom chain against actual Three.js UnrealBloomPass", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/tools/v9-unreal-bloom-parity/index.html`, { waitUntil: "domcontentloaded" });
    ...
    expect(result.schema).toBe("g3d-v9-unreal-bloom-parity/v1");
    ...
    for (const [kind, path] of Object.entries(ARTIFACTS)) {
      ...
      expect(stats.uniqueColorBuckets, `${kind} unique color buckets`).toBeGreaterThan(16);
      expect(statSync(resolve(path)).size, `${kind} PNG size`).toBeGreaterThan(4 * 1024);
    }
...
function assertReferenceUnrealBloomPassUsed(): void {
  const composerSource = readFileSync(resolve("node_modules/three/examples/jsm/postprocessing/EffectComposer.js"), "utf8");
  const passSource = readFileSync(resolve("node_modules/three/examples/jsm/postprocessing/UnrealBloomPass.js"), "utf8");
  expect(composerSource).toContain("class EffectComposer");
  expect(composerSource).toContain("renderTarget1");
  expect(passSource).toContain("class UnrealBloomPass");
  expect(passSource).toContain("luminosityThreshold");
...
function assertNoThreeJsInG3DBloomRuntimeSource(): void {
  ...
  for (const sourcePath of [
    "apps/v8-postprocessing-bloom/src/main.ts",
...
function stripDataUrls(result: Extract<UnrealBloomParityResult, { readonly status: "ready" }>): Omit<typeof result, "dataUrls"> {
...
type UnrealBloomParityResult =
  | {
      ...
      readonly schema: "g3d-v9-unreal-bloom-parity/v1";
