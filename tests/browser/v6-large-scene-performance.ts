...
async function run(): Promise<void> {
  ...
  try {
    ...
    const material = new InstancedUnlitMaterial({ name: "v6-browser-instanced", color: [0.95, 0.52, 0.15, 1] });
    const textured = new TexturedUnlitMaterial({ name: "v6-browser-textured", texture });
    const renderItems: RenderItem[] = [
      { geometry: Geometry.texturedCube(0.12), material: textured, label: "v6-browser-textured-probe" }
    ];
    for (let start = 0; start < renderedInstances; start += MAX_GPU_INSTANCES) {
      ...
      renderItems.push({
        ...
        label: `v6-browser-instance-${start}`
      });
    }
...
function createTexture(): Texture {
  ...
  return new Texture({ width: 16, height: 16, data, colorSpace: "srgb", label: "v6-browser-performance-texture" });
}
