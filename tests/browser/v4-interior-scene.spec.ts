...
const reportPath = "tests/reports/v4-interior-scene-browser.json";
const screenshotDir = "tests/reports/v4-gallery/scenes";
...
test.describe("V4 interior scene", () => {
  ...
  test("renders a multi-object interior scene in the example and Scene Studio Pro app", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/examples/interior-scene-v4/index.html`, { waitUntil: "domcontentloaded" });
    const exampleState = await waitForInteriorState(page, "interior-scene-v4");
    await page.locator("[data-testid='hr4-scene-canvas']").screenshot({ path: `${screenshotDir}/interior-scene-v4.png` });
    ...
    await page.locator("[data-testid='hr4-scene-canvas']").screenshot({ path: `${screenshotDir}/interior-scene-v4-night.png` });
    ...
    const report = {
      ok: errors.length === 0 &&
        statePasses(exampleState, "interior-scene-v4") &&
        statePasses(nightState, "interior-scene-v4") &&
        statePasses(appState, "scene-studio-pro") &&
        nightState.lightingPreset === "night",
      generatedAt: new Date().toISOString(),
      screenshots: [
        `${screenshotDir}/interior-scene-v4.png`,
        `${screenshotDir}/interior-scene-v4-night.png`,
    ...
    expect(statePasses(exampleState, "interior-scene-v4")).toBe(true);
    expect(statePasses(nightState, "interior-scene-v4")).toBe(true);
