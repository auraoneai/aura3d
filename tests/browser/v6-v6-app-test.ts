...
export function runV6V6AppTest(config: V6V6AppTestConfig): void {
  test.describe(`V6 ${config.appId}`, () => {
    ...
    test("renders a real imported asset and exposes production runtime metrics", async ({ page }) => {
      ...
      mkdirSync(resolve("tests/reports/v6-app-suite"), { recursive: true });
      const screenshotPath = `tests/reports/v6-app-suite/${config.appId}.png`;
      await page.locator("#viewport").screenshot({ path: screenshotPath });
      writeFileSync(resolve(`tests/reports/v6-app-suite/${config.appId}.json`), `${JSON.stringify({
        schema: "g3d-v6-app-runtime/v1",
