...
const reportDir = resolve("tests/reports/v3-threejs-comparison");
...
test.describe("V3 same-scene Three.js comparison", () => {
  ...
  test.afterAll(async () => {
    ...
    writeFileSync(join(reportDir, "manifest.json"), `${JSON.stringify({
      schema: "g3d-v3-threejs-comparison-browser/v1",
  ...
  for (const scene of scenes) {
    test(`${scene.id} scene captures G3D, Three.js, and diff images`, async ({ page }) => {
      ...
      const metrics = await page.evaluate(async ({ origin, scene }) => {
        const g3dModule = await import(`${origin}/benchmarks/v3/galileo/${scene.g3dModule}.ts`);
        const threeModule = await import(`${origin}/benchmarks/v3/threejs/${scene.threeModule}.ts`);
