...
test.describe("V7 material extension artifacts", () => {
  ...
  test("renders dedicated material extension assets across the V7 suite", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/tests/browser/v7-material-extensions.html`, { waitUntil: "domcontentloaded" });
    try {
    ...
    const screenshotPath = "tests/reports/v7/material-extensions/material-extensions.png";
    ...
    for (const asset of orderedAssets) {
      const artifactPath = `tests/reports/v7/material-extensions/${asset.id}.png`;
      await page.goto(`${server.origin}/tests/browser/v7-material-extensions.html?asset=${asset.id}`, { waitUntil: "domcontentloaded" });
      await page.waitForFunction(
        () => {
          const result = window.__V7_MATERIAL_EXTENSIONS__ as { status?: string } | undefined;
          return result?.status === "ready" || result?.status === "error";
    ...
    const reportPath = "tests/reports/v7/material-extensions/material-extensions.json";
