...
test.describe("V6 HD PBR material renderer", () => {
  ...
  test("renders PBR material extension assets at 1920x1080 with HDR evidence", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/tests/browser/v6-hd-materials.html`, { waitUntil: "domcontentloaded" });
    try {
    ...
    const screenshotPath = "tests/reports/v6-hd-materials/pbr-materials-hd.png";
    mkdirSync(dirname(resolve(screenshotPath)), { recursive: true });
    await page.locator("#v6-hd-materials").screenshot({ path: screenshotPath });
    ...
    const reportPath = "tests/reports/v6-hd-materials.json";
    writeFileSync(resolve(reportPath), `${JSON.stringify({
      schema: "g3d-v6-hd-materials/v1",
