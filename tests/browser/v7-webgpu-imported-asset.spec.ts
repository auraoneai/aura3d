  ...
  test("renders or honestly blocks a real imported GLB through the low-level WebGPU renderer", async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      const [
        ...
        import("/packages/rendering/src/v6/index.js")
      ]);
      ...
      try {
        const assetUrl = `${location.origin}/fixtures/v6/assets/corpus/damaged-helmet.glb`;
        const hdrUrl = `${location.origin}/fixtures/v6/environments/hdri/studio_small_08_1k.hdr`;
    ...
    const reportDir = "tests/reports/v7/webgpu-imported-asset";
