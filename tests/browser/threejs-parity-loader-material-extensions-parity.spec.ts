...
test.describe("loader material extensions same-scene Three.js parity", () => {
  ...
  test("loads KHR_materials_sheen and KHR_materials_transmission through A3D and actual Three.js GLTFLoader", async ({ page }) => {
    ...
    await page.waitForFunction(
      () => {
        const result = window.__THREEJS_PARITY_LOADER_MATERIAL_EXTENSIONS_PARITY__ as { readonly status?: string } | undefined;
    ...
    const result = await page.evaluate(() => window.__THREEJS_PARITY_LOADER_MATERIAL_EXTENSIONS_PARITY__) as LoaderMaterialExtensionsParityResult;
    ...
    expect(result.schema).toBe("a3d-threejs-parity-loader-material-extensions-parity");
...
type LoaderMaterialExtensionsParityResult =
  ...
  | { readonly status: "error"; readonly schema: "a3d-threejs-parity-loader-material-extensions-parity"; readonly error: string };
