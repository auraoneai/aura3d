...
test.describe("core EngineLoop RAF mode", () => {
  ...
  test("starts on requestAnimationFrame and stops without scheduling extra frames", async ({ page }) => {
    ...
    await page.waitForFunction(
      () => window.__AURA3D_CORE_RAF_TEST__?.status === "ready" || window.__AURA3D_CORE_RAF_TEST__?.status === "error",
      undefined,
      { timeout: 10_000 }
    );

    const result = await page.evaluate(() => window.__AURA3D_CORE_RAF_TEST__);
