...
const reportPath = "tests/reports/v4-character-viewer-browser.json";
const screenshotDir = "tests/reports/v4-gallery/characters";
...
test.describe("V4 character viewer", () => {
  ...
  test("renders character timeline controls in the example and Animation Studio Pro app", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/examples/character-viewer-v4/index.html`, { waitUntil: "domcontentloaded" });
    const exampleState = await waitForCharacterState(page, "character-viewer-v4");
    await page.locator("[data-testid='hr4-character-canvas']").screenshot({ path: `${screenshotDir}/character-viewer-v4.png` });
    ...
    await page.locator("[data-testid='hr4-character-canvas']").screenshot({ path: `${screenshotDir}/character-viewer-v4-scrubbed.png` });
    ...
    const report = {
      ok: errors.length === 0 && statePasses(exampleState, "character-viewer-v4") && statePasses(scrubState, "character-viewer-v4") && statePasses(appState, "animation-studio-pro") && scrubState.playing === false,
      generatedAt: new Date().toISOString(),
      screenshots: [`${screenshotDir}/character-viewer-v4.png`, `${screenshotDir}/character-viewer-v4-scrubbed.png`, `${screenshotDir}/animation-studio-pro.png`],
    ...
    expect(statePasses(exampleState, "character-viewer-v4")).toBe(true);
    expect(statePasses(scrubState, "character-viewer-v4")).toBe(true);
