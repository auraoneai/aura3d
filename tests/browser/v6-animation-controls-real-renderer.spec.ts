...
test.describe("V6 animation and controls real renderer", () => {
  ...
  test("renders imported skinned and morph animated assets with orbit control proof", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/tests/browser/v6-animation-controls-real-renderer.html`, { waitUntil: "domcontentloaded" });
    try {
    ...
    mkdirSync(resolve("tests/reports/v6-animation-controls"), { recursive: true });
    await page.locator("#cesium-man").screenshot({ path: "tests/reports/v6-animation-controls/cesium-man-animation.png" });
    await page.locator("#animated-morph-cube").screenshot({ path: "tests/reports/v6-animation-controls/animated-morph-cube.png" });
    writeFileSync(resolve("tests/reports/v6-animation-controls-real-renderer.json"), `${JSON.stringify({
      schema: "g3d-v6-animation-controls-real-renderer/v1",
