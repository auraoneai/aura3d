...
const reportPath = "tests/reports/v4-template-product-viewer-browser.json";
const templates = [
  { id: "v4-product-viewer", globalName: "__G3D_TEMPLATE_PRODUCT_VIEWER__", workflowKind: "scene-showcase" },
  { id: "v4-material-studio", globalName: "__G3D_TEMPLATE_MATERIAL_STUDIO__", workflowKind: "material-studio" },
  { id: "v4-asset-gallery", globalName: "__G3D_TEMPLATE_ASSET_GALLERY__", workflowKind: "asset-viewer" },
  { id: "v4-interactive-scene", globalName: "__G3D_TEMPLATE_INTERACTIVE_SCENE__", workflowKind: "interactive-scene" }
] as const;
...
test.describe("V4 V4 product viewer template", () => {
  ...
  test("renders every V4 template through the public root package API", async ({ page }) => {
    ...
    mkdirSync(join(process.cwd(), "tests/reports/v4-gallery/templates"), { recursive: true });
    const captures = [];
    for (const template of templates) {
      ...
      const screenshotPath = `tests/reports/v4-gallery/templates/${template.id}.png`;
