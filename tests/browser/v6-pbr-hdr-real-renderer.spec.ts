...
test.describe("V6 PBR/HDR real renderer", () => {
  ...
  test("renders the same imported GLB under two real HDR environments with visibly different pixels", async ({ page }) => {
    ...
    await page.goto(`${server.origin}/tests/browser/v6-pbr-hdr-real-renderer.html`, { waitUntil: "domcontentloaded" });
    try {
    ...
    mkdirSync(resolve("tests/reports/v6-pbr-hdr"), { recursive: true });
    await page.locator("#studio").screenshot({ path: "tests/reports/v6-pbr-hdr/damaged-helmet-studio-hdr.png" });
    await page.locator("#sunset").screenshot({ path: "tests/reports/v6-pbr-hdr/damaged-helmet-sunset-hdr.png" });
    const reportPath = "tests/reports/v6-pbr-hdr-real-renderer.json";
    writeFileSync(resolve(reportPath), `${JSON.stringify({
      schema: "g3d-v6-pbr-hdr-real-renderer/v1",
