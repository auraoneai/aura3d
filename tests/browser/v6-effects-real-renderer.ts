...
async function run(): Promise<void> {
  ...
  const pipeline = await loadV6GLTFRenderPipeline({
    url: `${location.origin}/fixtures/v6/assets/corpus/damaged-helmet.glb`,
  ...
  const key = pipeline.resources.scene.createLight("directional", "v6-shadow-key");
