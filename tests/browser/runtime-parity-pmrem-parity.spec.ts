...
test.describe("V7 PMREM parity artifact", () => {
  ...
  test("captures a same-scene A3D vs Three.js PMREM reflection delta", async ({ page }) => {
    ...
    const result = await page.evaluate(() => window.__V7_PMREM_PARITY__) as {
      ...
      skybox?: {
        parity?: { claim?: string };
        a3d?: { diagnostics: { drawCalls: number; lastError: string | null }; pixelStats: { nonBlackPixels: number; uniqueColorBuckets: number; averageLuma: number; maxLuma: number } };
      ...
      texturedTransmissionParity?: {
        claim?: string;
        a3d?: { diagnostics: { drawCalls: number; lastError: string | null }; pixelStats: { nonBlackPixels: number; uniqueColorBuckets: number; averageLuma: number; maxLuma: number }; materialPath: string };
      ...
      dataUrls?: {
        ...
        skyboxDiff: string;
        a3dTransmission: string;
        threejsTransmission: string;
        transmissionDiff: string;
        a3dTexturedParallax: string;
        a3dTexturedFlat: string;
    ...
    expect(result.a3d?.cubemapMipCount).toBeGreaterThanOrEqual(8);
    expect(result.a3d?.cubemapAtlas.faceSize).toBeGreaterThanOrEqual(128);
    expect(result.a3d?.cubemapAtlas.displayedMipLevels).toHaveLength(3);
    expect(result.a3d?.cubemapAtlas.displayedFaceCount).toBe(18);
    expect(result.a3d?.cubemapAtlas.model).toBe("equirectangular-to-cubemap-ggx-importance-sampled-prefilter");
    expect(result.a3d?.cubemapAtlas.luminanceVarianceByDisplayedMip[0] ?? 0).toBeGreaterThan(result.a3d?.cubemapAtlas.luminanceVarianceByDisplayedMip[2] ?? Number.POSITIVE_INFINITY);
    expect(result.a3d?.cubemapAtlas.edgeMeanDeltaByDisplayedMip.every((value) => Number.isFinite(value) && value >= 0)).toBe(true);
    expect(result.a3d?.diagnostics.drawCalls).toBeGreaterThanOrEqual(4);
    expect(result.a3d?.diagnostics.lastError).toBeNull();
    expect(result.a3d?.pixelStats.nonBlackPixels ?? 0).toBeGreaterThan(80_000);
    expect(result.a3d?.pixelStats.uniqueColorBuckets ?? 0).toBeGreaterThanOrEqual(120);
    expect(result.a3d?.pixelStats.maxLuma ?? 0).toBeGreaterThan(100);
    ...
    expect(Math.abs((result.a3d?.pixelStats.averageLuma ?? 0) - (result.threejs?.pixelStats.averageLuma ?? Number.POSITIVE_INFINITY))).toBeLessThan(10);
    ...
    expect(result.skybox?.a3d?.diagnostics.drawCalls ?? 0).toBeGreaterThanOrEqual(1);
    expect(result.skybox?.a3d?.diagnostics.lastError).toBeNull();
    expect(result.skybox?.a3d?.pixelStats.nonBlackPixels ?? 0).toBeGreaterThan(250_000);
    expect(result.skybox?.a3d?.pixelStats.uniqueColorBuckets ?? 0).toBeGreaterThanOrEqual(20);
    expect(result.skybox?.a3d?.pixelStats.maxLuma ?? 0).toBeGreaterThan(70);
    ...
    expect(result.skybox?.a3d?.pixelStats.uniqueColorBuckets ?? 0).toBeGreaterThanOrEqual(result.skybox?.threejs?.pixelStats.uniqueColorBuckets ?? Number.POSITIVE_INFINITY);
    expect(Math.abs((result.skybox?.a3d?.pixelStats.averageLuma ?? 0) - (result.skybox?.threejs?.pixelStats.averageLuma ?? Number.POSITIVE_INFINITY))).toBeLessThan(5);
    ...
    expect(Math.abs((result.transmission?.a3d?.pixelStats.averageLuma ?? 0) - (result.transmission?.threejs?.pixelStats.averageLuma ?? Number.POSITIVE_INFINITY))).toBeLessThan(12);
    ...
    expect(result.texturedParallax?.claim).toBe("a3d-textured-pbr-parallax-transmission-browser-proof");
    ...
    expect(result.texturedTransmissionParity?.a3d?.materialPath).toBe("TexturedPBRMaterial");
    expect(result.texturedTransmissionParity?.threejs?.materialPath).toBe("MeshPhysicalMaterial");
    expect(result.texturedTransmissionParity?.a3d?.diagnostics.lastError).toBeNull();
    expect(result.texturedTransmissionParity?.a3d?.diagnostics.drawCalls ?? 0).toBeGreaterThanOrEqual(6);
    ...
    expect(result.texturedTransmissionParity?.a3d?.pixelStats.nonBlackPixels ?? 0).toBeGreaterThan(90_000);
    expect(result.texturedTransmissionParity?.threejs?.pixelStats.nonBlackPixels ?? 0).toBeGreaterThan(90_000);
    expect(result.texturedTransmissionParity?.a3d?.pixelStats.uniqueColorBuckets ?? 0).toBeGreaterThanOrEqual(90);
    expect(result.texturedTransmissionParity?.threejs?.pixelStats.uniqueColorBuckets ?? 0).toBeGreaterThanOrEqual(60);
    expect(Math.abs((result.texturedTransmissionParity?.a3d?.pixelStats.averageLuma ?? 0) - (result.texturedTransmissionParity?.threejs?.pixelStats.averageLuma ?? Number.POSITIVE_INFINITY))).toBeLessThan(14);
    ...
    expect(result.dataUrls?.a3dSkybox).toMatch(/^data:image\/png;base64,/);
    ...
    expect(result.dataUrls?.a3dTransmission).toMatch(/^data:image\/png;base64,/);
    ...
    expect(result.dataUrls?.a3dTexturedParallax).toMatch(/^data:image\/png;base64,/);
    expect(result.dataUrls?.a3dTexturedFlat).toMatch(/^data:image\/png;base64,/);
    ...
    const pngs = [
      ["a3d", `${reportDir}/a3d-pmrem-spheres.png`, result.dataUrls?.a3d],
      ["threejs", `${reportDir}/threejs-pmrem-spheres.png`, result.dataUrls?.threejs],
      ["diff", `${reportDir}/pmrem-diff.png`, result.dataUrls?.diff],
      ["cubemapAtlas", `${reportDir}/a3d-cubemap-pmrem-atlas.png`, result.dataUrls?.cubemapAtlas],
      ["a3dSkybox", `${reportDir}/a3d-hdr-skybox.png`, result.dataUrls?.a3dSkybox],
      ...
      ["a3dTransmission", `${reportDir}/a3d-transmission-pmrem.png`, result.dataUrls?.a3dTransmission],
      ...
      ["a3dTexturedParallax", `${reportDir}/a3d-textured-parallax-transmission.png`, result.dataUrls?.a3dTexturedParallax],
      ["a3dTexturedFlat", `${reportDir}/a3d-textured-parallax-disabled.png`, result.dataUrls?.a3dTexturedFlat],
