...
describe("V8 GLTF animation runtime", () => {
  ...
  it("diagnoses Kira animation, skinning, texture, and static mesh limitations", async () => {
    expect(readGlbJson("fixtures/v8/assets/showcase/kira-ik-room.glb").animations ?? []).toEqual([]);
    const kira = await new GLTFLoader().load({
      url: toGlbDataUri("fixtures/v8/assets/showcase/kira-ik-room-animated.glb"),
      type: "gltf"
    }, new LoadContext());
