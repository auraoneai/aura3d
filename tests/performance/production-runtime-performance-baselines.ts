...
async function main(): Promise<void> {
  ...
  const reportPath = resolve("tests/reports/production-runtime-performance-baselines.json");
...
async function createLargeSceneBaseline(): Promise<V6PerformanceBaseline> {
  ...
  const materials = [
    new UnlitMaterial({ name: "production-runtime-perf-red", color: [0.95, 0.18, 0.08, 1] }),
    new UnlitMaterial({ name: "production-runtime-perf-blue", color: [0.12, 0.45, 0.95, 1] }),
    new PBRMaterial({ name: "production-runtime-perf-pbr", baseColor: [0.84, 0.64, 0.25, 1], metallic: 0.25, roughness: 0.44 }),
    ...textures.map((texture, index) => new TexturedUnlitMaterial({ name: `production-runtime-perf-texture-${index}`, texture })),
    new InstancedUnlitMaterial({ name: "production-runtime-perf-instanced", color: [1, 0.54, 0.16, 1] })
  ];
  ...
  for (let index = 0; index < staticMeshes; index += 1) {
    ...
    renderItems.push({
      ...
      label: `production-runtime-perf-static-${index}`
    });
  }
  for (let start = 0; start < visibleInstances; start += MAX_GPU_INSTANCES) {
    ...
    renderItems.push({
      ...
      label: `production-runtime-perf-instance-batch-${start}`
    });
  }
...
function createTextures(count: number): Texture[] {
  return Array.from({ length: count }, (_, index) => {
    ...
    return new Texture({ width: 8, height: 8, data, colorSpace: "srgb", label: `production-runtime-performance-texture-${index}` });
  });
}
...
function createCurrentFixtureBudgetWarnings(): readonly string[] {
  const roots = ["fixtures/threejs-parity/assets", "fixtures/advanced-gallery/assets"].map((path) => resolve(path)).filter((path) => existsSync(path));
