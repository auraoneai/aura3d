...
const swatches = document.getElementById('paint-swatches') as HTMLDivElement;
const materials = document.getElementById('materials') as HTMLDivElement;
const status = document.getElementById('status') as HTMLElement;
const explodeToggle = document.getElementById('explode-toggle') as HTMLButtonElement;
const lightingToggle = document.getElementById('lighting-toggle') as HTMLButtonElement;
...
function drawRoundedRect(x: number, y: number, width: number, height: number, radius: number, fill: string): void {
  if (!ctx) return;
  ctx.beginPath();
...
function drawWheel(x: number, y: number, radius: number): void {
  if (!ctx) return;
  ctx.save();
  ctx.shadowBlur = 18;
...
function polygon(points: Array<[number, number]>, fill: CanvasGradient | string, stroke?: string): void {
  if (!ctx) return;
  ctx.beginPath();
...
function drawCar(width: number, height: number): void {
  if (!ctx) return;
  ...
  polygon([
    [left + carW * 0.28, baseY - carH * 0.50 + explodeOffset],
    [left + carW * 0.41, baseY - carH * 0.86 + explodeOffset],
    [left + carW * 0.62, baseY - carH * 0.88 + explodeOffset],
    [left + carW * 0.76, baseY - carH * 0.50 + explodeOffset],
  ...
  polygon([
    [left + carW * 0.37, baseY - carH * 0.53 + explodeOffset],
    [left + carW * 0.44, baseY - carH * 0.78 + explodeOffset],
    [left + carW * 0.58, baseY - carH * 0.79 + explodeOffset],
    [left + carW * 0.69, baseY - carH * 0.53 + explodeOffset],
  ], glassGradient, '#08121f');

  polygon([
    [left + carW * 0.53, baseY - carH * 0.79 + explodeOffset],
    [left + carW * 0.61, baseY - carH * 0.79 + explodeOffset],
    [left + carW * 0.69, baseY - carH * 0.53 + explodeOffset],
    [left + carW * 0.55, baseY - carH * 0.53 + explodeOffset],
  ], '#0c1722');
...
function drawStudio(width: number, height: number): void {
  if (!ctx) return;
...
function renderControls(): void {
  ...
  const roleCount = scene.materialRoles.filter((role: ProductMaterialRole) => role.currentSource === 'original-car-glb-fixture').length;
