...
const modeButtons = Array.from(document.querySelectorAll<HTMLButtonElement>('#mode-segment button'));
...
function drawEllipse(cx: number, cy: number, rx: number, ry: number, rotation: number, color: string, width: number): void {
  if (!ctx) return;
  ctx.save();
  ctx.translate(cx, cy);
  ctx.rotate(rotation);
...
function drawNode(x: number, y: number, radius: number, fill: string, glow = false): void {
  if (!ctx) return;
  ctx.save();
  if (glow) {
...
function drawStream(x0: number, y0: number, x1: number, y1: number, color: string, width: number): void {
  if (!ctx) return;
  const mx = (x0 + x1) / 2;
...
function drawGalaxy(width: number, height: number, time: number): void {
  if (!ctx) return;
  const cx = width * 0.5;
