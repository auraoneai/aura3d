import {
  ...
  type ExternalParityPhysicalMaterialDescriptor,
...
declare global {
  interface Window {
    __A3D_EXTERNAL_PARITY_MATERIAL_STUDIO__?: unknown;
  }
}
...
const claimBoundary = "Milestone 8 material studio proof only; ExternalParity release still requires licensed production textures and same-scene Three.js visual parity.";

export async function mountExternalMaterialStudio(id: string): Promise<void> {
    ...
    <main style="display:grid;grid-template-columns:340px 1fr;height:100vh;background:#111316;color:#f4f1e8;font-family:Inter,system-ui,sans-serif">
      <aside style="border-right:1px solid #30363d;padding:18px;overflow:auto">
        ...
        <div data-testid="hr4-material-list" style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:16px">
          ${EXTERNAL_PARITY_PHYSICAL_MATERIAL_MATRIX.map((material) => `<div style="border:1px solid #30363d;padding:7px;background:#191e24">${material.label}</div>`).join("")}
        </div>
  ...
  function render(): void {
    ...
    const analyses = analyzeExternalParityMaterialMatrix();
    const state = {
      id,
      status: "ready",
      renderer: "webgl2",
      ...
      materialIds: EXTERNAL_PARITY_PHYSICAL_MATERIAL_MATRIX.map((material) => material.id),
      materialCount: EXTERNAL_PARITY_PHYSICAL_MATERIAL_MATRIX.length,
    ...
    window.__A3D_EXTERNAL_PARITY_MATERIAL_STUDIO__ = state;
...
function createMaterialRenderItems(textures: StudioTextures, environment: StudioEnvironment): RenderItem[] {
  ...
  const items: RenderItem[] = EXTERNAL_PARITY_PHYSICAL_MATERIAL_MATRIX.map((descriptor, index) => {
...
function createRenderMaterial(descriptor: ExternalParityPhysicalMaterialDescriptor, textures: StudioTextures, environment: StudioEnvironment): PBRMaterial | TexturedPBRMaterial {
  const common = {
...
function textureForNormal(descriptor: ExternalParityPhysicalMaterialDescriptor, textures: StudioTextures): Texture | undefined {
