...
function publish() {
  ...
  window.__GALILEO3D_EXPORTED_PROJECT__ = {
    ...
    screenshotPath: "tests/reports/v4-example-screenshots/editor-authored-v4-checked-in.png",
...
function draw() {
  ...
  for (const [index, node] of project.scene.nodes.entries()) {
    ...
    if (node.mesh?.primitive === "imported" || node.mesh?.assetId) {
      drawImportedModelNode(x, y, color);
...
function drawImportedModelNode(x, y, color) {
  context.save();
