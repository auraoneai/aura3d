...
export async function mountCharacterViewerV4(id: string): Promise<void> {
  ...
  const manifest = await fetch(`/${fixturePath}`).then((response) => response.json()) as CharacterFixture;
  ...
  function render(): void {
    ...
    const state = {
      ...
      sourceAsset: manifest.source.corpusAssetId,
      sourceRepository: manifest.source.repository,
      sourceRevision: manifest.source.revision,
      sourceLicense: manifest.source.license,
      licenseReviewRequired: manifest.source.licenseReviewRequired,
      clipCount: 1,
