...
const artifactDir = "tests/reports/v4-product-visual-parity";
const sourceFiles = [
...
export async function createV4ProductVisualParityReport(root = process.cwd()): Promise<V4ProductVisualParityReport> {
  ...
  const unityProductBaseline = readJson(root, "tests/reports/v4-unity-product-visual-baseline.json");
  const unrealProductBaseline = readJson(root, "tests/reports/v4-unreal-product-visual-baseline.json");
    ...
    try {
      ...
      return {
        ...baseReport(root, {
          ...
          command: "pnpm audit:v4-product-visual-parity",
          runIdPrefix: "v4-product-visual-parity",
          sourceFiles,
          screenshotPaths,
          violations,
          blockedClaims: [
...
export function validateExternalProductVisualBaseline(root: string, report: Record<string, unknown> | null, engine: "unity" | "unreal", galileoScreenshotPath?: string): ProductExternalBaselineValidation {
  const reportPath = `tests/reports/v4-${engine}-product-visual-baseline.json`;
...
function renderViolations(render: ProductVisualRender): string[] {
  return [
    ...
    ...(render.metrics.colorBuckets >= 5 ? [] : [`${render.engine}: product render has too few color buckets`]),
...
function sharedBrowserHelpers(): string {
  return String.raw`
    const PRODUCT_VISUAL_PRESENTATION_SCALE = 0.84;
  ...
  return `
    ...
    export async function renderProductVisualParity(canvas) {
      ...
      const materials = new Map(descriptor.materials.map((material) => {
        if (material.kind === "unlit") return [material.id, new UnlitMaterial({ name: "parity-" + material.id, color: material.color })];
