...
const reportPath = "tests/reports/v4-pbr-visual-parity.json";
const artifactDir = "tests/reports/v4-pbr-visual-parity";
const sourceFiles = [
    ...
    try {
      ...
      return {
        ...baseReport(root, {
          ...
          command: "pnpm audit:v4-pbr-visual-parity",
          runIdPrefix: "v4-pbr-visual-parity",
          sourceFiles,
          screenshotPaths,
          violations,
          blockedClaims: [
...
function renderViolations(render: PbrVisualRender): string[] {
  return [
    ...
    ...(render.metrics.colorBuckets >= 7 ? [] : [`${render.engine}: PBR render has too few color buckets`]),
...
function galileoBundleSource(): string {
  return `
    ...
    export async function renderPbrVisualParity(canvas) {
      ...
      const sphere = Geometry.uvSphere(0.5, 40, 20);
