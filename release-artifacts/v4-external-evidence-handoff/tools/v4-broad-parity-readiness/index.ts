...
const reportPath = "tests/reports/v4-broad-parity-readiness.json";
...
export function createV4BroadParityReadinessReport(root = process.cwd()) {
  ...
  const three = readJson(root, "tests/reports/v4-comparison-threejs.json");
  const babylon = readJson(root, "tests/reports/v4-comparison-babylon.json");
  ...
  const productVisualParity = readJson(root, "tests/reports/v4-product-visual-parity.json");
  const postprocessSuite = readJson(root, "tests/reports/v4-postprocess-suite.json");
  const shadowMapReadiness = readJson(root, "tests/reports/v4-shadow-map-readiness.json");
  const hdrRenderTargetReadiness = readJson(root, "tests/reports/v4-hdr-render-target-readiness.json");
  const pbrGltfReadiness = readJson(root, "tests/reports/v4-pbr-gltf-readiness.json");
  ...
  const claims: V4BroadParityClaimReadiness[] = [
    ...
    ], ["tests/reports/v4-production-readiness.json"], ["productionReady"]),
    ...
    ], ["tests/reports/v4-pbr-gltf-readiness.json"], ["pbrParity"]),
    ...
    ], ["tests/reports/v4-pbr-gltf-readiness.json"], ["gltfParity"]),
    ...
    ], ["tests/reports/v4-hdr-render-target-readiness.json"], ["hdrRenderTargetParity"]),
    ...
    ], ["tests/reports/v4-shadow-map-readiness.json"], ["shadowMapParity"]),
    ...
    ], ["tests/reports/v4-postprocess-suite.json"], ["postprocessSuiteParity"]),
  ...
  const report = {
    ...baseReport(root, {
      ...
      runIdPrefix: "v4-broad-parity-readiness",
      sourceFiles,
      violations: blockers,
    }),
...
function webgpuParityClaim(webgpu: Record<string, unknown> | null): V4BroadParityClaimReadiness {
  ...
  return readiness({
    ...
    evidencePaths: ["tests/reports/v4-webgpu-parity.json", "tests/reports/webgpu-hardware-matrix.json"],
...
function requiredCommandsForClaim(id: V4BroadParityClaimId): readonly string[] {
  switch (id) {
    ...
    case "babylonjs-broad-superiority":
      return ["pnpm verify:v4-benchmarks", "pnpm audit:v4-broad-parity", "pnpm verify:v4-report-freshness"];
    case "unity-parity":
    case "unreal-parity":
    case "unity-unreal-replacement":
      return ["pnpm verify:v4-external-engine-baselines", "pnpm audit:v4-product-visual-parity", "pnpm audit:v4-unity-unreal-parity", "pnpm audit:v4-broad-parity"];
    case "production-readiness":
      return ["pnpm verify:v4", "G3D_PUBLIC_DEMO_URL=https://... pnpm verify:public-demo-deployment", "pnpm audit:v4-production-readiness", "pnpm audit:v4-broad-parity"];
    case "full-pbr-parity":
      return ["pnpm audit:v4-pbr-visual-parity", "pnpm audit:v4-pbr-reference-readiness", "pnpm audit:v4-pbr-gltf-readiness", "pnpm audit:v4-broad-parity"];
    case "full-gltf-parity":
      return ["pnpm verify:v4-assets", "pnpm verify:v4-khronos-visuals", "pnpm audit:v4-gltf-loader-visual-parity", "pnpm audit:v4-pbr-gltf-readiness", "pnpm audit:v4-broad-parity"];
    case "full-webgpu-parity":
      return ["pnpm verify:v4-rendering", "pnpm audit:v4-broad-parity"];
    case "production-hdr-render-target-parity":
      return ["pnpm verify:v4-rendering", "pnpm audit:v4-hdr-visual-parity", "pnpm audit:v4-hdr-render-target-readiness", "pnpm audit:v4-broad-parity"];
    case "production-shadow-map-parity":
      return ["pnpm verify:v4-rendering", "pnpm audit:v4-shadow-visual-parity", "pnpm audit:v4-shadow-map-readiness", "pnpm audit:v4-broad-parity"];
    case "full-postprocess-suite-parity":
      return ["pnpm verify:v4-rendering", "pnpm audit:v4-postprocess-suite", "pnpm audit:v4-broad-parity"];
    case "rendered-product-visual-parity":
      return ["pnpm audit:v4-product-visual-parity", "pnpm audit:v4-unity-unreal-parity", "pnpm audit:v4-broad-parity"];
  }
}
...
function broadEngineClaim(id: V4BroadParityClaimId, claim: string, engine: "threejs" | "babylonjs", report: Record<string, unknown> | null): V4BroadParityClaimReadiness {
  ...
  return readiness({
    ...
    evidencePaths: [`tests/reports/v4-comparison-${engine === "threejs" ? "threejs" : "babylon"}.json`],
    checks: [
...
function featureParityClaim(
  ...
  return readiness({
    ...
    claim,
    requiredEvidence,
    evidencePaths: ["tests/reports/v4-rendering.json", "tests/reports/v4-asset-corpus.json", "tests/reports/v4-engine-comparison.json"],
    checks: [
...
function webgpuFullParityEvidenceBlockers(report: Record<string, unknown> | null): string[] {
  ...
  return [
    ...(report?.ok === true ? [] : ["tests/reports/v4-webgpu-parity.json is missing or failing"]),
...
function supportedNarrowClaimsFromComparison(comparison: Record<string, unknown> | null): V4SupportedNarrowClaim[] {
  ...
  return comparison.supportedNicheClaims.flatMap((entry): V4SupportedNarrowClaim[] => {
    ...
    const reportPath = typeof evidence.reportPath === "string" ? evidence.reportPath : "tests/reports/v4-engine-comparison.json";
    return [{
      id: entry.id,
      status: "supported",
      claim: entry.claim,
      evidencePaths: [...new Set(["tests/reports/v4-engine-comparison.json", reportPath])],
