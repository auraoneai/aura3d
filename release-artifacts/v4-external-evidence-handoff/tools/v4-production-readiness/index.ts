...
const reportPath = "tests/reports/v4-production-readiness.json";
const sourceFiles = [
...
export function createV4ProductionReadinessReport(root = process.cwd()): V4ProductionReadinessReport {
  ...
  const engineComparison = readJson(root, "tests/reports/v4-engine-comparison.json");
  const visualQuality = readJson(root, "tests/reports/v4-visual-quality.json");
  const broadReadiness = readJson(root, "tests/reports/v4-broad-parity-readiness.json");
  ...
  const releaseAreas = [
    releaseArea("local-verification", "All scoped V4 verification commands pass on this checkout.", "Core V4 reports are present and locally generated.", [
      ...(currentCapability?.ok === true ? [] : ["v4-current-capability report is missing or failing"]),
      ...(rendering?.ok === true ? [] : ["v4-rendering report is missing or failing"]),
  ...
  return {
    ...baseReport(root, {
      ...
      runIdPrefix: "v4-production-readiness",
      sourceFiles: [
        ...sourceFiles,
...
function validatePublicDemoDeploymentWorkflow(root: string): { readonly ok: boolean; readonly blockers: readonly string[] } {
  ...
  const requiredMarkers = [
    ...
    ["report freshness verification", "pnpm verify:v4-report-freshness"],
    ...
    ["broad parity readiness report upload", "tests/reports/v4-broad-parity-readiness.json"],
    ["completion audit report upload", "tests/reports/v4-completion-audit.json"],
    ["completion audit runbook upload", "tests/reports/v4-completion-audit-runbook.md"],
    ["report freshness upload", "tests/reports/v4-report-freshness.json"],
  ] as const;
