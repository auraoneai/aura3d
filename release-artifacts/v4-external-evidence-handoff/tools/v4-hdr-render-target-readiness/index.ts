...
const reportPath = "tests/reports/v4-hdr-render-target-readiness.json";
const sourceFiles = [
...
export function createV4HdrRenderTargetReadinessReport(root = process.cwd()): V4HdrRenderTargetReadinessReport {
  ...
  const hdrVisual = readJson(root, "tests/reports/v4-hdr-visual-parity.json");
  ...
  const supportedEvidence = [
    ...(postprocessChecks.colorManagementStateMatchesControls === true ? ["runtime-tone-mapper-exposure-white-point-controls"] : []),
    ...(postprocessChecks.linearSrgbCalibrationProof === true ? ["linear-srgb-calibration-browser-evidence"] : []),
    ...(postprocessChecks.oldBranchPresetEvidence === true ? ["tone-mapping-preset-and-auto-exposure-histogram-evidence"] : []),
  ...
  const validationRows = [
    validation("real-scene-tone-mapping-controls", supportedEvidence.includes("runtime-tone-mapper-exposure-white-point-controls") && supportedEvidence.includes("linear-srgb-calibration-browser-evidence"), "tests/reports/v4-rendering.json:postprocess-lab-runtime-color-management-controls", [
      "runtime tone mapper/color-space controls are not proven",
    ]),
    validation("auto-exposure-histogram-controls", supportedEvidence.includes("tone-mapping-preset-and-auto-exposure-histogram-evidence"), "tests/reports/v4-rendering.json:postprocess-lab-runtime-color-management-controls", [
    ...
    validation("bounded-hdr-ibl-evidence", boundedHdrIblEvidence, "tests/reports/v4-hdr-ibl-readiness.json", [
      "bounded linear-HDR IBL material and flagship screenshot evidence is missing",
    ]),
    validation("floating-point-hdr-render-targets", hasHdrBrowserFloatEvidence(hdrBrowser), "tests/reports/v4-hdr-render-target-browser.json", [
      "floating-point HDR render target browser evidence is missing",
    ]),
    validation("real-webgpu-hdr-render-target-postprocess", hasWebGPUHdrRenderTargetPostprocessEvidence(webgpu), "tests/reports/v4-webgpu-parity.json:real-webgpu-hdr-render-target-postprocess", [
      "real WebGPU HDR render-target float readback and tone-mapping postprocess evidence is missing",
    ]),
    validation("competitor-hdr-parity", comparison?.hdrRenderTargetParity === true || boundedHdrVisualParity, "tests/reports/v4-hdr-visual-parity.json", [
      "same-scene HDR/render-target parity against competitors is not proven",
    ]),
    validation("external-hdr-baseline-prepared-slot", hasExternalBaselineSlot(externalBaselines, "hdr-render-target"), "tests/reports/v4-external-engine-baselines.json", [
  ...
  return {
    ...baseReport(root, {
      ...
      command: "pnpm audit:v4-hdr-render-target-readiness",
      runIdPrefix: "v4-hdr-render-target-readiness",
      sourceFiles,
      violations,
      blockedClaims: [
