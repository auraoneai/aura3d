...
import { baseReport, isRecord, writeJson } from "../v4-reporting/index.js";

const reportPath = "tests/reports/v4-github-external-readiness.json" as const;
...
export function createV4GithubExternalReadinessReport(root = process.cwd(), runner: CommandRunner = defaultCommandRunner): V4GithubExternalReadinessReport {
  ...
  const report = {
    ...baseReport(root, {
      ok: true,
      command: "pnpm audit:v4-github-external-readiness",
      runIdPrefix: "v4-github-external-readiness",
      sourceFiles,
    }),
    ...
    nextCommands: blockers.length === 0 ? [
      ...
      "gh workflow run v4-external-engine-baselines.yml --repo <owner/repo> --ref <default-branch> -f engine=all",
...
function checkActionsConfiguration(root: string, runner: CommandRunner, repo: string): GithubReadinessCheck {
  ...
  return {
    ...
    evidence: [
      ...
      "G3D_RUN_UNITY_UNREAL_CLI_SMOKE is set to true inside .github/workflows/v4-external-engine-baselines.yml.",
    ],
    blockers: [
...
function blockedNextCommands(repo: string | undefined, defaultBranch: string | undefined, currentBranch: string | undefined): readonly string[] {
  return [
    ...
    repo && defaultBranch ? `gh workflow run v4-external-engine-baselines.yml --repo ${repo} --ref ${defaultBranch} -f engine=all` : "gh workflow run v4-external-engine-baselines.yml --repo <owner/repo> --ref <default-branch> -f engine=all",
    "pnpm preflight:v4-parity:after-external-evidence",
  ];
}
