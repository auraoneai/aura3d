...
export function createV4EcosystemReadinessReport(root = process.cwd()): V4EcosystemReadinessReport {
  ...
  const documentationCoverage = [
    ...
    check("tutorials-link-running-browser-evidence", documentationFiles.filter((path) => path.startsWith("docs/tutorials/")).every((path) => textIncludes(root, path, ["pnpm"])), documentationFiles.filter((path) => path.startsWith("docs/tutorials/")), ["tutorial docs must include runnable verification or serve commands"]),
