...
const appImports = listFiles("apps")
  .filter((path) => /apps\/v5-/.test(path) && /\.[tj]s$/.test(path))
...
const templateWorkspaceFiles = listFiles("templates")
  .filter((path) => /templates\/v5-/.test(path) && /package\.json$/.test(path))
...
const blockedClaims = existsSync(resolve("docs/project/v5-roadmap-blocked-claims.md")) ? readFileSync(resolve("docs/project/v5-roadmap-blocked-claims.md"), "utf8") : "";
const progress = readFileSync(resolve("docs/project/v5-roadmap-progress.md"), "utf8");
...
const checks = [
  ...
  { id: "no-v5-app-internal-renderer-test-imports", pass: appImports.length === 0, detail: appImports.join(", ") || "V5 apps use public packages" },
  { id: "no-v5-template-workspace-deps", pass: templateWorkspaceFiles.length === 0, detail: templateWorkspaceFiles.join(", ") || "V5 templates do not use workspace:*" },
  ...
  { id: "broad-replacement-readiness-pass", pass: readJson<GenericReport>("tests/reports/v5-broad-replacement-readiness.json").pass === true, detail: "broad replacement gate passed" },
...
const report = {
  schema: "g3d-v5-completion-audit/v1",
...
writeReport("tests/reports/v5-completion-audit.json", report);
if (!report.pass) {
