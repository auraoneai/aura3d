...
const matrix = readJson<MatrixReport>("tests/reports/v5-threejs-compatibility-matrix.json");
const inventory = readJson<GenericReport>("tests/reports/v5-threejs-inventory.json");
const examples = readJson<{ readonly pass: boolean; readonly examples: readonly { readonly browserTested: boolean }[] }>("tests/reports/v5-examples-readiness.json");
const visualParity = readJson<{ readonly pass: boolean; readonly comparisons: readonly { readonly id: string; readonly visualScore: number }[] }>("tests/reports/v5-threejs-visual-parity.json");
const migration = readJson<GenericReport>("tests/reports/v5-migration-readiness.json");
const packageSmoke = readJson<GenericReport>("tests/reports/v5-package-smoke.json");
const externalConsumer = readJson<GenericReport>("tests/reports/v5-external-consumer.json");
const docs = readJson<GenericReport>("tests/reports/v5-docs-readiness.json");
const release = readJson<GenericReport>("tests/reports/v5-release-readiness.json");
const claims = readJson<{ readonly pass: boolean; readonly claims: readonly { readonly id: string; readonly status: string }[] }>("tests/reports/v5-claim-registry.json");
const reviewText = existsSync(resolve("docs/project/v5-roadmap-human-visual-review.md")) ? readFileSync(resolve("docs/project/v5-roadmap-human-visual-review.md"), "utf8") : "";
...
const report = {
  schema: "g3d-v5-broad-replacement-readiness/v1",
...
writeReport("tests/reports/v5-broad-replacement-readiness.json", report);
if (!report.pass) {
