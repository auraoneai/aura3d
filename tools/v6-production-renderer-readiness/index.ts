...
const external = json("tests/reports/v6-external-consumer.json");
const performance = json("tests/reports/v6-performance-readiness.json");
const three = json("tests/reports/v6-threejs-parity-readiness.json");
const checks = [
  ...
  { id: "threejs-parity", pass: three.pass === true, detail: "tests/reports/v6-threejs-parity-readiness.json" },
  { id: "external-consumer", pass: external.pass === true, detail: "tests/reports/v6-external-consumer.json" },
  { id: "performance", pass: performance.pass === true, detail: "tests/reports/v6-performance-readiness.json" }
];
const report = {
  schema: "g3d-v6-production-renderer-readiness/v1",
  generatedAt: new Date().toISOString(),
  pass: checks.every((check) => check.pass),
  galleryEntryCount: entries.length,
  checks
};
write("tests/reports/v6-production-renderer-readiness.json", report);
if (!report.pass) {
