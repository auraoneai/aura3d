...
/**
 * Real-pixel animation fidelity gate.
 *
...
export interface AnimationVisualFidelityReport {
  readonly schema: "animation-studio-visual-fidelity/v1";
  readonly ok: boolean;
...
export interface AnimationVisualFidelityThresholds {
...
export const defaultFidelityThresholds: AnimationVisualFidelityThresholds = {
  minMeanLuma: 6,
  maxMeanLuma: 245,
...
const defaultOut = "tests/reports/aura3d11/animation-visual-fidelity.json";
...
function evaluateFrame(metrics: FrameVisualMetrics, thresholds: AnimationVisualFidelityThresholds): string[] {
...
export async function createAnimationVisualFidelityReport(
  root = process.cwd(),
  options: { framesDir?: string; thresholds?: AnimationVisualFidelityThresholds } = {}
): Promise<AnimationVisualFidelityReport> {
  ...
  return {
    schema: "animation-studio-visual-fidelity/v1",
...
export function writeAnimationVisualFidelityReport(root: string, report: AnimationVisualFidelityReport, out = defaultOut): void {
...
if (
  currentScript.endsWith("tools/animation-studio-visual-fidelity-gate/index.ts") ||
  currentScript.endsWith("tools/animation-studio-visual-fidelity-gate/index.js")
) {
  ...
  createAnimationVisualFidelityReport(root, framesDir ? { framesDir } : {})
    .then((report) => {
      writeAnimationVisualFidelityReport(root, report, out);
