...
let reportDir = ADVANCED_GALLERY_CONTEXTUAL_REPORT_DIR;
let outputPath = join(reportDir, "visual-review-report.json");
let contactSheetPath = join(reportDir, "current-contact-sheet.png");
let visualRegressionInventoryPath = join(reportDir, "visual-regression-inventory.json");
...
async function main(): Promise<void> {
  ...
  const screenshotPath = resolveExistingEvidencePath(demo.visualReview.screenshot);
...
function configureCli(args: readonly string[]): void {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--report-dir") {
      ...
      const value = args[index + 1];
      if (!value) throw new Error("--contact-sheet requires a value");
      contactSheetPath = value;
      contactSheetExplicit = true;
      index += 1;
    } else if (arg === "--visual-regression-inventory") {
      const value = args[index + 1];
      if (!value) throw new Error("--visual-regression-inventory requires a value");
      visualRegressionInventoryPath = value;
      visualRegressionInventoryExplicit = true;
      index += 1;
    } else {
      throw new Error(`Unknown argument: ${arg}`);
    }
  }
  if (!outputExplicit) outputPath = join(reportDir, "visual-review-report.json");
  if (!contactSheetExplicit) contactSheetPath = join(reportDir, "current-contact-sheet.png");
  if (!visualRegressionInventoryExplicit) visualRegressionInventoryPath = join(reportDir, "visual-regression-inventory.json");
}

function resolveExistingEvidencePath(path: string): string {
  if (existsSync(path)) return path;
  const legacyPath = legacyPathForContextualPath(path);
  return legacyPath !== path && existsSync(legacyPath) ? legacyPath : path;
}
