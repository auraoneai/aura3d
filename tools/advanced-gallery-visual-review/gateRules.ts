...
export function acceptedMetadataBlockers(input: VisualReviewMetadataForGate): string[] {
  ...
  if (!isAdvancedGalleryScreenshotPath(input.screenshot)) {
    blockers.push("Accepted review screenshot must live under the contextual advanced-gallery report directory or its legacy compatibility alias and be a PNG.");
  }
...
export function isAdvancedGalleryScreenshotPath(path: string): boolean {
  return [
    ADVANCED_GALLERY_CONTEXTUAL_REPORT_DIR,
    ADVANCED_GALLERY_LEGACY_REPORT_DIR
  ].some((dir) => path.startsWith(`${dir}/`) && /\.png$/i.test(path));
}
