...
import { createSkinningBlendController } from "/apps/v8-skinning-blending/src/blendController.ts";
...
async function run(): Promise<void> {
  ...
  try {
    ...
    const ready: SkinningBlendingParityReady = {
      status: "ready",
      schema: "g3d-v9-skinning-blending-parity/v1",
...
async function renderG3D(canvas: HTMLCanvasElement) {
  ...
  const result = renderer.renderFrame({
    ...
    metadata: {
      assetId: "v9-skinning-blending-parity",
      assetName: "V9 Skinning Blending Parity",
      assetUri: "/tools/v9-skinning-blending-parity/",
