...
const requiredFiles = [
  ...scenes.map((scene) => `benchmarks/v3/shared/scenes/${scene}-scene.ts`),
  ...scenes.map((scene) => `benchmarks/v3/galileo/${scene}-scene.ts`),
  ...scenes.map((scene) => `benchmarks/v3/threejs/${scene}-scene.ts`),
  "tests/browser/v3-threejs-comparison.spec.ts"
] as const;
const manifestPath = resolve("tests/reports/v3-threejs-comparison/manifest.json");
...
const screenshotChecks = scenes.flatMap((scene) => ["g3d", "threejs", "diff"].map((kind) => {
  const path = `tests/reports/v3-threejs-comparison/${scene}-${kind}.png`;
  return {
    scene,
    kind,
    path,
...
const report = {
  schema: "g3d-v3-threejs-comparison/v1",
  ...
  wins,
  gaps,
  browserManifestPath: "tests/reports/v3-threejs-comparison/manifest.json",
  browserManifestExists: existsSync(manifestPath)
};

const reportPath = resolve("tests/reports/v3-threejs-comparison.json");
