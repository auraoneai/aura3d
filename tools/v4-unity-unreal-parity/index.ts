...
const reportPath = "tests/reports/v4-unity-unreal-parity.json";
const sourceFiles = [
  ...
  "examples/editor-authored-v4-app/main.ts",
  ...
  "tests/browser/runtime-v4.spec.ts",
...
export function createV4UnityUnrealParityReport(root = process.cwd()): V4UnityUnrealParityReport {
  ...
  const baselineKitEvidence = baselineKit?.ok === true
    ...
    : "External baseline kit is absent; run pnpm verify:v4-external-engine-baselines.";
  ...
  const deploymentEvidenceText = deploymentEvidence.staticExportOk && deploymentEvidence.staticDemoServerSmokeOk
    ...
    : "Local static demo export or local static-server smoke is missing; run pnpm preflight:v4-production-readiness.";
  ...
  const runtimeEvidenceText = runtimeEvidence.runtimeReportOk
    ...
    : "V4 runtime report is missing or failing; run pnpm verify:v4-runtime.";
  ...
  const renderingEvidenceText = renderingEvidence.renderingReportOk
    ...
    : "V4 rendering report is missing or failing; run pnpm verify:v4-rendering.";
  ...
  return {
    ...baseReport(root, {
      ...
      runIdPrefix: "v4-unity-unreal-parity",
      sourceFiles,
      violations,
      blockedClaims: [
        "Unity parity",
...
function createEditorWorkflowEvidence(root: string): V4EditorWorkflowEvidence {
  ...
  const prefab = readJson(root, "tests/reports/v4-editor-prefab-workflow.json");
...
function externalEngineBaseline(root: string, engine: "unity" | "unreal", baselineSlots: readonly Record<string, unknown>[]): ExternalEngineBaseline {
  ...
  const baselineReportPath = engine === "unity" ? "tests/reports/v4-unity-baseline-render.json" : "tests/reports/v4-unreal-baseline-render.json";
  ...
  const productVisualBaselineReportPath = `tests/reports/v4-${engine}-product-visual-baseline.json`;
  ...
  const blockers = [
    ...
    ...(productVisualBaselineOk ? [] : [
      productVisualBaselinePresent
        ? `${productVisualBaselineReportPath} is present but did not pass the product visual baseline validation in tests/reports/v4-product-visual-parity.json.`
...
function validateExternalSceneBaseline(root: string, engine: "unity" | "unreal", slot: Record<string, unknown>): ExternalSceneBaselineValidation {
  ...
  const reportPath = typeof targets[engine] === "string" ? targets[engine] : `tests/reports/v4-${engine}-${baselineKind}-baseline.json`;
  const report = readJson(root, reportPath);
  const metrics = isRecord(report?.metrics) ? report.metrics : {};
