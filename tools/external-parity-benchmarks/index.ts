...
const reportPath = "tests/reports/external-parity-engine-comparison.json";
...
export function createV4BenchmarkReport() {
  ...
  const base = baseReport(root, {
    ...
    command: "pnpm verify:external-parity-benchmarks",
    runIdPrefix: "external-parity-benchmarks",
    sourceFiles,
    screenshotPaths,
    violations,
  });
