...
const requiredApps = [
  ...
  "v6-automotive-configurator",
  "v6-architecture-viewer",
  "v6-asset-inspector",
  "v6-material-studio",
  "v6-character-viewer",
  "v6-cinematic-postprocess",
  "v6-large-scene-lab",
  "v6-webgpu-lab",
  "v6-threejs-parity-lab"
] as const;
const requiredAppFiles = ["index.html", "src/main.ts", "src/scene.ts", "src/ui.ts", "src/assets.ts", "README.md"] as const;
const reportPath = resolve("tests/reports/v6-app-suite-readiness.json");
const appReports = requiredApps.map((appId) => {
  const path = resolve(`tests/reports/v6-app-suite/${appId}.json`);
...
const webgpuLab = appReports.find((entry) => entry.appId === "v6-webgpu-lab")?.report?.runtime?.webgpu;
const checks = [
  ...appFileChecks,
  ...runtimeChecks,
  ...screenshotChecks,
...
const report = {
  schema: "g3d-v6-app-suite-readiness/v1",
