...
export interface CartoonStudioDocsClaimsReport {
  readonly schema: "cartoon-studio-docs-claims/v1";
  readonly ok: boolean;
...
export interface CartoonStudioDocsClaimsOptions {
  readonly out?: string;
  readonly generatedAt?: string;
  readonly paths?: readonly string[];
}

const defaultOut = "tests/reports/aura3d11/cartoon-docs-claims.json";
...
const forbiddenPatterns: ReadonlyArray<readonly [RegExp, string]> = [
  ...
  [/\bturn(?:s|ed|ing)?\s+(?:any\s+)?(?:2D\s+)?image\s+into\s+(?:a\s+)?(?:3D\s+)?(?:cartoon|episode|video)\b/i, "single-image-to-cartoon claim"],
...
export function createCartoonStudioDocsClaimsReport(root = process.cwd(), options: CartoonStudioDocsClaimsOptions = {}): CartoonStudioDocsClaimsReport {
  ...
  return {
    schema: "cartoon-studio-docs-claims/v1",
...
export function writeCartoonStudioDocsClaimsReport(root: string, report: CartoonStudioDocsClaimsReport, out = defaultOut): void {
...
if (currentScript.endsWith("tools/cartoon-studio-docs-claims/index.ts") || currentScript.endsWith("tools/cartoon-studio-docs-claims/index.js")) {
  const args = parseArgs(process.argv.slice(2));
  const root = process.cwd();
  const report = createCartoonStudioDocsClaimsReport(root);
  writeCartoonStudioDocsClaimsReport(root, report, typeof args.out === "string" ? args.out : defaultOut);
  if (!report.ok) {
