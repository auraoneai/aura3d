...
const visualReview = readJson<{ readonly needingReview?: number; readonly issues?: readonly unknown[] }>("tests/reports/v9/visual-review.json");
...
const reportIssues = [
  ...requirePassingReport("tests/reports/v9/visual-review.json", "V9 visual review"),
  ...requirePassingReport("tests/reports/v9/same-scene-render.json", "V9 same-scene render"),
  ...visualBlockers
];
...
writeReport(outputPath, {
  ...
  decisions: [{
    ...
    evidence: ["tests/reports/v9/visual-review.json", "tests/reports/v9/same-scene-render.json"],
