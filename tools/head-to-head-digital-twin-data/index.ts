...
const checks = [
  ...
  { id: "frozen-native-viewport", pass: browser.before?.viewport?.width === context.commonRenderContract.viewport.width && browser.before?.viewport?.height === context.commonRenderContract.viewport.height && browser.before?.viewport?.dpr === context.commonRenderContract.devicePixelRatio && statSync(resolve("tests/reports/current-head-to-head/digital-twin-data/aura.png")).size > 10_000 && statSync(resolve("tests/reports/current-head-to-head/digital-twin-data/three.png")).size > 10_000 },
...
const report = {
  ...
  comparison: {
    ...
    observedLosses: [
      "Aura submits materially more draw calls for the same workcell and data marker.",
      "The retained Aura capture has a lighter background and flatter workcell contrast than the retained Three.js control under the shared authored lighting intent."
    ],
