...
const reportPath = "tests/reports/external-parity-postprocess-suite.json";
const sourceFiles = [
...
export function createExternalParityPostprocessSuiteReadinessReport(root = process.cwd()): ExternalParityPostprocessSuiteReadinessReport {
  ...
  const validationRows = [
    ...
    validation("postprocess-color-controls", rootPostprocessSuiteEvidence || (gradingChecks.gradingControlsPublished === true && gradingChecks.advancedPostprocessActive === true), "tests/browser/rendering-root-quality-gate.spec.ts + tests/reports/external-parity-rendering.json:postprocess-lab-runtime-color-grading-controls", [
    ...
    validation("old-branch-tone-mapping-preset-port", rootPostprocessSuiteEvidence || colorManagementChecks.oldBranchPresetEvidence === true, "tests/browser/rendering-root-quality-gate.spec.ts + tests/reports/external-parity-rendering.json:postprocess-lab-runtime-color-management-controls", [
      "old-branch tone-mapping presets and auto-exposure histogram evidence are not proven in browser state",
    ]),
    validation("competitor-postprocess-visual-parity", hasBoundedPostprocessComparison(comparison), "tests/reports/external-parity-engine-comparison.json", [
      "same-scene bounded postprocess visual parity against Three.js and Babylon.js is not proven",
    ]),
    validation("hdr-float-postprocess-tone-mapping", hasHdrFloatPostprocessEvidence(hdrBrowser), "tests/reports/external-parity-hdr-render-target-browser.json", [
      "HDR float render-target readback is not proven through the postprocess tone-mapping path",
    ]),
    validation("webgpu-hdr-postprocess-tone-mapping", hasWebGPUHdrPostprocessEvidence(webgpu), "tests/reports/external-parity-webgpu-parity.json:real-webgpu-hdr-render-target-postprocess", [
      "real WebGPU HDR render-target readback is not proven through the postprocess tone-mapping path",
    ]),
    validation("bounded-hdr-image-based-lighting", hasBoundedHdrIblEvidence(hdrIbl), "tests/reports/external-parity-hdr-ibl-readiness.json", [
      "bounded linear-HDR image-based-lighting evidence is missing",
    ]),
    validation("external-postprocess-baseline-prepared-slot", hasExternalBaselineSlot(externalBaselines, "postprocess-suite"), "tests/reports/external-parity-external-engine-baselines.json", [
      "external Unity/Unreal postprocess-suite baseline slot is not prepared",
    ]),
    validation("full-suite-no-blocked-effects", false, "tools/external-parity-postprocess-suite/index.ts", [
  ...
  return {
    ...baseReport(root, {
      ...
      command: "pnpm audit:external-parity-postprocess-suite",
      runIdPrefix: "external-parity-postprocess-suite",
      sourceFiles,
      screenshotPaths,
      violations,
      blockedClaims: [
