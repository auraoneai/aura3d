...
const progress = readFileSync(resolve("docs/project/v6-roadmap-progress.md"), "utf8");
...
const productDecisionPasses = reportPasses("tests/reports/v6-product-decision-record.json");
const literalCompletionPasses = reportPasses("tests/reports/v6-literal-completion.json");
const pass =
...
const report = {
  schema: "g3d-v6-completion-audit/v1",
...
mkdirSync(dirname(resolve("tests/reports/v6-completion-audit.json")), { recursive: true });
writeFileSync(resolve("tests/reports/v6-completion-audit.json"), `${JSON.stringify(report, null, 2)}\n`);
if (!report.pass) {
