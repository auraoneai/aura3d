...
const outputDir = resolve(root, process.env.A3D_AUDIT_OUTPUT ?? "tests/reports/live-showcase-2.0.1");
...
if (packageJson.version !== "2.0.1") throw new Error(`Expected Aura3D 2.0.1, found ${packageJson.version}`);
...
for (const card of cards) {
  ...
  }).catch(() => ({ title: "", ready: null, canvas: null, fallbackVisible: true, bodyTextLength: 0 }));
  ...
  const pass = !navigationError && !screenshotError && !state.fallbackVisible && consoleErrors.length === 0
  ...
  results.push(result);
  process.stdout.write(`${card.number}/36 ${pass ? "PASS" : "REVIEW"} ${card.title}\n`);
  await page.close();
}
