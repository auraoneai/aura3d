...
interface V9StereoParityReady {
  ...
  readonly threejs: {
    ...
    readonly effect: { readonly actualStereoEffect: true; readonly scissorViewports: true; readonly halfWidthViewports: true };
    readonly pixels: PixelStats;
  };
...
interface V9StereoParityError {
  ...
  readonly expectedReferenceEffect: "StereoEffect";
}
...
async function run(): Promise<void> {
  ...
  try {
    ...
    const ready: V9StereoParityReady = {
      status: "ready",
      schema: "g3d-v9-stereo-parity/v1",
      ...
      threejs: {
        ...
        effect: {
          actualStereoEffect: threejs.actualStereoEffect,
          scissorViewports: threejs.scissorViewports,
          halfWidthViewports: threejs.halfWidthViewports
        },
...
async function renderG3DStereo(outputCanvas: HTMLCanvasElement) {
  ...
  const frame = computePerspectiveCameraFrame(SCENE.frameBounds, { width: SCENE.eyeWidth, height: SCENE.height }, {
    yawRadians: -0.38,
    pitchRadians: -0.1,
    paddingRatio: 0.13,
    fovYRadians: 0.66,
  ...
  const effectPlan = createStereoEffectPlan({
    ...
    width: SCENE.width,
    height: SCENE.height,
    eyeSeparation: SCENE.eyeSeparation,
  ...
  const stereoRig = createStereoCameraRig({
    frame,
    viewport: { x: 0, y: 0, width: SCENE.width, height: SCENE.height },
  ...
  return {
    ...
    rigViews: stereoRig.views.length,
    layout: effectPlan.layout,
    composition: effectPlan.composition,
    dataUrl: outputCanvas.toDataURL("image/png")
  };
}

async function renderThreeStereo(canvas: HTMLCanvasElement) {
  ...
  if (!(renderer instanceof THREE.WebGLRenderer)) throw new Error("Stereo parity requires an actual THREE.WebGLRenderer.");
  ...
  if (!(effect instanceof StereoEffect)) throw new Error("Stereo parity requires the actual Three.js StereoEffect.");
  effect.setEyeSeparation(SCENE.eyeSeparation * SCENE.parallaxStrength);
  effect.setSize(SCENE.width, SCENE.height);
  effect.render(scene, camera);
  const effectSource = StereoEffect.toString();
  return {
    actualThreeRenderer: true as const,
    actualStereoEffect: true as const,
    scissorViewports: /setScissorTest\( true \)/.test(effectSource) && /setScissor/.test(effectSource),
    halfWidthViewports: /size\.width \/ 2/.test(effectSource) && /setViewport/.test(effectSource),
...
function putFlippedPixels(context: CanvasRenderingContext2D, source: Uint8Array, xOffset: number): void {
  const output = new Uint8ClampedArray(source.length);
  for (let y = 0; y < SCENE.height; y += 1) {
    const sourceY = SCENE.height - y - 1;
    const sourceOffset = sourceY * SCENE.eyeWidth * 4;
    const outputOffset = y * SCENE.eyeWidth * 4;
    output.set(source.slice(sourceOffset, sourceOffset + SCENE.eyeWidth * 4), outputOffset);
  }
  context.putImageData(new ImageData(output, SCENE.eyeWidth, SCENE.height), xOffset, 0);
}

function createSource(resources: StereoResources, cameraPosition: readonly [number, number, number]): RenderSource {
  return {
...
function createResources(eye: "left" | "right"): StereoResources {
  ...
  return {
    ...
    floor: new PBRMaterial({ name: `stereo-parity-floor-${eye}`, baseColor: [0.06 * tint, 0.07 * tint, 0.085 * tint, 1], roughness: 0.58, metallic: 0.05 }),
    red: new PBRMaterial({ name: `stereo-parity-red-${eye}`, baseColor: [0.9, 0.12 * tint, 0.1 * tint, 1], roughness: 0.28, clearcoatFactor: 0.35 }),
    cyan: new PBRMaterial({ name: `stereo-parity-cyan-${eye}`, baseColor: [0.08 * tint, 0.68, 0.92, 1], roughness: 0.26, clearcoatFactor: 0.35 }),
    brass: new PBRMaterial({ name: `stereo-parity-brass-${eye}`, baseColor: [1, 0.62, 0.18, 1], roughness: 0.22, metallic: 0.5 }),
    glass: new PBRMaterial({ name: `stereo-parity-glass-${eye}`, baseColor: [0.46, 0.72, 1, 1], roughness: 0.08, clearcoatFactor: 0.8, transmissionFactor: 0.1 }),
    dark: new PBRMaterial({ name: `stereo-parity-dark-${eye}`, baseColor: [0.02, 0.028, 0.04, 1], roughness: 0.5, metallic: 0.08 })
  };
}

function createItems(resources: StereoResources): readonly RenderItem[] {
  return [
    { label: "stereo-parity-floor", geometry: resources.cube, material: resources.floor, modelMatrix: composeMat4([0, -0.68, 0], [0, 0, 0, 1], [4.2, 0.05, 3.0]) },
    { label: "stereo-parity-back", geometry: resources.cube, material: resources.dark, modelMatrix: composeMat4([0, 0.35, -1.48], [0, 0, 0, 1], [4.2, 2.1, 0.05]) },
    { label: "stereo-parity-center-glass", geometry: resources.sphere, material: resources.glass, modelMatrix: composeMat4([0, 0.32, 0], quatFromEuler(0, 0.25, 0), [0.72, 0.72, 0.72]) },
    { label: "stereo-parity-near-brass", geometry: resources.cube, material: resources.brass, modelMatrix: composeMat4([0.26, -0.2, 0.88], quatFromEuler(0.25, 0.38, 0.1), [0.34, 0.34, 0.34]) },
    { label: "stereo-parity-left-column", geometry: resources.cylinder, material: resources.cyan, modelMatrix: composeMat4([-0.95, -0.16, -0.36], [0, 0, 0, 1], [0.16, 0.9, 0.16]) },
    { label: "stereo-parity-right-column", geometry: resources.cylinder, material: resources.red, modelMatrix: composeMat4([0.95, -0.16, -0.36], [0, 0, 0, 1], [0.16, 0.9, 0.16]) },
    { label: "stereo-parity-far-left", geometry: resources.sphere, material: resources.red, modelMatrix: composeMat4([-1.32, -0.18, -0.9], [0, 0, 0, 1], [0.22, 0.22, 0.22]) },
    { label: "stereo-parity-far-right", geometry: resources.sphere, material: resources.cyan, modelMatrix: composeMat4([1.32, -0.18, -0.9], [0, 0, 0, 1], [0.22, 0.22, 0.22]) }
  ];
}

function createLights(): readonly CollectedLight[] {
  const key = new DirectionalLight("stereo-parity-key");
  key.intensity = 3.6;
  key.color = [1, 0.94, 0.84];
  const rim = new DirectionalLight("stereo-parity-rim");
  rim.intensity = 1.8;
...
function createThreeScene(): THREE.Scene {
  ...
  const brassMaterial = new THREE.MeshStandardMaterial({ color: 0xff9e2d, roughness: 0.22, metalness: 0.5 });
  ...
  scene.add(sphere);
  addThreeCube(scene, [0.26, -0.2, 0.88], [0.34, 0.34, 0.34], [0.25, 0.38, 0.1], brassMaterial);
...
async function dataUrlToPixels(dataUrl: string): Promise<ImageData> {
  ...
  if (!context) throw new Error("Unable to create stereo parity pixel canvas.");
...
function analyzeImageData(image: ImageData): PixelStats {
  ...
  let leftNonBlackPixels = 0;
  let rightNonBlackPixels = 0;
  let lumaTotal = 0;
  ...
  for (let y = 0; y < image.height; y += 1) {
    for (let x = 0; x < image.width; x += 1) {
      ...
      const active = red + green + blue > 12;
      if (active) {
        nonBlackPixels += 1;
        if (x < image.width / 2) leftNonBlackPixels += 1;
        else rightNonBlackPixels += 1;
      }
...
function computeDiff(left: ImageData, right: ImageData): DiffStats {
  if (left.width !== right.width || left.height !== right.height) {
    throw new Error(`Cannot diff stereo captures with mismatched size: ${left.width}x${left.height} vs ${right.width}x${right.height}.`);
  }
  let totalDelta = 0;
  let maxDelta = 0;
  let changedPixels = 0;
...
async function loadImage(dataUrl: string): Promise<HTMLImageElement> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    image.onerror = () => reject(new Error("Unable to decode stereo parity image data URL."));
    image.src = dataUrl;
  });
  return image;
}
