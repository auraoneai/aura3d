...
const reportPath = "tests/reports/v4-pbr-reference-readiness.json";
const sourceFiles = [
...
export function createV4PbrReferenceReadinessReport(root = process.cwd()): V4PbrReferenceReadinessReport {
  ...
  const validationRows = [
    ...
    validation("bounded-browser-pbr-visual-evidence", boundedVisual, "tests/reports/v4-pbr-visual-parity.json", [
      "Bounded Three.js/Babylon browser PBR visual comparison is missing or failing.",
    ]),
    validation("bounded-hdr-ibl-evidence", boundedHdrIbl, "tests/reports/v4-hdr-ibl-readiness.json", [
      "Bounded linear-HDR IBL resource evidence is missing.",
    ]),
    validation("external-pbr-baseline-prepared-slot", hasExternalBaselineSlot(externalBaselines, "pbr-visual"), "tests/reports/v4-external-engine-baselines.json", [
  ...
  return {
    ...baseReport(root, {
      ...
      command: "pnpm audit:v4-pbr-reference-readiness",
      runIdPrefix: "v4-pbr-reference-readiness",
      sourceFiles,
      violations,
      blockedClaims: [
        "full PBR parity",
