...
const existing = readJson(root, "tests/reports/v4-rendering.json");
const webgpuParity = readJson(root, "tests/reports/v4-webgpu-parity.json") ?? readJson(root, "tests/reports/webgpu-parity.json");
...
const checks = [
  {
    id: "v4-rendering-report-produced-by-renderer-agent",
    passed: existing?.ok === true,
    evidencePaths: ["tests/reports/v4-rendering.json"],
    blocker: "V4 renderer visual quality report is not yet passing.",
  },
  {
    id: "v4-rendering-screenshots-exist-and-are-valid-pngs",
  ...
  },
  {
    ...
    evidencePaths: ["examples/postprocess-lab/main.ts", "tests/browser/rendering-v4-visuals.spec.ts", "tests/reports/v4-example-screenshots/postprocess-lab.png"],
  ...
  },
  {
    ...
    evidencePaths: ["tests/reports/v4-webgpu-parity.json", "tests/browser/webgpu-parity.spec.ts"],
  ...
  },
  {
    ...
    evidencePaths: ["tests/reports/v4-webgpu-parity.json", "docs/project/v4-decision-gates.md"],
  ...
  },
  {
    ...
    evidencePaths: ["examples/webgpu-capability/main.ts", "tests/browser/rendering-v4-visuals.spec.ts", "tests/reports/v4-example-screenshots/webgpu-capability.png"],
...
const report = {
  ...baseReport(root, {
    ...
    command: "pnpm verify:v4-rendering",
    runIdPrefix: "v4-rendering",
    sourceFiles: ["tools/v4-rendering/index.ts", "tests/reports/v4-rendering.json"],
    violations,
  }),
...
if (isMain) {
  if (existing?.ok === true && violations.length === 0) {
    const normalized = {
      ...
      normalizedBy: "tools/v4-rendering/index.ts",
      checks,
      screenshotValidations,
      webgpuEvidence: summarizeWebGPUEvidence(),
    };
    writeJson(root, "tests/reports/v4-rendering.json", normalized);
    console.log(JSON.stringify({ ok: true, normalized: "tests/reports/v4-rendering.json" }, null, 2));
    process.exit(0);
  }
  writeJson(root, "tests/reports/v4-rendering.json", report);
