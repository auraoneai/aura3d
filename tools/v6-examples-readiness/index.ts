...
const catalog = JSON.parse(readFileSync(resolve("examples/v6/catalog.json"), "utf8")) as Catalog;
const reports = catalog.examples.map((example) => {
  const sourcePath = resolve(`examples/v6/${example.slug}/main.ts`);
  const screenshotPath = resolve(`tests/reports/v6-examples/${example.slug}.png`);
  const runtimePath = resolve(`tests/reports/v6-examples/${example.slug}.json`);
  const source = existsSync(sourcePath) ? readFileSync(sourcePath, "utf8") : "";
  const report = existsSync(runtimePath) ? JSON.parse(readFileSync(runtimePath, "utf8")) as RuntimeReport : {};
  const runtime = report.runtime;
  return {
    slug: example.slug,
    filesPresent: existsSync(resolve(`examples/v6/${example.slug}/index.html`)) && existsSync(sourcePath),
...
const checks = [
  { id: "schema", pass: catalog.schema === "g3d-v6-example-catalog/v1", detail: catalog.schema },
...
const report = {
  schema: "g3d-v6-examples-readiness/v1",
  generatedAt: new Date().toISOString(),
  pass: checks.every((check) => check.pass),
  examples: reports,
  checks
};
const reportPath = resolve("tests/reports/v6-examples-readiness.json");
