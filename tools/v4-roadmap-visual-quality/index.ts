...
const gallery = readJson("tests/reports/v4-screenshot-gallery.json");
...
check("no-legacy-v1-gallery", entries.every((entry) => !String(entry.path).includes("product-viewer-v1") && !String(entry.path).includes("material-studio-v1")), "V4 gallery must not depend on the rejected V1 screenshots.");

const pass = checks.every((entry) => entry.pass);
const report = {
  schema: "g3d-v4-visual-quality/v1",
...
mkdirSync(dirname(resolve("tests/reports/v4-visual-quality.json")), { recursive: true });
writeFileSync(resolve("tests/reports/v4-visual-quality.json"), `${JSON.stringify(report, null, 2)}\n`);
if (!pass) {
