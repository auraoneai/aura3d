...
const missingScreenshots = comparisons.flatMap((comparison) => [comparison.a3dScreenshot, comparison.threejsScreenshot, comparison.diffScreenshot].filter((file) => !existsSync(resolve(file))));
const visualValidations = comparisons.flatMap((comparison) => [
  { id: `${comparison.id}:a3d`, ...validatePngVisual(resolve(comparison.a3dScreenshot), { minByteLength: 30000, minMeanLuma: 32, minColorBuckets: 70, maxDominantBucketRatio: 0.58, minEdgePixelRatio: 0.014, minLocalContrastRatio: 0.03 }) },
...
const checks = [
  ...
  { name: "screenshots", pass: missingScreenshots.length === 0, detail: missingScreenshots.join(", ") || "A3D, Three.js, and diff screenshots exist for every comparison" },
  ...
  { name: "setup-complexity", pass: lowerSetupCount >= 8, detail: `${lowerSetupCount}/8 comparisons have lower A3D setup complexity` },
