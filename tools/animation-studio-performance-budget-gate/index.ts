...
/**
 * Animation Studio "video-render performance budget gate".
 *
...
const defaultManifestPath =
  "packages/create-aura3d/templates/animation-studio/dist/episodes/moon-garden-001/render-manifest.json";
const defaultSummaryPath =
  "packages/create-aura3d/templates/animation-studio/dist/episodes/live-3d/render-live-summary.json";
const defaultOut = "tests/reports/aura3d11/animation-performance-budget.json";
...
export interface PerformanceBudgetReport {
  readonly schema: "animation-studio-performance-budget/v1";
  readonly ok: boolean;
...
interface ParsedSummary {
  ...
  readonly episodeDuration: number | null;
...
export function parseSummary(raw: unknown): ParsedSummary {
  ...
  return {
    ...
    frameCount: toNumber(record.frameCount),
    episodeDuration: toNumber(record.episodeDuration),
    maxDrawCalls,
...
export function createPerformanceBudgetReport(
  ...
  if (summaryRel) {
    ...
    if (existsSync(absolute)) {
      ...
      // Encode size + bitrate from the REAL render summary (the live-3D path is
      // now the canonical render; the standard manifest is the deleted SVG path).
      metrics.push(
        metricLte(
          "summary-total-encoded-bytes",
          "Live render encoded video size",
          "render-live-summary",
          summary.videoBytes,
          MAX_TOTAL_ENCODED_BYTES,
          "bytes"
      ...
      const summaryBytesPerSecond =
        summary.videoBytes !== null && summary.episodeDuration && summary.episodeDuration > 0
          ? Math.round(summary.videoBytes / summary.episodeDuration)
          : null;
      metrics.push(
        metricLte(
          "summary-encoded-bytes-per-second",
          "Live render encoded bitrate",
          "render-live-summary",
      ...
      // Encoded frame count must match duration*fps within tolerance (real timing).
      const expectedSummaryFrames =
        summary.episodeDuration !== null && summary.frameRate !== null
          ? summary.episodeDuration * summary.frameRate
          : null;
      const summaryFrameDelta =
        expectedSummaryFrames !== null && summary.frameCount !== null
          ? Math.abs(summary.frameCount - expectedSummaryFrames)
          : null;
      const summaryFrameTolerance =
        expectedSummaryFrames !== null
          ? Math.max(1, Math.ceil(expectedSummaryFrames * FRAME_COUNT_TOLERANCE_FRACTION))
          : 0;
      const summaryFrameOk = summaryFrameDelta !== null && summaryFrameDelta <= summaryFrameTolerance;
      metrics.push({
        id: "summary-frame-count-matches-duration",
        label: "Live render frame count vs duration*fps",
        source: "render-live-summary",
        value: summaryFrameDelta,
        budget: summaryFrameTolerance,
        comparison: "approx==",
        unit: "frames-delta",
        ok: summaryFrameOk,
        detail:
          summaryFrameDelta === null
            ? "Cannot verify frame count: episodeDuration, frameRate, or frameCount missing from summary."
            : summaryFrameOk
              ? `Rendered frames ${summary.frameCount} match expected ${expectedSummaryFrames} (delta ${summaryFrameDelta} <= ${summaryFrameTolerance}).`
              : `Rendered frames ${summary.frameCount} differ from expected ${expectedSummaryFrames} by ${summaryFrameDelta} (> ${summaryFrameTolerance}).`
      });
    }
  }
  ...
  return {
    schema: "animation-studio-performance-budget/v1",
...
if (
  currentScript.endsWith("tools/animation-studio-performance-budget-gate/index.ts") ||
  currentScript.endsWith("tools/animation-studio-performance-budget-gate/index.js")
) {
