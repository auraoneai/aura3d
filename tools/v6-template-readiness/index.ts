...
const reportPath = resolve("tests/reports/v6-template-readiness.json");
const packDir = resolve("tests/reports/v6-template-pack");
const previewRoot = resolve("tests/reports/v6-template-external-builds");
...
const templateReports = templates.map((template) => {
  ...
  const runtimePath = resolve(`tests/reports/v6-templates/${template}.json`);
  const runtime = existsSync(runtimePath) ? JSON.parse(readFileSync(runtimePath, "utf8")) as { runtime?: Obj } : {};
  const screenshotPath = resolve(`tests/reports/v6-templates/${template}.png`);
  return {
    template,
    ...
    publicImport: main.includes("from \"@galileo3d/engine/workflows/v6\"") && !main.includes("workspace:") && !main.includes("/packages/"),
    ...
    assetManifest: manifest.fetchInstructions?.includes("/fixtures/v6") === true && (manifest.assets?.length ?? 0) >= 2 && manifest.assets?.every((asset) => typeof asset.sha256 === "string"),
...
const tempRoot = mkdtempSync(join(tmpdir(), "g3d-v6-template-build-"));
...
const report = {
  schema: "g3d-v6-template-readiness/v1",
