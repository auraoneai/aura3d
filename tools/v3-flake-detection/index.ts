...
import { baseReport, hashText, readJson, writeJson } from "../v3-reporting/index.js";
...
const targets: readonly FlakeTarget[] = [
  {
    ...
    reportPath: "tests/reports/v3-example-screenshots/manifest.json",
  ...
  },
  {
    ...
    command: ["pnpm", "--silent", "verify:v3-benchmarks"],
    reportPath: "tests/reports/v3-engine-comparison.json",
    stableSignature: (report) => {
...
export function runV3FlakeDetection(iterations = 2): number {
  ...
  const report = {
    ...baseReport(root, {
      ...
      runIdPrefix: "v3-flake-detection",
      sourceFiles: [
        "package.json",
        "tools/v3-flake-detection/index.ts",
  ...
  writeJson(root, "tests/reports/v3-flake-detection.json", report);
...
function collectScreenshotPaths(): readonly string[] {
  const manifest = readJson(root, "tests/reports/v3-example-screenshots/manifest.json");
