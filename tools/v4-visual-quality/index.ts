...
const visualReviewPath = "tests/reports/v4-example-visual-review.json";
const requiredScreenshots: readonly RequiredScreenshot[] = [
  ...
  { id: "product-visual-parity-galileo", path: "tests/reports/v4-product-visual-parity/galileo-product.png", source: "standalone", minImageColorBuckets: 24, minMeanLuma: 80, maxDarkPixelRatio: 0.2, maxDominantBucketRatio: 0.62, minEdgePixelRatio: 0.025, maxFlatPixelRatio: 0.72, minLocalContrastRatio: 0.09 },
  ...
  { id: "asset-viewer", path: "tests/reports/v4-example-screenshots/asset-viewer.png", source: "asset-viewer" },
  ...
  { id: "shadow-lab", path: "tests/reports/v4-example-screenshots/shadow-lab.png", source: "rendering", validationName: "shadow-lab-v4-preset", minImageColorBuckets: 8, minMeanLuma: 42, maxDarkPixelRatio: 0.48, maxDominantBucketRatio: 0.72, minEdgePixelRatio: 0.04, maxFlatPixelRatio: 0.68, minLocalContrastRatio: 0.09 },
] as const;
...
export function createV4VisualQualityReport(root = defaultRoot) {
  ...
  const renderingReport = readJson(root, "tests/reports/v4-rendering.json");
  const assetViewerReport = readJson(root, "tests/reports/v4-asset-viewer-browser.json");
  ...
  const primaryAssetChecks = [
    ...
    })),
    {
      ...
      evidencePaths: ["tests/reports/v4-asset-viewer-browser.json", "tests/reports/v4-example-screenshots/asset-viewer.png"],
  ...
  const shadowChecks = [
    ...
    },
    {
      ...
      evidencePaths: ["tests/reports/v4-rendering.json", "tests/reports/v4-example-screenshots/shadow-lab.png"],
  ...
  const postprocessChecks = [
    {
      ...
      evidencePaths: ["tests/reports/v4-rendering.json", "tests/reports/v4-example-screenshots/postprocess-lab.png"],
    ...
    },
    {
      ...
      evidencePaths: ["tests/reports/v4-rendering.json", "tests/reports/v4-example-screenshots/material-showroom.png"],
  ...
  const checks = [
    ...
    ...postprocessChecks,
    {
      ...
      evidencePaths: ["tests/reports/v4-example-screenshots/manifest.json", "tests/reports/v4-example-screenshots/portfolio.png"],
    ...
    },
    {
      ...
      evidencePaths: ["tests/reports/v4-example-screenshots/manifest.json", "examples/portfolio/main.ts"],
    ...
    },
    {
      ...
      passed: true,
      evidencePaths: ["tools/v4-visual-quality/index.ts"],
  ...
  const report = {
    ...baseReport(root, {
      ...
      command: "pnpm verify:v4-visual-quality",
      runIdPrefix: "v4-visual-quality",
      sourceFiles: [
  ...
  writeJson(root, "tests/reports/v4-visual-quality.json", report);
  return report;
}
...
function validatePortfolioFreshness(root: string, manifest: Record<string, unknown> | null) {
  const manifestPath = `${root}/tests/reports/v4-example-screenshots/manifest.json`;
  const portfolioPath = `${root}/tests/reports/v4-example-screenshots/portfolio.png`;
  ...
  const hasCurrentCards = cards.length >= 3 && cards.every((card) => {
    ...
    return typeof card.screenshotPath === "string"
      && card.screenshotPath.includes("/tests/reports/v4-example-screenshots/")
      && (visualGate?.status === "passed-v4-screenshot-audit" || visualGate?.status === "blocked-v4-visual-quality")
      && visualGate?.reportPath === "/tests/reports/v4-example-screenshots/manifest.json"
      && visualGate?.visualQualityReportPath === "/tests/reports/v4-visual-quality.json";
  });
...
function validatePortfolioBlockedCards(manifest: Record<string, unknown> | null) {
  ...
  const blockedCardIds = cards
    .filter((card) => {
      if (!isRecord(card)) return false;
      const visualGate = isRecord(card.visualGate) ? card.visualGate : null;
      return visualGate?.status === "blocked-v4-visual-quality";
    })
