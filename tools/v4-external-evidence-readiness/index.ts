...
import { validatePublicDemoDeploymentSmokeEvidence } from "../v4-production-readiness/index.js";
...
const reportPath = "tests/reports/v4-external-evidence-readiness.json";
...
export function createV4ExternalEvidenceReadinessReport(root = process.cwd()): V4ExternalEvidenceReadinessReport {
  ...
  const githubExternal = readJson(root, "tests/reports/v4-github-external-readiness.json");
  ...
  const report = {
    ...baseReport(root, {
      ok: true,
      command: "pnpm audit:v4-external-evidence-readiness",
      runIdPrefix: "v4-external-evidence-readiness",
      sourceFiles,
      violations,
      blockedClaims: [
...
function unityUnrealArtifactChecklist(
  ...
  const smokeReportPath = `tests/reports/v4-${engine}-editor-cli-smoke.json`;
  const renderWorkflowReportPath = `tests/reports/v4-${engine}-baseline-render.json`;
  const assetImportWorkflowReportPath = `tests/reports/v4-${engine}-asset-import-workflow.json`;
  ...
  return [
    ...
    },
    {
      ...
      expectedRunnerEvidencePath: typeof assetImportWorkflowPlan?.runnerEvidencePath === "string" ? assetImportWorkflowPlan.runnerEvidencePath : `tests/reports/v4-${engine}-asset-import-workflow.evidence.json`,
      command: typeof assetImportWorkflowPlan?.reportCommand === "string"
        ? assetImportWorkflowPlan.reportCommand
        : `node fixtures/external-engine-baselines/v4/write-asset-import-workflow-report.mjs ${engine} tests/reports/v4-${engine}-asset-import-workflow.evidence.json ${assetImportWorkflowReportPath}`,
      validationCommands: [
...
function externalBaselineCiWorkflowArtifact(root: string): ExternalEvidenceArtifact {
  ...
  return {
    ...
    validationCommands: ["pnpm audit:v4-external-evidence-readiness", "pnpm audit:v4-completion"],
    localEvidence: [
...
function publicDeploymentCiWorkflowArtifact(root: string): ExternalEvidenceArtifact {
  ...
  return {
    ...
    validationCommands: ["pnpm audit:v4-production-readiness", "pnpm audit:v4-external-evidence-readiness"],
    localEvidence: [
...
function githubRemoteExternalReadinessArea(githubExternal: Record<string, unknown> | null): ExternalEvidenceArea {
  ...
  return {
    ...
    blockers: githubExternal
      ? blockers
      : ["tests/reports/v4-github-external-readiness.json is missing; run pnpm audit:v4-github-external-readiness."],
  };
}

function externalBaselineWorkflowValidation(root: string): { readonly path: ".github/workflows/v4-external-engine-baselines.yml"; readonly ready: boolean; readonly blockers: readonly string[] } {
  const path = ".github/workflows/v4-external-engine-baselines.yml" as const;
  ...
  const requiredFragments = [
    ...
    ["Unity CLI smoke", "run-editor-cli-smoke.mjs unity tests/reports/v4-unity-editor-cli-smoke.json"],
    ["Unreal CLI smoke", "run-editor-cli-smoke.mjs unreal tests/reports/v4-unreal-editor-cli-smoke.json"],
    ...
    ["external evidence runbook upload", "tests/reports/v4-external-evidence-missing-artifacts.md"],
    ["completion runbook upload", "tests/reports/v4-completion-audit-runbook.md"],
    ["artifact upload", "actions/upload-artifact@v4"],
    ["Unity editor smoke report upload", "tests/reports/v4-unity-editor-cli-smoke.json"],
    ["Unity render workflow report upload", "tests/reports/v4-unity-baseline-render.json"],
    ["Unity product baseline report upload", "tests/reports/v4-unity-product-visual-baseline.json"],
    ["Unity PBR baseline report upload", "tests/reports/v4-unity-pbr-visual-baseline.json"],
    ["Unity shadow baseline report upload", "tests/reports/v4-unity-shadow-visual-baseline.json"],
    ["Unity HDR baseline report upload", "tests/reports/v4-unity-hdr-render-target-baseline.json"],
    ["Unity postprocess baseline report upload", "tests/reports/v4-unity-postprocess-suite-baseline.json"],
    ["Unreal editor smoke report upload", "tests/reports/v4-unreal-editor-cli-smoke.json"],
    ["Unreal render workflow report upload", "tests/reports/v4-unreal-baseline-render.json"],
    ["Unreal product baseline report upload", "tests/reports/v4-unreal-product-visual-baseline.json"],
    ["Unreal PBR baseline report upload", "tests/reports/v4-unreal-pbr-visual-baseline.json"],
    ["Unreal shadow baseline report upload", "tests/reports/v4-unreal-shadow-visual-baseline.json"],
    ["Unreal HDR baseline report upload", "tests/reports/v4-unreal-hdr-render-target-baseline.json"],
    ["Unreal postprocess baseline report upload", "tests/reports/v4-unreal-postprocess-suite-baseline.json"],
    ["product screenshot artifact upload", "tests/reports/v4-product-visual/**"],
    ["PBR screenshot artifact upload", "tests/reports/v4-pbr-visual/**"],
    ["shadow screenshot artifact upload", "tests/reports/v4-shadow-visual/**"],
    ["HDR screenshot artifact upload", "tests/reports/v4-hdr-render-target/**"],
    ["postprocess screenshot artifact upload", "tests/reports/v4-postprocess-suite/**"],
    ["product visual final audit upload", "tests/reports/v4-product-visual-parity.json"],
    ["PBR/glTF final audit upload", "tests/reports/v4-pbr-gltf-readiness.json"],
    ["Unity/Unreal final audit upload", "tests/reports/v4-unity-unreal-parity.json"],
    ["production final audit upload", "tests/reports/v4-production-readiness.json"],
    ["broad parity final audit upload", "tests/reports/v4-broad-parity-readiness.json"],
    ["freshness final audit upload", "tests/reports/v4-report-freshness.json"],
  ] as const;
...
function publicDeploymentWorkflowValidation(root: string): { readonly path: ".github/workflows/v4-public-demo-deploy.yml"; readonly ready: boolean; readonly blockers: readonly string[] } {
  ...
  const requiredFragments = [
    ...
    ["production readiness audit", "pnpm audit:v4-production-readiness"],
    ["external evidence readiness audit", "pnpm audit:v4-external-evidence-readiness"],
    ["broad parity audit", "pnpm audit:v4-broad-parity"],
    ["completion audit", "pnpm audit:v4-completion"],
    ["freshness verification", "pnpm verify:v4-report-freshness"],
    ["public deployment smoke report upload", "tests/reports/public-demo-deployment-smoke.json"],
    ["production readiness report upload", "tests/reports/v4-production-readiness.json"],
    ["external evidence readiness report upload", "tests/reports/v4-external-evidence-readiness.json"],
    ["external evidence missing-artifacts runbook upload", "tests/reports/v4-external-evidence-missing-artifacts.md"],
    ["completion audit runbook upload", "tests/reports/v4-completion-audit-runbook.md"],
  ] as const;
  const blockers = requiredFragments.flatMap(([label, fragment]) => workflow.includes(fragment) ? [] : [`${path} is missing ${label}: ${fragment}`]);
...
function finalAuditArtifactChecklist(
  ...
  const audits = [
    { id: "unity-unreal-parity", path: "tests/reports/v4-unity-unreal-parity.json", ready: unityUnreal?.unityParity === true && unityUnreal.unrealParity === true && unityUnreal.replacement === true, command: "pnpm audit:v4-unity-unreal-parity" },
    { id: "production-readiness", path: "tests/reports/v4-production-readiness.json", ready: production?.productionReady === true, command: "pnpm audit:v4-production-readiness" },
    { id: "pbr-gltf-readiness", path: "tests/reports/v4-pbr-gltf-readiness.json", ready: pbrGltf?.pbrParity === true && pbrGltf.gltfParity === true, command: "pnpm audit:v4-pbr-gltf-readiness" },
  ] as const;
...
function commandsForArea(areaId: string): readonly string[] {
  switch (areaId) {
    case "external-baseline-kit":
      return ["pnpm verify:v4-external-engine-baselines"];
    case "external-baseline-ci-workflow":
      return [
        ...
        "gh workflow run v4-external-engine-baselines.yml -f engine=all",
    ...
    default:
      return ["pnpm audit:v4-external-evidence-readiness"];
  }
}
...
function externalBaselineKitArea(report: Record<string, unknown> | null): ExternalEvidenceArea {
  ...
  return {
    ...
    evidencePaths: ["tests/reports/v4-external-engine-baselines.json", "fixtures/external-engine-baselines/v4"],
    localEvidence: [
...
function externalEvidenceLocalSummaries(report: Record<string, unknown> | null): ExternalEvidenceLocalSummaries {
  ...
  return {
    ...
    ] : ["No local editor workflow summary is present in tests/reports/v4-unity-unreal-parity.json."],
    ...
    ] : ["No local asset import preflight summary is present in tests/reports/v4-unity-unreal-parity.json."],
    runtime: runtime ? [
    ...
    ] : ["No local runtime summary is present in tests/reports/v4-unity-unreal-parity.json."],
    rendering: rendering ? [
    ...
    ] : ["No local rendering summary is present in tests/reports/v4-unity-unreal-parity.json."],
    ...
    ] : ["No local deployment summary is present in tests/reports/v4-unity-unreal-parity.json."],
  };
}
...
function blenderSameCorpusArea(report: Record<string, unknown> | null): ExternalEvidenceArea {
  ...
  return {
    ...
    evidencePaths: ["tests/reports/v4-pbr-gltf-readiness.json", "tests/reports/blender-export-validation.json", "tests/reports/blender-same-corpus-export.json"],
    localEvidence: [
