...
const packDir = resolve("tests/reports/v6-package");
const reportPath = resolve("tests/reports/v6-package-smoke.json");
...
const tempRoot = mkdtempSync(join(tmpdir(), "g3d-v6-package-smoke-"));
...
try {
  ...
  const stdout = execFileSync("node", ["--input-type=module", "-e", `
    const workflows = await import("@galileo3d/engine/workflows/v6");
  ...
  const report = {
    schema: "g3d-v6-package-smoke/v1",
