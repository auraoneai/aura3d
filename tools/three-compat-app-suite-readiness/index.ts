...
const checks = [
  ...
  { name: "public-imports", pass: internalImports.length === 0, detail: internalImports.join(", ") || "apps use public Three.js compatibility APIs" }
];
const pass = checks.every((item) => item.pass);
const report = { schema: "a3d-three-compat-app-suite-readiness", generatedAt: new Date().toISOString(), pass, apps, checks };
...
console.log(`Three.js compatibility app suite readiness passed: ${apps.length} apps.`);
