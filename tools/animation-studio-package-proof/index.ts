...
export interface AnimationStudioPackageArtifact {
  readonly id: string;
...
export interface AnimationStudioPackageProofReport {
  readonly schema: "animation-studio-package-proof/v1";
  readonly ok: boolean;
  readonly generatedAt: string;
  readonly packageDir: string;
  readonly artifacts: readonly AnimationStudioPackageArtifact[];
  readonly sourceIntegrity: AnimationStudioSourceIntegrityReport;
  readonly blockers: readonly string[];
}

export interface AnimationStudioSourceIntegrityReport {
  readonly ok: boolean;
  readonly scannedJsonFiles: readonly string[];
  readonly forbiddenEvidence: readonly AnimationStudioForbiddenEvidence[];
}

export interface AnimationStudioForbiddenEvidence {
  readonly path: string;
  readonly reason: "sourceOnly" | "notTrue3D" | "image-puppet";
  readonly detail: string;
}

export interface AnimationStudioPackageProofOptions {
  readonly packageDir?: string;
  readonly out?: string;
  readonly generatedAt?: string;
...
const defaultOut = "tests/reports/aura3d11/animation-package.json";

interface RequiredAnimationStudioArtifact {
  readonly id: string;
  readonly path: string;
...
const requiredArtifacts: readonly RequiredAnimationStudioArtifact[] = [
...
export function createAnimationStudioPackageProofReport(root = process.cwd(), options: AnimationStudioPackageProofOptions = {}): AnimationStudioPackageProofReport {
  ...
  const artifacts = requiredArtifacts.map((artifact): AnimationStudioPackageArtifact => {
  ...
  return {
    schema: "animation-studio-package-proof/v1",
...
export function inspectSourceIntegrity(root: string, jsonPaths: readonly string[]): AnimationStudioSourceIntegrityReport {
  const scannedJsonFiles: string[] = [];
  const forbiddenEvidence: AnimationStudioForbiddenEvidence[] = [];
...
export function collectForbiddenEvidence(value: unknown, path: string, output: AnimationStudioForbiddenEvidence[], keyPath = "$"): void {
  ...
  for (const [key, entry] of Object.entries(record)) {
    if (typeof entry === "string" && /image-puppet|animation-image-puppet-animation|view=image-puppet/i.test(entry)) {
      output.push({
        path,
        reason: "image-puppet",
...
export function writeAnimationStudioPackageProofReport(root: string, report: AnimationStudioPackageProofReport, out = defaultOut): void {
...
if (currentScript.endsWith("tools/animation-studio-package-proof/index.ts") || currentScript.endsWith("tools/animation-studio-package-proof/index.js")) {
  ...
  writeAnimationStudioPackageProofReport(root, report, typeof args.out === "string" ? args.out : defaultOut);
  if (!report.ok) {
