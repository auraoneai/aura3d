...
const releaseReadiness = json("tests/reports/external-parity-release-readiness.json");
const progress = json("tests/reports/external-parity-progress.json");
const claimRegistry = json("tests/reports/external-parity-claim-registry.json");
const externalConsumer = json("tests/reports/external-parity-external-consumer.json");
const visualQuality = json("tests/reports/external-parity-visual-quality.json");
...
mkdirSync(dirname(resolve("tests/reports/external-parity-completion-audit.json")), { recursive: true });
writeFileSync(resolve("tests/reports/external-parity-completion-audit.json"), `${JSON.stringify(report, null, 2)}\n`);
if (!pass) {
