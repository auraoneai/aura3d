...
import { baseReport, listFiles, readJson, writeJson } from "../v3-reporting/index.js";
...
const reportPath = "tests/reports/v3-current-capability.json";
...
export function createV3CurrentCapabilityReport(root = process.cwd()): V3CurrentCapabilityReport {
  ...
  const taskAssignmentReport = existsSync(`${root}/tests/reports/v3-task-assignments.json`)
    ? "tests/reports/v3-task-assignments.json"
    : null;
  const claimGates = readJson(root, "tests/reports/v3-claim-gates.json");
  const exampleTruth = readJson(root, "tests/reports/v3-example-truth-audit.json");
  const screenshots = readJson(root, "tests/reports/v3-example-screenshots/manifest.json");
  const rendering = readJson(root, "tests/reports/v3-rendering.json");
  const assets = readJson(root, "tests/reports/v3-asset-corpus.json");
  const editor = readJson(root, "tests/reports/v3-editor-authoring.json");
  const runtime = readJson(root, "tests/reports/v3-runtime-systems.json");
  const comparison = readJson(root, "tests/reports/v3-engine-comparison.json");
  const aggregateVerify = readJson(root, "tests/reports/v3-verify.json");
  ...
  const gates: V3GateStatus[] = [
    ...
    },
    {
      ...
      blockers: [
        ...
        ...(aggregateVerify?.ok === true ? [] : ["aggregate verify:v3 has not been proven by tests/reports/v3-verify.json"]),
      ],
    },
  ];
  ...
  const base = baseReport(root, {
    ...
    runIdPrefix: "v3-current-capability",
    sourceFiles: [...docs, "tests/reports/v3-task-assignments.json", "tests/reports/v3-claim-gates.json", "tests/reports/v3-example-truth-audit.json", "tests/reports/v3-verify.json"],
    violations,
  });
  return {
    ...base,
