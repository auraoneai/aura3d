...
const reportPath = resolve("tests/reports/v6-workflows-readiness.json");
const assetManifest = JSON.parse(readFileSync(resolve("fixtures/v6/assets/manifest.json"), "utf8")) as {
  assets?: {
    id: string;
    localPath?: string;
    sourceUri?: string;
...
const galleryManifest = JSON.parse(readFileSync(resolve("tests/reports/v6-gallery/manifest.json"), "utf8")) as {
  entries?: {
    screenshot: string;
...
const checks = [
  ...
  { id: "gallery-manifest-exists", pass: existsSync(resolve("tests/reports/v6-gallery/manifest.json")), detail: "tests/reports/v6-gallery/manifest.json" }
];
const report = {
  schema: "g3d-v6-workflows-readiness/v1",
