...
const report = {
  ...
  sourceFiles,
  fixtureChecks,
  browserManifestPath: "tests/reports/foundation-assets/manifest.json",
  browserManifestExists: existsSync(browserManifestPath),
  captureChecks
};

mkdirSync(resolve("tests/reports"), { recursive: true });
writeFileSync(resolve("tests/reports/foundation-assets-readiness.json"), `${JSON.stringify(report, null, 2)}\n`);
