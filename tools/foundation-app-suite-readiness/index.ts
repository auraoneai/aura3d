...
const requiredSourceFiles = [
  "apps/legacy-common/src/WorkflowWorkbench.ts",
  ...requiredApps.flatMap((app) => [`apps/${app}/index.html`, `apps/${app}/src/main.ts`]),
  "tests/browser/foundation-app-suite.spec.ts"
] as const;
const manifestPath = resolve("tests/reports/foundation-app-suite/manifest.json");
...
const report = {
  ...
  sourceFiles,
  workflowUsage,
  browserManifestPath: "tests/reports/foundation-app-suite/manifest.json",
  browserManifestExists: existsSync(manifestPath),
  captureChecks
};

mkdirSync(resolve("tests/reports"), { recursive: true });
writeFileSync(resolve("tests/reports/foundation-app-suite-readiness.json"), `${JSON.stringify(report, null, 2)}\n`);
