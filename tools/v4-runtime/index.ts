...
import { baseReport, blockedV4Claims, isRecord, readJson, sourceFilesFromReport, writeJson } from "../v4-reporting/index.js";

const root = process.cwd();
const reportPath = "tests/reports/v4-runtime.json";
...
const screenshot = Array.isArray(existing?.screenshots) && typeof existing.screenshots[0] === "string"
  ...
  : "tests/reports/v4-example-screenshots/game-slice-runtime.png";

const checks = [
  check("v4-runtime-report-produced-by-runtime-agent", existing?.ok === true, [reportPath], "V4 runtime systems report is not yet passing."),
...
const audit = {
  ...baseReport(root, {
    ...
    command: "pnpm verify:v4-runtime",
    runIdPrefix: "v4-runtime",
    sourceFiles: [
      "tools/v4-runtime/index.ts",
...
if (isMain) {
  if (existing?.ok === true) {
    writeJson(root, reportPath, {
      ...
      normalizedBy: "tools/v4-runtime/index.ts",
    });
  }
  writeJson(root, "tests/reports/v4-runtime-audit.json", audit);
  console.log(JSON.stringify({ ok: audit.ok, report: "tests/reports/v4-runtime-audit.json", checks: checks.length }, null, 2));
