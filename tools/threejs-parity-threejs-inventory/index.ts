...
const HISTORICAL_THREE_VERSION = "0.165.0";
const historicalScopeLine = `Historical scope: generated from the frozen \`three@${HISTORICAL_THREE_VERSION}\` comparison inventory. This is not a current r185 market verdict.`;
...
const inventory: readonly InventoryItem[] = [
  ...
  item("misc_controls_trackball", "controls", "medium", "/apps/controls-trackball/", "matched", ["F", "K"], ["@aura3d/controls exports TrackballControls and the CurrentRoutes trackball route proves rotate, pan, dolly, and roll state driving a A3D-rendered camera example. TrackballControls now includes bounded damping plus keyboard pan, roll, and dolly handling through public control methods, with unit coverage for the damped update path and browser coverage proving the rendered interaction route stays live beyond a one-click smoke test."], [], ["tests/unit/controls/three-compat-controls.test.ts", "tests/browser/current-routes-parity-evidence.spec.ts"], ["tests/reports/current-routes/controls/trackball.png"]),
  ...
  item("webxr_vr_ballshooter", "webxr", "low", "/apps/webxr-interactions/", "matched", ["A", "F", "K"], ["@aura3d/input exports WebXRSessionController and the CurrentRoutes WebXR interactions route starts injected immersive-vr, samples controller trigger input, target-ray poses, grip poses, and haptic actuator availability, then maps trigger presses to ball-shooter actions. The scoped browser route and unit tests prove the public controller/session path without claiming untested headset hardware coverage; its Canvas2D image is an explanatory UI preview, not Aura3D rendering evidence."], [], ["tests/unit/input/webxr-session-controller.test.ts", "tests/browser/current-routes-parity-evidence.spec.ts"], ["tests/reports/current-routes/webxr/interactions.png"]),
  item("webxr_vr_dragging", "webxr", "low", "/apps/webxr-interactions/", "matched", ["F", "K"], ["The WebXR interactions route samples squeeze/trigger controller state through WebXRSessionController, including target-ray and grip matrices, and maps squeeze state to dragged-object counts. Unit coverage verifies grip/target pose sampling and `pulseHaptics()` actuator dispatch, while browser evidence proves the injected interaction workflow; its Canvas2D image is explanatory UI, not rendering evidence."], [], ["tests/unit/input/webxr-session-controller.test.ts", "tests/browser/current-routes-parity-evidence.spec.ts"], ["tests/reports/current-routes/webxr/interactions.png"]),
  item("webxr_ar_cones", "webxr", "low", "/apps/webxr-interactions/", "matched", ["A", "K"], ["The WebXR interactions route starts injected immersive-ar, samples hit-test results through WebXRSessionController, and maps hit-test matrices to previewed AR cone placements. Unit and browser coverage prove the scoped injected AR hit-test sample path without treating its Canvas2D preview as Aura3D rendering evidence or claiming physical-device camera/light-estimation coverage."], [], ["tests/unit/input/webxr-session-controller.test.ts", "tests/browser/current-routes-parity-evidence.spec.ts"], ["tests/reports/current-routes/webxr/interactions.png"])
];
...
const report = {
  ...
  historicalThreeVersion: HISTORICAL_THREE_VERSION,
...
function renderStatusDoc(input: typeof report): string {
  return [
    ...
    "- Animation route suite now requires canvas frame-difference motion evidence for animation/skinning routes: `tests/browser/current-routes-route-health.spec.ts`",
    ...
    "- WebGPU render-target route proves public WebGPU render-target draw/readback/present/disposal behavior through package code: `apps/wow-webgpu-render-target/src/main.ts`",
    "- WebGPU compute route proves public WebGPUParticleBackend storage-buffer compute dispatch and CPU-reference readback parity: `apps/wow-webgpu-compute-particles/src/main.ts`",
    "- WebGPU PBR asset route proves public PBR and textured PBR material rendering through the A3D WebGPU backend: `apps/wow-webgpu-pbr-asset/src/main.ts`",
    "- WebGPU instancing route proves repeated geometry rendering through the A3D WebGPU backend: `apps/wow-webgpu-instancing/src/main.ts`",
