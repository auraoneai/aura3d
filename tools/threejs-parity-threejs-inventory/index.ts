...
const inventory: readonly InventoryItem[] = [
  ...
  item("webgl_effects_anaglyph", "effects", "medium", "/apps/stereo-effects/", "matched", ["H", "K"], ["The V8 stereo effects route drives both eyes through public createStereoCameraRig and public createStereoEffectPlan, and @aura3d/rendering now exports createAnaglyphPixelComposite for renderer-owned red/cyan channel composition over real left/right RGBA frame buffers. Unit coverage verifies the public plan and pixel-composite path alongside the parallax-barrier compositor."], [], ["tests/unit/rendering/stereo-effects.test.ts", "tests/browser/current-routes-animation-examples.spec.ts"], ["tests/reports/current-routes/effects/stereo-effects.png"]),
  ...
  item("webxr_vr_ballshooter", "webxr", "low", "/apps/webxr-interactions/", "matched", ["A", "F", "K"], ["@aura3d/input exports WebXRSessionController and the V8 WebXR interactions route starts injected immersive-vr, samples controller trigger input, target-ray poses, grip poses, and haptic actuator availability, then maps trigger presses to ball-shooter actions. The scoped browser route and unit tests prove the public controller/session path without claiming untested headset hardware coverage."], [], ["tests/unit/input/webxr-session-controller.test.ts", "tests/browser/current-routes-animation-examples.spec.ts"], ["tests/reports/current-routes/webxr/interactions.png"]),
...
const report = {
  schema: "a3d-threejs-parity-threejs-inventory/v1",
