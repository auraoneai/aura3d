...
const requiredFiles = [
  ...
  "examples/product-configurator-v4/main.ts",
  "examples/product-configurator-v4/ProductConfiguratorV4.ts",
  ...
  "tests/reports/v4-product-configurator-browser.json"
] as const;
...
const shared = readText("examples/product-configurator-v4/ProductConfiguratorV4.ts");
check(
...
check(
  ...
  readText("examples/product-configurator-v4/main.ts").includes("mountProductConfiguratorV4(\"product-configurator-v4\")") &&
    readText("apps/product-studio-pro/src/main.ts").includes("ProductConfiguratorV4") &&
    !readText("apps/product-studio-pro/src/main.ts").includes("product-configurator-v4/main"),
  "Example and app must share a side-effect-free module instead of importing the example entry."
);

const galileoBenchmark = readText("benchmarks/v4/galileo/product-configurator.ts");
const threeBenchmark = readText("benchmarks/v4/threejs/product-configurator.ts");
check(
  "benchmark-sources",
...
const browser = readJson("tests/reports/v4-product-configurator-browser.json");
...
const expectedScreenshots = [
  ...
  "tests/reports/v4-gallery/product/product-configurator-v4-variant.png",
  "tests/reports/v4-gallery/product/product-studio-pro.png"
];
check(
  "browser-report",
  browser?.ok === true &&
    statePasses(example, "product-configurator-v4") &&
    statePasses(variant, "product-configurator-v4") &&
...
const report = {
  schema: "g3d-v4-product-readiness/v1",
...
const reportPath = resolve("tests/reports/v4-product-readiness.json");
