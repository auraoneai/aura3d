...
function publicDeploymentRunbookSource(report: PublicDemoDeploymentSmokeReport): string {
  return `# External parity Public Demo Deployment Runbook
...
Do not claim production readiness until this runbook shows \`Deployment ready: yes\`, \`${reportPath}\` has \`ok: true\`, \`pnpm audit:external-parity-production-readiness\` has \`productionReady: true\`, and \`pnpm audit:external-parity-completion\` marks the production-readiness criterion achieved.
`;
}
