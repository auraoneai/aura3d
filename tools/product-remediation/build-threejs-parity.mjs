...
const ROWS = [
  ...
  { category: "physics", capability: "vehicle dynamics", expected: "Rapier vehicle controller, hand-tuned", symbols: ["createVehicleChassis", "vehicleChassisSpecFromBounds"], integrated: true, claim: "parity-unproven", notes: "Downgraded from exceed 2026-08-04, and the stated reason is now RESOLVED but the row stays parity-unproven. The original downgrade said the VehicleSurface input was an analytic flat plane (TRACK_SURFACE_Y minus a shoulder ramp). WS-4.1 replaced it: the geometry extractor emits topology.drivableMesh (2,902 triangles, 529 distinct elevations against the centreline's 13), the scene binding exposes vehicleSurface() over a real mesh query, and all five route-local surface constants are deleted. The WS-7.1 penetration gate named in the old note now passes, and turbo-drift-real-circuit-contact.test.ts proves per-wheel independent heights against the committed circuit mesh. What still blocks an upgrade is different and narrower: game.racing integrates its own kinematic motion rather than the force-based tyre model in packages/physics/src/VehicleMotion.ts, so heading still comes from steering input. See GameEngine-PRD WS-3.8, attempted and reverted with the finding recorded. Restore an exceed claim only when the kit consumes the shared model.", evidence: ["tests/reports/turbo-vehicle-grounding"] },
  { category: "physics", capability: "vehicle AI driving", expected: "no standard solution; hand-written per project", symbols: ["createVehicleDriverAi"], integrated: true, claim: "parity-unproven", notes: "Downgraded from exceed 2026-08-04. The driver AI is sound in isolation (look-ahead line, curvature corner speeds, recovery, deterministic per seed). The original reason — that it drives a vehicle whose contact model is an analytic plane — no longer holds: WS-4.1 put the vehicle on a real mesh surface query and the penetration gate passes. It stays parity-unproven because the vehicle it drives still uses the kit's kinematic motion rather than the shared force-based tyre model, so understeer and slip are not represented in what the AI is steering. Tied to GameEngine-PRD WS-3.8.", evidence: ["tests/reports/turbo-vehicle-grounding"] },
