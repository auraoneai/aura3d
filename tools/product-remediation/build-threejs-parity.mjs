...
const NAMESPACE_ALIASES = {
  ...
  RapierCharacterControllerHandle: ["characterController", "RapierCharacterControllerHandle"],
...
{ category: "core-rendering", capability: "context loss recovery", expected: "webglcontextlost handling by hand", symbols: ["onDeviceLost", "onDeviceRestored"], integrated: true, claim: "parity", notes: "Closed for the bounded root path by WS-2.6 and WS-3.9. app.onDeviceLost(), app.onDeviceRestored() and app.deviceLost() surface WebGL context loss through the root API. The retained route pauses on loss, explicitly remounts the same scene after restoration, waits for a new production runtime, and proves identical restored pixels. Subscriptions registered before renderer mount attach on arrival and unsubscribe is listener-specific. Not claimed: universal transparent recreation of arbitrary caller-owned GPU resources; recovery is app-driven through the public remount API.", evidence: ["tests/browser/context-loss-recovery.spec.ts", "tests/reports/public-renderer-normal-path/context-lifecycle.json"] },
  ...
  { category: "ecosystem-helpers", capability: "scene inspector", expected: "three-devtools or custom", symbols: ["createGameInspector", "collectAuraSceneEvidence"], integrated: true, claim: "parity" },
    /*
   * Stays a `gap` against troika-three-text / TextGeometry. Aura3D does ship a bounded
   * `text3D` extruded-glyph mesh and a DOM `labels.*` layer; neither is a general font stack.
   * How-to: docs/rendering/world-labels-and-text.md.
   */
  { category: "ecosystem-helpers", capability: "text rendering", expected: "troika-three-text or TextGeometry", symbols: ["TextGeometry"], integrated: false, claim: "gap", notes: "Aura3D does not ship troika-three-text or a general TextGeometry font stack. `text3D` is a bounded extruded bitmap-glyph mesh (A–Z, 0–9, space, dash, period). World labels are DOM UI — accessible, collision-avoiding, occlusion-aware — and are not lit 3D text. See docs/rendering/world-labels-and-text.md." },
  ...
  // --- Physics ---
  { category: "physics", capability: "rigid bodies", expected: "Rapier integration", symbols: ["PhysicsWorld", "RigidBody"], integrated: true, claim: "parity" },
  ...
  { category: "physics", capability: "physical character controller", expected: "Rapier KinematicCharacterController", symbols: ["RapierCharacterControllerHandle"], integrated: true, claim: "parity", evidence: ["tests/unit/physics-rapier/rapier-adapter.test.ts"] },
