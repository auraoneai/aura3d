...
const requiredFiles = [
  ...
  "tests/reports/v4-postprocess-suite-browser.json"
] as const;
const checks: Check[] = [];
...
const report = json("tests/reports/v4-postprocess-suite-browser.json");
...
const output = { schema: "g3d-v4-postprocess-readiness/v1", generatedAt: new Date().toISOString(), pass, checks };
mkdirSync(dirname(resolve("tests/reports/v4-postprocess-readiness.json")), { recursive: true });
writeFileSync(resolve("tests/reports/v4-postprocess-readiness.json"), `${JSON.stringify(output, null, 2)}\n`);
if (!pass) {
