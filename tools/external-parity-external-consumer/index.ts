...
const packageSmoke = readJson("tests/reports/external-parity-package-smoke.json");
const tarballPath = typeof packageSmoke?.tarballPath === "string" && existsSync(packageSmoke.tarballPath)
  ? packageSmoke.tarballPath
  : packFallback();
const reportDir = resolve("tests/reports/external-parity-external-consumer");
...
function packFallback(): string {
  const reportDir = resolve("tests/reports/external-parity-external-consumer-pack");
  mkdirSync(reportDir, { recursive: true });
