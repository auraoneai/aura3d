...
const requiredFiles = [
  ...
  "examples/character-viewer-v4/main.ts",
  "examples/character-viewer-v4/CharacterViewerV4.ts",
  "tests/browser/v4-character-viewer.spec.ts",
  "tests/reports/v4-character-viewer-browser.json"
] as const;
const checks: Check[] = [];
...
check("viewer-source", includesAll(text("examples/character-viewer-v4/CharacterViewerV4.ts"), ["timeline-scrub", "play-pause", "__G3D_V4_CHARACTER_VIEWER__", "real skinned glTF rendered animation parity"]), "Character viewer must expose timeline/play state and proof boundary.");
check("app-entry-no-example-side-effect", text("examples/character-viewer-v4/main.ts").includes("mountCharacterViewerV4(\"character-viewer-v4\")") && text("apps/animation-studio-pro/src/main.ts").includes("CharacterViewerV4") && !text("apps/animation-studio-pro/src/main.ts").includes("character-viewer-v4/main"), "Animation Studio Pro must import side-effect-free shared module.");

const browser = json("tests/reports/v4-character-viewer-browser.json");
...
const expectedScreenshots = ["tests/reports/v4-gallery/characters/character-viewer-v4.png", "tests/reports/v4-gallery/characters/character-viewer-v4-scrubbed.png", "tests/reports/v4-gallery/characters/animation-studio-pro.png"];
check("browser-report", browser?.ok === true && statePasses(example, "character-viewer-v4") && statePasses(scrubbed, "character-viewer-v4") && statePasses(app, "animation-studio-pro") && scrubbed.playing === false, "Browser report must prove example, scrubbed state, and app.");
...
const report = { schema: "g3d-v4-character-readiness/v1", generatedAt: new Date().toISOString(), pass, summary: pass ? "V4 Milestone 11 character product surface is ready. Real skinned glTF/Three.js animation parity remains required." : "V4 Milestone 11 character proof is incomplete.", checkedFiles: requiredFiles, checks };
mkdirSync(dirname(resolve("tests/reports/v4-character-readiness.json")), { recursive: true });
writeFileSync(resolve("tests/reports/v4-character-readiness.json"), `${JSON.stringify(report, null, 2)}\n`);
