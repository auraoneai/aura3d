...
const status = readIfExists("docs/project/v5-roadmap-status.md");
const progress = readIfExists("docs/project/v5-roadmap-progress.md");
const visualFailures = readIfExists("docs/project/v5-roadmap-visual-failures.md");
const v5 = readIfExists("docs/project/v5-roadmap-visual-engine-plan.md");
const requiredStatusPatterns = [
  /\bnot complete until\b[\s\S]*pnpm v5:release/i,
...
const missingPlanPhrases = requiredPlanPatterns.filter((pattern) => !pattern.test(v5)).map(String);
...
const completionAuditPasses = reportPasses("tests/reports/v5-completion-audit.json");
const progressDoesNotClaimComplete = !progressClaimsComplete || completionAuditPasses;

const report = {
  schema: "g3d-v5-truth/v1",
...
writeFileSync(resolve("tests/reports/v5-truth.json"), `${JSON.stringify(report, null, 2)}\n`);
