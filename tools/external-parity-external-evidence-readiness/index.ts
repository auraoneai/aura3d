...
interface ExternalEvidenceLocalPreflight {
  ...
  readonly publicDeployment: {
    readonly envName: "A3D_PUBLIC_DEMO_URL";
    readonly envSet: boolean;
    readonly value?: string;
    readonly durableHttpsCandidate: boolean;
    readonly command: "A3D_PUBLIC_DEMO_URL=https://... pnpm verify:public-demo-deployment";
    readonly blockers: readonly string[];
  };
  readonly canRunExternalEvidenceHere: boolean;
  readonly firstMissingCapability?: string;
...
interface ExternalEditorPreflight {
  ...
  readonly envName: "A3D_UNITY_EDITOR" | "A3D_UNREAL_EDITOR";
  readonly searchRootsEnvName: "A3D_UNITY_SEARCH_ROOTS" | "A3D_UNREAL_SEARCH_ROOTS";
  readonly envSet: boolean;
...
export function createV4ExternalEvidenceReadinessReport(root = process.cwd()): V4ExternalEvidenceReadinessReport {
  ...
  const commandPlan = readJson(root, "fixtures/external-engine-baselines/external-parity/external-baseline-command-plan.json");
...
function renderMissingArtifactRunbook(report: V4ExternalEvidenceReadinessReport): string {
  ...
  const lines = [
    ...
    "",
    "Do not fabricate these files, loosen thresholds, reuse Aura3D screenshots as Unity/Unreal screenshots, or mark parity complete until the validators accept real external evidence.",
    "",
    "## Summary",
    "",
    ...
    `- Public deployment URL env: ${report.localPreflight.publicDeployment.envSet ? "`A3D_PUBLIC_DEMO_URL` is set" : "`A3D_PUBLIC_DEMO_URL` is not set"}`,
...
function unityUnrealArtifactChecklist(
  ...
  const sceneArtifacts = sceneReports.map((path): ExternalEvidenceArtifact => {
    ...
    return {
      ...
      command: typeof capture?.reportCommand === "string" ? capture.reportCommand : `node fixtures/external-engine-baselines/external-parity/write-baseline-report.mjs ${engine} <baseline-kind> <screenshot-path> ${path}`,
      validationCommands: stringArray(capture?.validationCommands),
      localEvidence: [
        `${engine} ${typeof capture?.baselineKind === "string" ? capture.baselineKind : "scene"} descriptor, Aura3D reference, report writer command, and validation thresholds are prepared locally.`,
  ...
  return [
    {
      ...
      command: typeof renderWorkflowPlan?.cliSmokeCommand === "string"
        ...
        : `node fixtures/external-engine-baselines/external-parity/run-editor-cli-smoke.mjs ${engine} ${smokeReportPath}`,
      localEvidence: [
    ...
    ...sceneArtifacts,
    {
      ...
      command: typeof renderWorkflowPlan?.reportCommand === "string"
        ...
        : `node fixtures/external-engine-baselines/external-parity/write-render-workflow-report.mjs ${engine} ${renderWorkflowReportPath}`,
      localEvidence: [
    ...
    },
    {
      ...
      command: typeof assetImportWorkflowPlan?.reportCommand === "string"
        ...
        : `node fixtures/external-engine-baselines/external-parity/write-asset-import-workflow-report.mjs ${engine} tests/reports/external-parity-${engine}-asset-import-workflow.evidence.json ${assetImportWorkflowReportPath}`,
      validationCommands: [
...
function externalBaselineWorkflowValidation(root: string): { readonly path: ".github/workflows/external-parity-external-engine-baselines.yml"; readonly ready: boolean; readonly blockers: readonly string[] } {
  ...
  const requiredFragments = [
    ...
    ["Unity editor env", "A3D_UNITY_EDITOR"],
    ["Unreal editor env", "A3D_UNREAL_EDITOR"],
...
function publicDeploymentWorkflowValidation(root: string): { readonly path: ".github/workflows/v4-public-demo-deploy.yml"; readonly ready: boolean; readonly blockers: readonly string[] } {
  ...
  const blockers = requiredFragments.flatMap(([label, fragment]) => workflow.includes(fragment) ? [] : [`${path} is missing ${label}: ${fragment}`]);
  if (!/A3D_PUBLIC_DEMO_URL:\s*\$\{\{\s*steps\.deployment\.outputs\.page_url\s*\}\}/u.test(workflow)) {
...
export function externalEvidenceLocalPreflight(): ExternalEvidenceLocalPreflight {
  ...
  const deploymentUrl = process.env.A3D_PUBLIC_DEMO_URL?.trim();
  const publicDeployment = {
    envName: "A3D_PUBLIC_DEMO_URL" as const,
    envSet: Boolean(deploymentUrl),
    value: deploymentUrl || undefined,
    durableHttpsCandidate: isDurableHttpsCandidate(deploymentUrl),
    command: "A3D_PUBLIC_DEMO_URL=https://... pnpm verify:public-demo-deployment" as const,
    blockers: [
      ...(deploymentUrl ? [] : ["A3D_PUBLIC_DEMO_URL is not set."]),
      ...(isDurableHttpsCandidate(deploymentUrl) ? [] : ["A3D_PUBLIC_DEMO_URL is not a durable public HTTPS candidate."]),
    ],
  };
  const missing = [
...
function editorLocalPreflight(engine: "unity" | "unreal"): ExternalEditorPreflight {
  const envName = engine === "unity" ? "A3D_UNITY_EDITOR" as const : "A3D_UNREAL_EDITOR" as const;
  const searchRootsEnvName = engine === "unity" ? "A3D_UNITY_SEARCH_ROOTS" as const : "A3D_UNREAL_SEARCH_ROOTS" as const;
  ...
  const cliSmokeOptIn = process.env.A3D_RUN_UNITY_UNREAL_CLI_SMOKE === "true";
  const smokeReportPath = `tests/reports/external-parity-${engine}-editor-cli-smoke.json`;
  return {
    engine,
    envName,
    ...
    cliSmokeOptIn,
    smokeReportPath,
    smokeCommand: `node fixtures/external-engine-baselines/external-parity/run-editor-cli-smoke.mjs ${engine} ${smokeReportPath}`,
    blockers: [
      ...(executableAvailable ? [] : [`${envName} is not set to a usable executable and no ${engine} editor executable was auto-discovered.`]),
      ...(cliSmokeOptIn ? [] : ["A3D_RUN_UNITY_UNREAL_CLI_SMOKE=true has not been set."]),
    ],
  };
}
...
function externalBaselineKitArea(report: Record<string, unknown> | null): ExternalEvidenceArea {
  ...
  return {
    ...
    evidencePaths: ["tests/reports/external-parity-external-engine-baselines.json", "fixtures/external-engine-baselines/external-parity"],
    localEvidence: [
...
function unityUnrealEditorArea(root: string, engine: "unity" | "unreal", baselineKit: Record<string, unknown> | null, localEvidence: ExternalEvidenceLocalSummaries): ExternalEvidenceArea {
  ...
  const cliSmoke = process.env.A3D_RUN_UNITY_UNREAL_CLI_SMOKE === "true";
  ...
  return {
    ...
    blockers: [
      ...
      ...(cliSmoke ? [] : ["A3D_RUN_UNITY_UNREAL_CLI_SMOKE=true has not been set for editor binary verification"]),
...
function renderedProductVisualArea(report: Record<string, unknown> | null, localEvidence: ExternalEvidenceLocalSummaries): ExternalEvidenceArea {
  ...
  return {
    ...
    requiredExternalEvidence: [
      "Real Unity product visual baseline report and screenshot must pass current Aura3D diff validation.",
      "Real Unreal product visual baseline report and screenshot must pass current Aura3D diff validation.",
    ],
    blockers: [
      ...(rendered.unity === true ? [] : ["Unity product visual baseline is missing or failing current Aura3D diff validation"]),
      ...(rendered.unreal === true ? [] : ["Unreal product visual baseline is missing or failing current Aura3D diff validation"]),
...
function publicDeploymentArea(root: string, publicDeployment: Record<string, unknown> | null, production: Record<string, unknown> | null, staticExport: Record<string, unknown> | null, localEvidence: ExternalEvidenceLocalSummaries): ExternalEvidenceArea {
  ...
  return {
    ...
    blockers: [
      ...(deploymentUrl ? [] : ["A3D_PUBLIC_DEMO_URL has not been validated against a durable public HTTPS origin"]),
      ...(validation.ok ? [] : [
