...
interface GalileoModule {
  readonly v8FlagshipViewerScene: V8FlagshipViewerSceneConfig;
  renderG3DFlagshipViewer(canvas: HTMLCanvasElement, scene?: V8FlagshipViewerSceneConfig): Promise<V8FlagshipRenderResult>;
}
...
interface V8ThreejsParityReady {
  readonly status: "ready";
  readonly schema: "g3d-v8-threejs-parity/v1";
  readonly purpose: "same-scene flagship G3D vs Three.js competitor baseline";
  readonly generatedInBrowserAt: string;
  readonly scene: V8FlagshipViewerSceneConfig;
  readonly g3d: Omit<V8FlagshipRenderResult, "dataUrl">;
...
async function validateNodeReport(): Promise<void> {
  ...
  const reportPath = "tests/reports/v8-threejs-parity.json";
  ...
  const failures = [
    ...
    ...(report.assertions?.noG3DRuntimeThreeImport ? [] : ["noG3DRuntimeThreeImport assertion failed"]),
  ...
  const result = {
    schema: "g3d-v8-threejs-parity-node-validation/v1",
...
async function run(): Promise<void> {
  ...
  try {
    ...
    const galileo = await withTimeout(
      import("../../benchmarks/galileo/src/scenes/v8-flagship-viewer") as Promise<GalileoModule>,
      45_000,
      "G3D benchmark module import timed out before the parity harness could run."
    );
    const threeModule = await withTimeout(
      import("../../benchmarks/threejs/src/scenes/v8-flagship-viewer") as Promise<ThreejsModule>,
      45_000,
      "Three.js benchmark module import timed out before the parity harness could run."
    );
    const scene = galileo.v8FlagshipViewerScene;
    const g3dCanvas = requiredCanvas("g3d-flagship-viewer", scene);
    const threeCanvas = requiredCanvas("threejs-flagship-viewer", scene);
    const sideBySideCanvas = requiredCanvas("side-by-side", scene);
    if (status) status.textContent = "rendering G3D flagship viewer";
    const g3d = await withTimeout(
      galileo.renderG3DFlagshipViewer(g3dCanvas, scene),
      75_000,
      "G3D flagship viewer render timed out before publishing a capture."
    );
    ...
    const threejs = await withTimeout(threeModule.renderThreeFlagshipViewer({
      ...
      scene,
      camera: {
        cameraPosition: g3d.camera.cameraPosition,
        target: g3d.camera.target,
        fovYRadians: scene.camera.fovYRadians
      },
      bounds: g3d.bounds
    }), 75_000, "Three.js flagship viewer render timed out before publishing a capture.");
    const diff = computeDiff(await dataUrlToPixels(g3d.dataUrl), await dataUrlToPixels(threejs.dataUrl));
    const sideBySide = await drawSideBySide(sideBySideCanvas, g3d.dataUrl, threejs.dataUrl, diff, scene);
    const ready: V8ThreejsParityReady = {
      status: "ready",
      schema: "g3d-v8-threejs-parity/v1",
      purpose: "same-scene flagship G3D vs Three.js competitor baseline",
      generatedInBrowserAt: new Date().toISOString(),
      scene,
      g3d: stripDataUrl(g3d),
      ...
      assertions: {
        sameAsset: g3d.asset.uri === threejs.asset.uri && g3d.asset.id === threejs.asset.id,
        sameHdri: g3d.environment.uri === threejs.environment.uri && g3d.environment.id === threejs.environment.id,
        sameResolution: scene.width === g3dCanvas.width
          && scene.height === g3dCanvas.height
          && g3dCanvas.width === threeCanvas.width
          && g3dCanvas.height === threeCanvas.height,
        realThreeRenderer: threejs.renderer.actualThreeRenderer,
        noG3DRuntimeThreeImport: true,
      ...
      openGaps: [
        "G3D and Three.js use different product-stage implementations, so stage/shadow deltas are expected.",
...
async function drawSideBySide(
  ...
  context.fillText("G3D flagship viewer", 24, scene.height + 34);
