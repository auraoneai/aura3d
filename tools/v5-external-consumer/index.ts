...
const consumerSource = requiredSubpaths.map((subpath) => {
  const importPath = subpath === "." ? "@galileo3d/engine" : `@galileo3d/engine/${subpath.slice(2)}`;
  return `import "${importPath}";`;
}).join("\n");
const monorepoInternalImports = /\.\.\/|packages\//.test(consumerSource);
const report = {
  schema: "g3d-v5-external-consumer/v1",
...
const sourcePath = resolve("tests/reports/v5-external-consumer/consumer.ts");
mkdirSync(dirname(sourcePath), { recursive: true });
writeFileSync(sourcePath, `${consumerSource}\n`);
const reportPath = resolve("tests/reports/v5-external-consumer.json");
