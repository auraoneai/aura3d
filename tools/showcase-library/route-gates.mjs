...
/**
 * Release classes whose retained evidence may be frozen.
 *
 * A frozen route keeps `published: true` so its build, deploy, and classification gates still
 * run, but evidence *producers* skip it: its retained probes are historical certification
 * records for a route that has already been superseded, so regenerating them churns artifacts
 * and rebinds asset evidence to screenshots nobody reviews.
 */
const frozenEligibleReleaseClasses = new Set([
  "game-layer-diagnostic",
  "removed-from-public-showcase"
]);
...
function validateRouteGateConfig(config, configPath) {
  ...
  for (const route of config.routes) {
    ...
    if (route.retainedEvidenceFrozen !== undefined) {
      if (typeof route.retainedEvidenceFrozen !== "boolean") {
        throw new Error(`${configPath} route ${route.id} retainedEvidenceFrozen must be a boolean`);
      }
      // Freezing means "this route's retained evidence is historical and must not be
      // regenerated by sweeps". Only non-promoted classes may claim it; a promoted route
      // must keep regenerating its own proof.
      if (route.retainedEvidenceFrozen && !frozenEligibleReleaseClasses.has(route.releaseClass)) {
        throw new Error(
          `${configPath} route ${route.id} cannot set retainedEvidenceFrozen with releaseClass ${route.releaseClass}`
        );
      }
    }
