...
const requiredReportsByTrack: Record<Track, readonly string[]> = {
  code: ["tests/reports/v3-current-capability.json", "tests/reports/v3-task-assignments.json"],
  examples: ["tests/reports/v3-example-truth-audit.json", "tests/reports/v3-example-screenshots/manifest.json"],
  rendering: ["tests/reports/v3-rendering.json"],
  assets: ["tests/reports/v3-asset-corpus.json"],
  editor: ["tests/reports/v3-editor-authoring.json"],
  runtime: ["tests/reports/v3-runtime-systems.json"],
  benchmarks: ["tests/reports/v3-engine-comparison.json"],
  all: [
    "tests/reports/v3-current-capability.json",
    "tests/reports/v3-task-assignments.json",
    "tests/reports/v3-example-truth-audit.json",
...
function main(): void {
  ...
  const report: V3VerificationReport = {
    ...
    runId,
    command: `node --experimental-strip-types tools/v3-verification/index.ts --track ${args.track}${args.strict ? " --strict" : ""}`,
    commit,
    track: args.track,
    strict: args.strict,
    docs,
...
function writeClaimGateReport(
  ...
  writeJson("tests/reports/v3-claim-gates.json", {
    ok: unchecked.length === 0 && requiredReports.every((report) => report.freshForCurrentCommit) && broadClaims.length === 0,
    generatedAt: new Date().toISOString(),
    runId,
    commit,
    command: "node --experimental-strip-types tools/v3-verification/index.ts --track all",
    gates: [
...
function writeCurrentCapabilityReport(
  ...
  writeJson("tests/reports/v3-current-capability.json", {
    ok: false,
    generatedAt: new Date().toISOString(),
    runId,
    commit,
    command: "node --experimental-strip-types tools/v3-verification/index.ts --track code",
    status: "blocked",
...
function reportPathForTrack(track: Track): string {
  return track === "all" ? "tests/reports/v3.json" : `tests/reports/v3-${track}.json`;
}
