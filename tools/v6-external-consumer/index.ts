...
const build = json("tests/reports/v6-external-vite-build.json");
const render = json("tests/reports/v6-external-consumer-render.json");
...
const screenshot = String(render.screenshot ?? "tests/reports/v6-external-consumer/external-consumer-render.png");
const checks = [
  ...
  { id: "real-v6-scene", pass: Array.isArray(runtimeMetrics.assetIds) && runtimeMetrics.assetIds.includes("damaged-helmet") && runtimeMetrics.hdrEnvironmentId === "studio-small-08", detail: JSON.stringify(runtimeMetrics) },
...
const report = {
  schema: "g3d-v6-external-consumer/v1",
...
const reportPath = resolve("tests/reports/v6-external-consumer.json");
