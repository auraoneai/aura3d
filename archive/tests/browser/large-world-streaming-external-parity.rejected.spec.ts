...
test.describe("large-world streaming ExternalParity example", () => {
  ...
  test("streams cells, culls, selects LODs, and reports camera-path metrics", async ({ page }) => {
    ...
    expect(state?.featureEvidence?.weatherRendered).toBe(true);
    ...
    expect(state?.featureEvidence?.voxelBlocksRendered).toBe(true);
    ...
    expect(String(state?.metrics?.weatherStateHash ?? "")).toMatch(/^[0-9a-f]{8}$/);
    ...
    expect(String(state?.metrics?.voxelWorldHash ?? "")).toMatch(/^[0-9a-f]{8}$/);
