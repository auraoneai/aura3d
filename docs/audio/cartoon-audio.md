# Cartoon Audio Helpers

The Aura3D 1.1 cartoon audio helpers in `@aura3d/audio` cover the scoped
cartoon-episode workflow: building a predictable voice/music/SFX/ambient
...
cartoon buses (`voice`, `music`, `sfx`, `ambient`) at sensible default
volumes.
...
```ts
import { AudioContextManager, createCartoonAudioMixer } from "@aura3d/audio";

const contextManager = new AudioContextManager();
const cartoonMixer = createCartoonAudioMixer(contextManager.context, {
  musicVolume: 0.5
});

// Mute music while a line of dialogue plays.
cartoonMixer.buses.music.mute(true);
...
- "The cartoon audio mixer provides the standard voice/music/SFX/ambient
