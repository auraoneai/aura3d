...
3. Wait for `window.__GALILEO3D_EDITOR_APP__.getState().status` to become `ready`.
...
- Static export `runtime.js` generation is owned by `@galileo3d/editor-runtime` through `createStaticExportRuntime()`, then consumed by the editor app exporter.
