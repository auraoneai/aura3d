...
The V5 asset library is the content backbone for proving that G3D can render real-world browser 3D workloads instead of toy meshes. Assets are tracked in `fixtures/v5/assets/manifest.json` and exposed through `packages/assets/src/v5/V5AssetRegistry.ts`.
...
The asset readiness gate is `pnpm v5:assets`. It must prove:
...
The V5 asset library is sufficient for the V5 broad replacement track. It is not a claim that G3D supports every glTF extension, every DCC export pipeline, or every commercial asset pipeline. Those gaps remain in `docs/project/v5-roadmap-known-gaps.md`.
