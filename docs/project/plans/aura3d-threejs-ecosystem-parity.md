...
| context loss recovery | webglcontextlost handling by hand | `onDeviceLost`, `onDeviceRestored` | yes | 1 | parity | `tests/browser/context-loss-recovery.spec.ts` | WS-3.9 proves a real `WEBGL_lose_context` loss/restoration through the root API, immediate app pause, explicit `setScene` remount, a newly mounted production runtime, and identical restored framebuffer pixels. Subscriptions registered before renderer mount attach on arrival and unsubscribe is listener-specific. Recovery is app-driven; universal transparent recreation of arbitrary caller-owned GPU resources remains unclaimed. |
