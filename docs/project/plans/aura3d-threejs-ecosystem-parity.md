...
| text rendering | parity-unproven | Root `text3D` now produces depth-bearing, transformable, lit/material-bearing indexed triangle glyph meshes with browser proof; see `docs/rendering/geometry-instancing-lod-text.md`. Its built-in catalog is bounded to uppercase alphanumerics and punctuation, so it is not claimed as parity with arbitrary-font TextGeometry or troika shaping/SDF. Accessible world labels remain a separate DOM UI surface. |
...
| context loss recovery | webglcontextlost handling by hand | `onDeviceLost`, `onDeviceRestored` | yes | 1 | parity | `tests/browser/context-loss-recovery.spec.ts` | Closed for public lifecycle visibility: app.onDeviceLost(), app.onDeviceRestored() and app.deviceLost() surface a real `WEBGL_lose_context` loss and restoration through the root API on the default primitive production-runtime route and the explicit safe-basic compatibility path. Subscriptions registered before renderer mount attach on arrival, and unsubscribe is listener-specific. Not claimed here: automatic GPU-resource recreation and restored pixels; that stronger recovery obligation remains WS-3.9. |
...
| text rendering | troika-three-text or TextGeometry | `text3D`, `createAuraText3DGeometry` | yes | 1 | parity-unproven | `tests/browser/root-geometry-instancing-lod-text.spec.ts` | Root mesh text has real extrusion depth, normals, transforms, PBR material/lighting participation, and pixel evidence. The catalog is intentionally bounded to uppercase A-Z, digits, space, dash, and period; arbitrary fonts, Unicode shaping, SDF/MSDF, and curved text remain unproven. DOM world labels stay accessible UI and are not counted as 3D text. |
