# Aura3D Showcase Visual Quality Standard
...
Current status (2026-07-28): Blockfall Reactor, Turbo Drift Circuit, and Skyline
Runner are `prototype-blocked` while their visual rebuilds and current
hash-bound independent reviews are pending. Their typed assets, mounted
gameplay, bounded geometry, route-local AI/challenge logic, and route evidence
remain technical proof only. The retained July 19 review is stale and does not
approve the current routes.

Automated geometry/contact results are structural checks, not visual approval.
The visual-QA report must use final screenshot pixels and an isolated
visible-versus-suppressed subject render to measure subject size and clipping,
desktop/mobile composition, UI overlap, foreground/background balance, giant
occluders, empty proof staging, and scene-level change outside HUD-only
