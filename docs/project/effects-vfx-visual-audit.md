...
| --- | --- | --- | --- | --- | --- |
| pass | blocker | Public prompt API / effects.fog | Fog has a real manual renderer code atmosphere path and Canvas2D fallback. | Three renderer maps fog to THREE.FogExp2.; Canvas2D fallback paints an atmospheric overlay. | Keep fog tied to renderer atmosphere and screenshot contrast checks; do not replace it with a label or DOM overlay. |
