# Three.js Parity Execution Goal
...
Remove the 64-joint uniform-array cap by uploading the matrix palette as a float data texture,
and support `JOINTS_1` / `WEIGHTS_1` as real extra influences instead of a diagnostic.
...
Deliverables:
- `threejs-parity-status.md`, `threejs-parity-claim-boundary.md`, and
  `threejs-parity-parity-matrix.md` reference only named passing tests.
