# Galileo3D Docs Site Map

Version: 1.0.0

This file is the deployment manifest for the current documentation set. A docs deployment must include these pages and example targets for the same package version.

## API

- [Public API reference](api/public-api.md)
- [API overview](api/README.md)

## Tutorials

- [Getting started real scene](tutorials/getting-started-real-scene.md)
- [Product configurator tutorial](tutorials/product-configurator.md)

## Examples

- [Product demo index](examples/product-demos.md)
- [External demo status](examples/external-demos.md)
- [Product configurator example](../examples/product-configurator/index.html)
- [Architecture viewer example](../examples/architecture-viewer/index.html)
- [Game slice example](../examples/game-slice/index.html)
- [Physics sandbox example](../examples/physics-sandbox/index.html)

## Release And Governance

- [Changelog](../CHANGELOG.md)
- [Release process](release-process.md)
- [Release checklist](release-checklist.md)
- [Master gate evidence](v2-master-gate-evidence.md)
- [Browser and hardware matrix](browser-hardware-matrix.md)
- [Asset corpus report](assets/asset-corpus-report.md)
- [WebGPU fallback behavior](rendering/webgpu-fallback.md)
- [WebGPU hardware matrix](rendering/webgpu-hardware-matrix.md)
- [Compatibility matrix](compatibility.md)
- [Known limits](known-limits.md)
- [Claim guidelines](claim-guidelines.md)
- [Migration notes](migration.md)
- [Security policy](security-policy.md)
- [Support policy](support-policy.md)
- [Contribution guide](../CONTRIBUTING.md)

## Current V9 Parity Docs

- [V9 status](v9-roadmap-status.md)
- [V9 Three.js parity plan](v9-roadmap-three-js-parity-plan.md)
- [V9 parity matrix](v9-roadmap-parity-matrix.md)
- [V9 claim boundary](v9-roadmap-claim-boundary.md)
- [V9 code backlog](v9-roadmap-code-backlog.md)

## Historical Roadmaps

- [V2 product asset pipeline plan](v2-roadmap-product-asset-pipeline-plan.md)
- [V3 product workflow plan](v3-roadmap-product-workflow-plan.md)
- [V4 visual engine plan](v4-roadmap-visual-engine-plan.md)
- [V5 visual engine plan](three-compat-roadmap-visual-engine-plan.md)
- [V6 production renderer plan](production-runtime-roadmap-production-renderer-plan.md)
- [V7 runtime parity plan](v7-roadmap-runtime-parity-plan.md)
- [V8 Three.js competitor plan](v8-roadmap-three-js-competitor-plan.md)

## V2 Governance

- [V2 README](v2-readme.md)
- [V2 decision gates](v2-decision-gates.md)
- [V2 execution checklist](v2-filename-level-execution-checklist.md)
- [V2 claim registry](v2-claim-registry.md)
