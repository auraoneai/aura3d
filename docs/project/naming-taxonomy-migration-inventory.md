...
```bash
rg --files | rg '(^|/)(apps|examples|fixtures|tools|tests|docs|templates)/|package.json$|\.github/' | rg '(^|/|[-_])v[0-9]+($|[-_/\.])'
find . -maxdepth 3 \( -path './node_modules' -o -path './.git' \) -prune -o -type d | sed 's#^./##' | rg '(^|/|[-_])v[0-9]+($|[-_/])'
rg -n 'v[0-9]+' package.json README.md docs apps packages tests tools fixtures templates .github
```

Current first-pass path inventory from 2026-05-20 found `1978` version-style file paths in the scoped roots:

| Root | Version-Style File Count | Initial Classification |
| --- | ---: | --- |
| `.github` | 2 | CI workflow names. Require workflow rename/alias review. |
| `apps` | 222 | Active and historical browser routes. High risk. |
| `docs` | 182 | Mostly historical project docs plus active tutorial names. Medium risk. |
| `examples` | 246 | Public example paths and quarantined example paths. High risk. |
| `fixtures` | 514 | Fixture URLs used by tests/apps/reports. High risk. |
| `packages` | 62 | Package exports and internal versioned surfaces. Highest risk. |
| `release-artifacts` | 130 | Historical release artifacts. Archive unless active readers depend on them. |
| `templates` | 78 | Starter templates. Medium risk. |
| `tests` | 257 | Browser specs, reports, and harnesses. High risk. |
| `tools` | 285 | Report generators, parity tools, audit scripts. High risk. |
