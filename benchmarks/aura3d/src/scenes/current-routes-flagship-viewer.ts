...
export interface CurrentFlagshipRenderResult {
  ...
  readonly renderer: {
    ...
    readonly sdkSurface: "@aura3d/engine/production-runtime";
...
export async function renderA3DFlagshipViewer(
  ...
  let viewer: A3DProductViewer | undefined;
  try {
    ...
    if (!camera) throw new Error("A3D flagship viewer did not publish camera diagnostics.");
    const target = [
