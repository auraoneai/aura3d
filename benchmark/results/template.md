# Benchmark Round Template

Round: `N`
Date:
Commit:
Runner:
Scorer:
Scorer neutrality statement:
Scorer signature:
User signature: `gchahal1982`

## Context Bundles

| Run | Agent | Library | Context source | Notes |
|---|---|---|---|---|
| A | Codex | Aura3D |  |  |
| B | Codex | Three.js |  |  |
| C | Claude Code | Aura3D |  |  |
| D | Claude Code | Three.js |  |  |

## Prompt Scores

| Prompt | Codex Aura3D | Codex Three.js | Codex Result | Claude Aura3D | Claude Three.js | Claude Result | Notes |
|---|---:|---:|---|---:|---:|---|---|
| 01 Physics playground |  |  |  |  |  |  |  |
| 02 Particle fountain |  |  |  |  |  |  |  |
| 03 Solar system |  |  |  |  |  |  |  |
| 04 Neon tunnel |  |  |  |  |  |  |  |
| 05 Data visualization |  |  |  |  |  |  |  |
| 06 Mini-golf |  |  |  |  |  |  |  |
| 07 Material lab |  |  |  |  |  |  |  |
| 08 City block |  |  |  |  |  |  |  |
| 09 Primitive humanoid |  |  |  |  |  |  |  |
| 10 Sneaker viewer |  |  |  |  |  |  |  |

Numeric score cells are visual match scores. Result cells are `Aura3D win`,
`tie`, or `Aura3D loss`.

## Metric Detail

Copy this table once per prompt and run.

| Metric | Aura3D | Three.js | Winner | Evidence |
|---|---:|---:|---|---|
| Compiles |  |  |  |  |
| Runs in browser |  |  |  |  |
| Visual match |  |  |  |  |
| Lines of user-written code |  |  |  |  |
| Files created |  |  |  |  |
| Hallucinated APIs |  |  |  |  |
| Invented asset paths |  |  |  |  |
| Repair turns |  |  |  |  |
| Time to first usable render |  |  |  |  |
| Bundle size |  |  |  |  |
| Modifiability |  |  |  |  |

## Round Summary

- Codex Aura3D wins:
- Claude Aura3D wins:
- Codex wins among prompts 7/8/10:
- Claude wins among prompts 7/8/10:
- Aura3D visual scores of 4 or higher:
- Aura3D visual scores below 3:
- Engine parity result:
- Pass criteria met before Phase C decision: yes/no

Follow-up library work:

## Engine Parity Summary

Detailed engine results belong in `round-N-engine.md`.

| Scene | Visual parity | FPS result | Memory result | Bundle result | Pass |
|---|---:|---|---|---|---|
| engine-01-material-grid |  |  |  |  |  |
| engine-02-city-block |  |  |  |  |  |
| engine-03-particles-vfx |  |  |  |  |  |
| engine-04-physics-ramp |  |  |  |  |  |
| engine-05-sneaker-product |  |  |  |  |  |
