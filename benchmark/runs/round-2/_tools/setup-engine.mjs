...
function commonFiles(sourceDir, scene, library) {
  ...
  writeFileSync(
    ...
    [
      ...
      "    <title>Round 2 Engine Benchmark</title>",
      "  </head>",
      "  <body>",
      "    <div id=\"app\"></div>",
...
function auraSource(scene) {
  ...
  const nodes = {
    ...
    "engine-03-particles-vfx": `
      .addMany(prefabs.particleFountain({ color: "#66e6ff", count: 1400 }))
    ...
    "engine-05-sneaker-product": `
      ...
      .add(model(assets.sneaker, { name: "sneaker", scale: 2.55, position: [0, 0.45, -0.65], castShadow: true, receiveShadow: true }))`
  }[scene];
  ...
  const imports = scene === "engine-05-sneaker-product"
    ? "camera, createAuraApp, defineAuraAssets, effects, interactions, lights, material, model, prefabs, primitives, scene"
