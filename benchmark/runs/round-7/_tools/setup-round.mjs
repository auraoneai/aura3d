...
function writeScaffold(sourceDir, run, promptNumber) {
  ...
  const packageName = `round7-${run.id}-prompt-${promptNumber}`;
  ...
  writeFileSync(
    ...
    [
      ...
      "    <title>Aura3D Round 7 Benchmark</title>",
      "  </head>",
      "  <body>",
      "    <div id=\"app\"></div>",
...
for (const run of runConfigs) {
  for (const promptFile of promptFiles) {
    ...
    writeJson(join(promptDir, "run-metadata.json"), {
      ...
      sourceDirectory: `benchmark/runs/round-7/${run.id}/prompt-${promptNumber}/source`,
