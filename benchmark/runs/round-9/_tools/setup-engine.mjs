...
function commonFiles(sourceDir, scene, library) {
  ...
  writeFileSync(
    ...
    [
      ...
      "    <title>Round 9 Engine Benchmark</title>",
      "  </head>",
      "  <body>",
      "    <div id=\"app\"></div>",
...
function auraSource(scene) {
  ...
  const nodes = {
    "engine-01-material-grid": `
      ...
      .add(primitives.sphere({ name: "emissive reference swatch", material: material.emissive({ color: "#ff45c8", emissive: "#ff45c8", roughness: 0.25 }) }).position(2.25, 0.48, -0.75).scale(0.42))`,
    "engine-02-city-block": `
  ...
  const imports = scene === "engine-05-sneaker-product"
    ? "camera, createAuraApp, effects, interactions, lights, material, model, prefabs, primitives, scene, timeline"
    : "camera, createAuraApp, effects, interactions, lights, material, prefabs, primitives, scene";
  return `
...
loader.load("/benchmark/assets/sneaker.glb", (gltf) => {
  ...
  obj.position.sub(center); obj.scale.setScalar(2.6 / Math.max(size.x, size.y, size.z)); obj.position.y = .65; scene.add(obj); sneakerReady = true;
...
for (const sceneName of scenes) {
  ...
  writeFileSync(
    ...
    [
      `scene: ${sceneName}`,
      "visual target: match the Engine Parity Benchmark reference scene requirements in benchmark/engine/README.md",
      sceneName === "engine-05-sneaker-product" ? "allowed asset: benchmark/assets/sneaker.glb" : "allowed asset: none",
      "machine/viewport: benchmark/runner/README.md default machine, Playwright Chromium, 1440 x 960, deviceScaleFactor 1",
      `aura3d source: benchmark/runs/round-9/engine/${sceneName}/aura3d/source/src/main.ts`,
      `threejs source: benchmark/runs/round-9/engine/${sceneName}/threejs/source/src/main.ts`,
      ""
    ].join("\n")
  );
