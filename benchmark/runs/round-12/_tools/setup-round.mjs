...
const roundRoot = resolve(repoRoot, "benchmark/runs/round-12");
...
function writeScaffold(sourceDir, run, promptNumber) {
  ...
  const packageName = `round12-${run.id}-prompt-${promptNumber}`;
...
for (const run of runConfigs) {
  for (const promptFile of promptFiles) {
    ...
    writeJson(join(promptDir, "run-metadata.json"), {
      ...
      sourceDirectory: `benchmark/runs/round-12/${run.id}/prompt-${promptNumber}/source`,
