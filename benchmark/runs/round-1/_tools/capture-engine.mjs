  ...
  if (buildStatus === 0) {
    ...
    try {
      ...
      if (loaded) {
        ...
        const samples = await page.evaluate(async () => {
          ...
          return await new Promise((resolve) => {
            function frame(now) {
              ...
              last = now;
              if (now - start >= 15000) resolve(frameTimes.slice(1));
        ...
        const p50Frame = sorted[Math.floor(sorted.length * 0.5)] ?? null;
        p95FrameTimeMs = sorted[Math.floor(sorted.length * 0.95)] ?? null;
        p50Fps = p50Frame ? 1000 / p50Frame : null;
