...
function writeScaffold(sourceDir, run, promptNumber) {
  ...
  const packageName = `round1-${run.id}-prompt-${promptNumber}`;
  ...
  writeFileSync(
    ...
    [
      ...
      "    <title>Aura3D Round 1 Benchmark</title>",
      "  </head>",
      "  <body>",
      "    <div id=\"app\"></div>",
...
for (const run of runConfigs) {
  for (const promptFile of promptFiles) {
    ...
    writeJson(join(promptDir, "run-metadata.json"), {
      ...
      sourceDirectory: `benchmark/runs/round-1/${run.id}/prompt-${promptNumber}/source`,
