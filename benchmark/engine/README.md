...
The prompt benchmark measures whether agents can build better 3D apps with Aura3D than with low-level renderer code. This engine benchmark measures the library itself without the agent layer.

Aura3D is a low-level renderer code competitor, so the engine must be measured directly.
...
2. low-level renderer code reference implementation.

The two implementations must aim for the same visual target. Do not simplify the low-level renderer code version to make Aura3D look better. Do not use Aura3D templates or agent helpers in the low-level renderer code version.
...
Record these for Aura3D and low-level renderer code:
...
- Aura3D p50 FPS is no worse than 20% below low-level renderer code in at least 4 of 5 scenes.
- Aura3D p50 FPS is no worse than 35% below low-level renderer code in any scene.
- Aura3D JS heap peak is no worse than 25% above low-level renderer code in at least 4 of 5 scenes.
- Aura3D JS heap peak is no worse than 50% above low-level renderer code in any scene.
- Draw-call differences above 25% are explained in the result file.
- Bundle-size differences are reported. A scene fails if Aura3D adds more than 250 KB gzip over the low-level renderer code reference without a written justification accepted by the user.
