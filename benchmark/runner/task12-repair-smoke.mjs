...
writeFileSync(join(outDir, "src", "main.ts"), `import { camera, city, createAuraApp, defineAuraAssets, effects, lights, prefabs, scene, sceneKits, ui } from "@aura3d/engine";
import "./style.css";
...
if (activeScene === "particle") {
  ...
  createAuraApp("#data", sceneKits.dataViz({ dataset: dataSceneKitDataset }).toAppOptions());
} else if (activeScene === "city") {
  const cityIsNight = params.get("mode") === "night";
  const cityState = city.createState({ blocks: 20, litWindows: true, timeOfDay: cityIsNight ? "night" : "day" });
  const cityApp = createAuraApp("#city", { scene: cityState.scene(), diagnostics: true });
  city.bindDayNightToggle("#city-toggle", cityApp, cityState, {
    onChange: (timeOfDay) => ui.setText("#city-title", timeOfDay === "night" ? "City Night" : "City Day")
  });
...
function createParticleScene(emissionRate: number) {
  return createAuraApp("#particle", {
    scene: scene()
      .background("#030711")
      .addMany(prefabs.particleFountain({ count: 900, emissionRate }))
...
try {
  ...
  const fpsCalibration = await runFpsCalibration(browser, {
  ...
  await captureRoute(browser, `${url}?scene=particle`, "particle-panel", [["particle-control.png", 1500]], summary, fpsCalibration);
  await captureRoute(browser, `${url}?scene=neon`, "neon-panel", [["neon-frame-1.png", 1500], ["neon-frame-2.png", 1800]], summary, fpsCalibration);
  await captureRoute(browser, `${url}?scene=physics`, "physics-panel", [["physics-playground.png", physicsCaptureWaitMs]], summary, fpsCalibration);
  await captureRoute(browser, `${url}?scene=solar`, "solar-panel", [["solar-system.png", solarCaptureWaitMs]], summary, fpsCalibration);
  await captureRoute(browser, `${url}?scene=data`, "data-panel", [["data-default.png", dataCaptureWaitMs]], summary, fpsCalibration);
  await captureRoute(browser, `${url}?scene=data&hover=1`, "data-panel", [["data-hover.png", dataCaptureWaitMs]], summary, fpsCalibration);
  await captureRoute(browser, `${url}?scene=city&mode=day`, "city-panel", [["city-day.png", 1500]], summary, fpsCalibration);
  await captureRoute(browser, `${url}?scene=city&mode=night`, "city-panel", [["city-night.png", 1500]], summary, fpsCalibration);
  await captureRoute(browser, `${url}?scene=product`, "product-panel", [["product-landscape.png", productCaptureWaitMs]], summary, fpsCalibration);
  await captureRoute(browser, `${url}?scene=mini-golf`, "mini-golf-panel", [["mini-golf.png", miniGolfCaptureWaitMs]], summary, fpsCalibration);
  await captureRoute(browser, `${url}?scene=material`, "material-panel", [["material-lab.png", materialCaptureWaitMs]], summary, fpsCalibration);
  await captureRoute(browser, `${url}?scene=humanoid`, "humanoid-panel", [["humanoid-frame-1.png", 900], ["humanoid-frame-2.png", 1200]], summary, fpsCalibration);
  ...
  writeFileSync(join(outDir, "metrics.json"), `${JSON.stringify({
    ...
    pass: summary.routeMetrics.length >= 12 && summary.routeMetrics.every((route) => route.routeHealth === "pass" && route.fpsInstrumentationStatus === "pass" && Number.isFinite(route.p50Fps))
  }, null, 2)}\n`);
...
async function captureRoute(browserInstance, routeUrl, panelId, captures, summary, fpsCalibration) {
  ...
  try {
    ...
    const routeMetadata = await page.evaluate(() => window.__AURA3D_ROUTE_READY__?.diagnostics ?? null);
    const capturePlan = captures.map(([fileName, waitMs]) => ({ fileName, waitMs }));
    const fpsSample = await samplePageFps(page, { warmupMs: fpsWarmupMs, sampleMs: fpsSampleMs });
    summary.routeMetrics.push(applyFpsCalibrationToMetrics({
      route: routeUrl,
      panelId,
      routeHealth: "pass",
      routeHealthMethod: "HTTP load + canvas present + screenshot capture",
      routeMetadata,
      capturePlan,
