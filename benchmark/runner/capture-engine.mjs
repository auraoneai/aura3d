...
if (!sceneDirArg || !["aura3d", "threejs"].includes(libraryArg)) {
  console.error("Usage: node benchmark/runner/capture-engine.mjs <engine-scene-dir> <aura3d|threejs>");
  process.exit(2);
}
...
function runInstall() {
  const first = run("npm", ["install"], "install.log");
  if (first.status === 0) return first;
  spawnSync("npm", ["cache", "verify"], { cwd: sourceDir, encoding: "utf8", maxBuffer: 1024 * 1024 * 32 });
  const second = spawnSync("npm", ["install", "--prefer-online"], { cwd: sourceDir, encoding: "utf8", maxBuffer: 1024 * 1024 * 32 });
  const firstLog = existsSync(join(outputDir, "install.log")) ? readFileSync(join(outputDir, "install.log"), "utf8") : "";
  writeFileSync(
  ...
  if (buildStatus === 0) {
    ...
    try {
      ...
      if (loaded) {
        ...
        failureStage = "runtime";
        failureReason = "production preview page did not load";
        routeHealthEvidence = {
          status: "fail",
          method: "HTTP load",
          url: routeUrl,
...
async function stopProcessTree(child) {
  ...
  const exited = new Promise((resolveExit) => child.once("close", () => resolveExit(true)));
  try {
