import { camera, createAuraApp, environments, interactions, material, model, primitives, scene, type AuraLeanApp } from "@aura3d/lean/product";
...
async function mount(interacted: boolean) {
  ...
  try { await app.ready(); const diagnostics = app.diagnostics(); const canvas = document.querySelector<HTMLCanvasElement>("#app canvas")!; window.__CLEAN_AURA__ = { ready: true, interacted, package: "@aura3d/lean", entry: "product", publicApiOnly: true, backend: diagnostics.backend, runtimeBackend: diagnostics.runtimeBackend, drawCalls: diagnostics.drawCalls, hash: hash(canvas.toDataURL("image/png")) }; }
