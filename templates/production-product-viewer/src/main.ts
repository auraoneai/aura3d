import { createProductViewer, loadGltfScene, loadHdrEnvironment } from "@galileo3d/engine/production-runtime";

const canvas = document.getElementById("viewport");
const metrics = document.getElementById("metrics");
if (!(canvas instanceof HTMLCanvasElement) || !(metrics instanceof HTMLElement)) {
  throw new Error("G3D V6 Product Viewer requires canvas#viewport and #metrics.");
}
...
document.getElementById("capture")?.addEventListener("click", () => {
  window.__g3dProductViewerCapture = viewer.captureScreenshot();
  render("captured");
});

declare global {
  interface Window {
    __g3dProductViewerCapture?: string;
  }
}
