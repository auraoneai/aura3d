#!/usr/bin/env node
/* g2 probe: real-input play, screenshots, error collection for the 4 g2 games.
 * Usage: node /tmp/g2-probe.mjs --out <evidence-root> --games turbo,skyline,courier,patrol
 * Evidence: <out>/<game>/01-*.png ... + probe.json
 */
...
const GAMES = {
  turbo: {
    ...
    extended: { total: 45000, tick: async (page, i) => {
      await page.keyboard.down("KeyW");
      await page.waitForTimeout(2600);
      await page.keyboard.up("KeyW");
      await page.keyboard.down(i % 2 ? "KeyA" : "KeyD"); await page.waitForTimeout(700); await page.keyboard.up(i % 2 ? "KeyA" : "KeyD");
      if (i % 3 === 2) { await page.keyboard.press("Space"); }
    }},
    fail: [{ hold: "KeyW", duration: 2500 }, { hold: "KeyD", duration: 2000 }, { wait: 900 }],
    reset: [{ press: "KeyR" }, { wait: 1200 }],
    mobileKeys: [{ press: "Space" }, { wait: 1200 }],
  },
  skyline: {
    ...
    fail: [{ hold: "KeyD", duration: 4000 }, { wait: 1200 }],
    reset: [{ press: "KeyR" }, { wait: 1200 }],
    mobileKeys: [{ press: "Space" }, { wait: 1200 }],
  },
  courier: {
    ...
    extended: { total: 45000, tick: async (page, i) => {
      ...
      await page.keyboard.up("KeyW");
      await page.keyboard.down(i % 2 ? "KeyA" : "KeyD"); await page.waitForTimeout(900); await page.keyboard.up(i % 2 ? "KeyA" : "KeyD");
  ...
  patrol: {
    ...
    extended: { total: 45000, tick: async (page, i) => {
      ...
      await page.keyboard.up("KeyW");
      await page.keyboard.down(i % 2 ? "KeyA" : "KeyD"); await page.waitForTimeout(800); await page.keyboard.up(i % 2 ? "KeyA" : "KeyD");
      await page.keyboard.press("Space");
...
async function readState(page) {
  try {
    return await page.evaluate(() => {
      ...
      const probe = window.__AURA3D_GAME_PROBE__ ? (() => { try { return window.__AURA3D_GAME_PROBE__(); } catch { return null; } })() : null;
      return { hud, canvas: canvas ? `${canvas.width}x${canvas.height}` : null, luma, colors, probe };
    });
...
async function probeGame(browser, key) {
  ...
  const browser_ctx = await browser.newContext({ viewport: { width: 1280, height: 720 } });
  const page = await browser_ctx.newPage();
  ...
  const shot = async (label) => {
    ...
    result.steps.push({ label, file: p, state: await readState(page) });
  };

  const t0 = Date.now();
  try {
    await page.goto(BASE + g.route, { waitUntil: "load", timeout: 60000 });
    // wait for canvas + HUD
    const deadline = Date.now() + 30000;
    let canvasAt = null;
    while (Date.now() < deadline) {
      const s = await page.evaluate(() => ({ c: !!document.querySelector("canvas"), h: (document.body?.innerText ?? "").trim().length })).catch(() => ({ c: false, h: 0 }));
      if (s.c && canvasAt === null) canvasAt = Date.now();
      if (s.c && s.h > 12) break;
      await page.waitForTimeout(250);
    }
    result.boot = { canvasBootMs: canvasAt ? canvasAt - t0 : null, totalMs: Date.now() - t0, ready: canvasAt !== null };
    await page.waitForTimeout(2500);
  } catch (e) { result.loadError = String(e).slice(0, 200); }
  ...
  // 05 impact/collision + 06 fail
  await runKeys(page, g.fail); await shot("05-impact-or-collision");
  const failState = await readState(page);
  await shot("06-fail-or-ko");

  // extended play (~60s total incl. steps)
  ...
  // win/completion attempt: keep playing toward objective is game-specific; capture state
  ...
  try {
    await mp.goto(BASE + g.route, { waitUntil: "load", timeout: 60000 });
    ...
    result.steps.push({ label: "09-mobile", file: mp2, state: await readState(mp) });
  } catch (e) { result.steps.push({ label: "09-mobile", error: String(e).slice(0, 120) }); }
  await mctx.close();

  result.verdict = {
    booted: !!result.boot?.ready,
...
for (const key of only) {
  ...
  process.stderr.write(`probing ${key} ... `);
  try {
    const r = await probeGame(browser, key);
    process.stderr.write(`boot=${r.boot?.canvasBootMs ?? "?"}ms fps=${r.fpsEarly}/${r.fpsLate} consoleErr=${r.consoleErrors.length} pageErr=${r.pageErrors.length} net=${r.requestFailures.length}\n`);
  } catch (e) { process.stderr.write(`FATAL ${String(e).slice(0, 150)}\n`); }
}
await browser.close();
