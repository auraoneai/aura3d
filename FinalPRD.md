...
- the Three.js parity matrix is upgraded to matched/exceeded with no production-critical gaps;
- the superiority audit produces a defensibility matrix proving feature parity, visual parity or better, animation parity or better, physics/workflow advantage, and performance parity or better;
...
Completion requires a generated `tests/reports/superiority/superiority-audit.json` with this decision matrix:
...
The answer to every question must point to a file under `tests/reports/superiority/`, `docs/project/threejs-superiority-status.md`, or a checked route/test artifact.

## Public Language Record

After `pnpm superiority:audit` passes, public current-state docs and the README use only evidence-backed current-product language:
...
| [x] | `apps/example-parity-lab/src/main.ts` | Upgrade lab to current ThreejsParity route set. | Visual parity lab is clear. |
| [x] | `tests/browser/threejs-parity-*-parity.spec.ts` | Add missing official example categories. | Required examples pass. |
...
| [x] | `README.md` | Update after gates pass. | States parity/exceeds claims backed by superiority reports and links each claim to evidence. |
...
| Status | File | Task | Acceptance |
|---|---|---|---|
| [x] | `tools/superiority-audit/index.ts` | New final audit aggregator. | Fails unless all gates pass. |
| [x] | `tests/reports/superiority/superiority-audit.json` | Final superiority report. | Generated by audit, not hand-written. |
| [x] | `tests/reports/superiority/feature-parity.json` | Feature coverage and Three.js inventory decision matrix. | Every production-critical category is `parity` or `exceeds`. |
| [x] | `tests/reports/superiority/visual-quality.json` | Visual comparison metrics. | All required routes pass. |
| [x] | `tests/reports/superiority/performance.json` | CPU/GPU/frame/startup metrics. | A3D beats or equals baselines. |
| [x] | `tests/reports/superiority/animation-fidelity.json` | Animation metrics. | Motion deltas under threshold. |
| [x] | `tests/reports/superiority/physics-fidelity.json` | Physics stability metrics. | Determinism and interaction pass. |
| [x] | `tests/reports/superiority/memory-lifecycle.json` | Leak and disposal metrics. | No unbounded growth. |
| [x] | `tests/reports/superiority/developer-workflow.json` | Code size/time-to-build metrics. | Workflow APIs reduce implementation effort. |
| [x] | `tests/reports/superiority/claim-defense.json` | Maps every README superiority claim to exact evidence files. | No public claim lacks evidence. |
| [x] | `docs/project/threejs-superiority-status.md` | Generated status doc. | Mirrors report and explicitly states parity/exceeds decisions by category. |
...
pnpm threejs-parity:route-health
pnpm threejs-parity:official-example-parity
pnpm threejs-parity:same-scene-render
pnpm threejs-parity:completion-audit
...
If a script does not exist yet, create it in `package.json`, implement the matching tool under `tools/superiority-*`, and add the report under `tests/reports/superiority/`.
...
`README.md`, `docs/project/current-state.md`, `docs/project/competitive-positioning.md`, `docs/project/go-to-market-strategy.md`, and `docs/project/threejs-superiority-status.md` were updated after `pnpm superiority:audit` and `pnpm superiority:claim-defense` passed.
...
The final README must link each subsection to `tests/reports/superiority/claim-defense.json` and `docs/project/threejs-superiority-status.md`.
...
8. Complete benchmark scenes and superiority audit tools.
