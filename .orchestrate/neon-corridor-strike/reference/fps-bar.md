...
- `examples/neon-corridor-strike` does **NOT** exist on `origin/main`.
  - `git ls-tree -r origin/main --name-only | grep -i neon-corridor` → empty.
  - Not present in the working tree either. There is no local WIP to salvage.
- Therefore this is a greenfield build, not a repair. Do not assume any prior
  neon-corridor route, asset manifest, or evidence exists.
- The overall goal explicitly permits deletion instead of shipping: if a real
  10/10 FPS cannot be proven with browser pixel evidence, the route must NOT be
  shown/claimed. Prototype-label or omit rather than fake.
