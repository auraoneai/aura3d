...
export type CurrentRoutesAssetClass =
  | "product"
  | "automotive"
  | "character"
  | "animation"
...
export interface CurrentRoutesAssetRequirement {
  ...
  readonly requiredClasses: readonly CurrentRoutesAssetClass[];
  readonly requiredFeatures: readonly string[];
}

export interface CurrentRoutesAssetManifestEntry {
  readonly id: string;
  readonly name: string;
  readonly class: CurrentRoutesAssetClass;
...
export interface CurrentRoutesEnvironmentManifestEntry {
  readonly id: string;
  readonly name: string;
  readonly localPath: string;
  readonly source: string;
...
export interface CurrentRoutesAssetManifest {
  readonly schema: "a3d-current-routes-local-asset-corpus";
  readonly claimBoundary: string;
  readonly requirements: CurrentRoutesAssetRequirement;
  readonly assets: readonly CurrentRoutesAssetManifestEntry[];
  readonly environments: readonly CurrentRoutesEnvironmentManifestEntry[];
}

export interface CurrentRoutesGlbInspection {
...
export interface CurrentRoutesAssetReadinessEntry {
  readonly id: string;
  readonly localPath: string;
  ...
  readonly inspection: CurrentRoutesGlbInspection | null;
}

export interface CurrentRoutesEnvironmentReadinessEntry {
  readonly id: string;
  readonly localPath: string;
  readonly exists: boolean;
  readonly bytesMatch: boolean;
...
export interface AdvancedAssetCorpusSummary {
  readonly schema: "a3d-current-routes-asset-readiness";
  ...
  readonly classCoverage: readonly CurrentRoutesAssetClass[];
  readonly featureCoverage: readonly string[];
  readonly failures: readonly string[];
  readonly entries: readonly CurrentRoutesAssetReadinessEntry[];
  readonly environments: readonly CurrentRoutesEnvironmentReadinessEntry[];
}
...
export function loadCurrentRoutesAssetManifest(path = "fixtures/threejs-parity/assets/manifest.json"): CurrentRoutesAssetManifest {
  return JSON.parse(readFileSync(resolve(path), "utf8")) as CurrentRoutesAssetManifest;
}

export function inspectCurrentRoutesGlb(path: string): CurrentRoutesGlbInspection {
...
export function createAdvancedAssetCorpusSummary(manifest = loadCurrentRoutesAssetManifest()): AdvancedAssetCorpusSummary {
  const entries = manifest.assets.map((asset): CurrentRoutesAssetReadinessEntry => {
    ...
    const inspection = exists ? inspectCurrentRoutesGlb(path) : null;
  ...
  const environments = manifest.environments.map((environment): CurrentRoutesEnvironmentReadinessEntry => {
  ...
  const classCoverage = [...new Set(manifest.assets.map((asset) => asset.class))].sort() as CurrentRoutesAssetClass[];
  ...
  return {
    schema: "a3d-current-routes-asset-readiness",
...
export function writeAdvancedAssetCorpusReport(path = "tests/reports/current-routes-assets.json"): AdvancedAssetCorpusSummary {
  const summary = createAdvancedAssetCorpusSummary();
