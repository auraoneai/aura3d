...
export interface ProductionAssetCorpusRequirements {
...
export interface ProductionAssetManifestEntry {
  ...
  readonly class: ProductionAssetClass;
...
export interface ProductionAssetManifest {
  readonly schema: "a3d-production-runtime-real-asset-corpus";
  readonly sourceManifest: string;
  readonly requirements: ProductionAssetCorpusRequirements;
  readonly claimBoundary: string;
  readonly assets: readonly ProductionAssetManifestEntry[];
}

export interface ProductionGlbInspection {
...
export interface ProductionAssetReadinessEntry {
  readonly id: string;
  readonly localPath: string;
  ...
  readonly inspection: ProductionGlbInspection | null;
}

export interface ProductionAssetCorpusSummary {
  readonly schema: "a3d-production-runtime-asset-readiness";
  ...
  readonly entries: readonly ProductionAssetReadinessEntry[];
}

export function loadProductionAssetManifest(path = "fixtures/asset-corpus/manifest.json"): ProductionAssetManifest {
  return JSON.parse(readFileSync(resolve(path), "utf8")) as ProductionAssetManifest;
}
...
export function createProductionAssetCorpusSummary(manifest = loadProductionAssetManifest()): ProductionAssetCorpusSummary {
  const entries = manifest.assets.map((asset): ProductionAssetReadinessEntry => {
    ...
    const inspection = exists ? inspectProductionGlb(path) : null;
    return {
      id: asset.id,
  ...
  return {
    schema: "a3d-production-runtime-asset-readiness",
