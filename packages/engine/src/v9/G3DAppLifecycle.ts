export interface G3DDisposable {
  dispose(): void;
}

export interface G3DAppLifecycleSnapshot {
...
export class G3DAppLifecycle {
  private readonly disposables = new Set<G3DDisposable>();
  ...
  addDisposable<T extends G3DDisposable>(disposable: T): T {
  ...
  dispose(): void {
    ...
    if (errors.length > 0) {
      throw new AggregateError(errors, "G3D app lifecycle disposal failed.");
    }
  }

  snapshot(): G3DAppLifecycleSnapshot {
    return {
  ...
  private assertAlive(operation: string): void {
    if (this.disposed) {
      throw new Error(`Cannot ${operation} on a disposed G3D app lifecycle.`);
    }
  }
}
