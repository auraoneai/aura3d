import { Scene, type Mesh, type MeshOptions } from "@aura3d/scene";
import type { RenderSource, Geometry, RenderMaterial, MorphTargetDelta } from "@aura3d/rendering";

export interface A3DSceneMeshOptions extends Omit<MeshOptions, "renderable"> {
...
export interface A3DSceneRenderSourceOptions extends Omit<RenderSource, "scene" | "geometryLibrary" | "materialLibrary" | "morphTargetLibrary"> {
...
export class A3DScene extends Scene {
  ...
  addGeometry(id: string, geometry: Geometry): this {
    if (!id.trim()) throw new Error("A3DScene geometry id is required.");
    this.geometryLibrary.set(id, geometry);
    return this;
  }

  addMaterial(id: string, material: RenderMaterial): this {
    if (!id.trim()) throw new Error("A3DScene material id is required.");
    this.materialLibrary.set(id, material);
    return this;
  }

  addMorphTargets(id: string, morphTargets: readonly MorphTargetDelta[]): this {
    if (!id.trim()) throw new Error("A3DScene morph target id is required.");
    this.morphTargetLibrary.set(id, morphTargets);
    return this;
  }

  createRenderableMesh(options: A3DSceneMeshOptions): Mesh {
  ...
  toRenderSource(options: A3DSceneRenderSourceOptions = {}): RenderSource {
    return {
  ...
  private resolveGeometryHandle(geometry: string | Geometry, preferredId?: string): string {
    if (typeof geometry === "string") {
      if (!this.geometryLibrary.has(geometry)) throw new Error(`A3DScene missing geometry resource: ${geometry}`);
      return geometry;
    }
  ...
  private resolveMaterialHandle(material: string | RenderMaterial, preferredId?: string): string {
    if (typeof material === "string") {
      if (!this.materialLibrary.has(material)) throw new Error(`A3DScene missing material resource: ${material}`);
      return material;
    }
