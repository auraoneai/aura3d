...
createAuraApp("#app", {
  ...
  scene: scene()
    ...
    .add(model(assets.playerModel, { name: "player", material: material.emissive({ color: "#ffd84a", emissive: "#ffd84a" }) }).position(-1.42, 0.04, 0.54).rotate(0, 0.72, 0).scale(0.62))
    .add(primitives.box({ name: "player orange bill marker", material: material.emissive({ color: "#ff8a4c", emissive: "#ff8a4c" }) }).position(-1.08, 0.42, 0.48).rotate(0, 0.52, 0).scale([0.28, 0.08, 0.12]))
