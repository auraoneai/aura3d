...
createAuraApp("#app", {
  ...
  scene: scene()
    ...
    .add(model(assets.product).position(0, 0.02, -0.68).rotate(-0.18, -0.46, 0.03).scale(1.18))
    .add(lights.ambient({ intensity: 0.16, color: "#dce9ff" }))
    .add(lights.point({ name: "large cool softbox", position: [-2.2, 2.45, 2.25], color: "#eef6ff", intensity: 2.45 }))
    .add(lights.point({ name: "warm product rim", position: [2.1, 1.72, 0.15], color: "#ffd09a", intensity: 1.05 }))
