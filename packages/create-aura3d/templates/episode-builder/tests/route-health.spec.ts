...
test("episode builder route loads", async ({ page }) => {
  ...
  writeFileSync(resolve("tests/reports/route-health.json"), `${JSON.stringify({
    template: "episode-builder", url: page.url(), canvasBounds, runtime: await page.evaluate(() => { const api = (window as unknown as { __AURA3D_ANIMATION_TEMPLATE__?: { diagnostics?(): unknown } }).__AURA3D_ANIMATION_TEMPLATE__; return { mounted: Boolean(api), diagnostics: api?.diagnostics?.() }; })
  }, null, 2)}\n`);
});
