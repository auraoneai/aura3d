...
export interface CartoonFrameVisualInput {
  readonly id: string;
...
export interface CartoonFrameVisualQuality {
  readonly id: string;
  readonly ok: boolean;
  readonly failures: readonly string[];
}

export interface CartoonVisualQualityReport {
  readonly kind: "cartoon-visual-quality";
  readonly ok: boolean;
  readonly frames: readonly CartoonFrameVisualQuality[];
  readonly visibleCharacterCount: number;
  readonly blockers: readonly string[];
}

export interface CartoonVisualQualityOptions {
  readonly thresholds?: FrameVisualQualityThresholds | undefined;
  readonly minVisibleCharacters?: number | undefined;
}

export function createCartoonVisualQualityReport(
  frames: readonly CartoonFrameVisualInput[],
  options: CartoonVisualQualityOptions = {}
): CartoonVisualQualityReport {
  const thresholds = options.thresholds ?? defaultCartoonVisualQualityThresholds;
  const frameResults = frames.map((frame): CartoonFrameVisualQuality => {
...
export const defaultCartoonVisualQualityThresholds: FrameVisualQualityThresholds = {
