import { CartoonToonMaterial, type CartoonToonMaterialOptions } from "./CartoonToonMaterial.js";
import type { CartoonRenderPresetEvidence } from "./CartoonRenderPreset.js";
import type { CartoonMaterialStyle } from "./CartoonMaterialStyle.js";
import { colorGradePixels, outlinePixels, type ColorGradeResult, type OutlineResult } from "../PostProcessPass.js";

/**
 * applyCartoonRenderPreset — turns a (metadata) CartoonRenderPreset into the
 * concrete, pixel-affecting treatment it describes:
 *
 *  1. MATERIAL: builds a real `CartoonToonMaterial` (banded N·L ramp + rim) from
 ...
 *    rendering with a ShaderLibrary that has the cartoon program registered produces
...
export interface ApplyCartoonRenderPresetFrame {
...
export interface CartoonRenderPresetLightingDescriptor {
...
export interface ApplyCartoonRenderPresetOptions {
  ...
  readonly material?: Partial<CartoonToonMaterialOptions> | undefined;
  /** Optional frame to run the outline + color-grade post-passes against. */
  readonly frame?: ApplyCartoonRenderPresetFrame | undefined;
}

export interface ApplyCartoonRenderPresetResult {
  readonly kind: "cartoon-render-preset-application";
  /** Real engine material — assign to scene meshes to get banded toon pixels. */
  readonly material: CartoonToonMaterial;
  readonly materialStyle: CartoonMaterialStyle;
  /** Key-light descriptor derived from the preset, also baked into the material. */
  readonly lighting: CartoonRenderPresetLightingDescriptor;
...
/**
 * Map a CartoonMaterialStyle.treatment to a shadow-floor that keeps the chosen
 * look readable: flatter treatments use a higher floor (less contrast between bands).
 */
function shadowFloorForTreatment(style: CartoonMaterialStyle): number {
...
export function applyCartoonRenderPreset(
  preset: CartoonRenderPresetEvidence,
  options: ApplyCartoonRenderPresetOptions = {}
): ApplyCartoonRenderPresetResult {
  ...
  const material = new CartoonToonMaterial({
  ...
  const lighting: CartoonRenderPresetLightingDescriptor = {
    keyLightDirection,
  ...
  return {
    kind: "cartoon-render-preset-application",
    material,
    materialStyle: style,
    lighting,
    outlineEnabled,
