...
export class Renderer {
  ...
  private executeFusedLdrPostprocess(current: RenderTarget, passes: readonly RendererPostProcessPassPlan[], outputTarget?: RenderTarget): boolean {
    ...
    if (!this.device.presentLdrPostprocess && passes.some((pass) => pass.name === "depth-of-field" || pass.name === "ssao" || pass.name === "ssr")) return false;
...
function canFuseLdrPostprocess(source: RenderTarget, passes: readonly RendererPostProcessPassPlan[]): boolean {
  ...
  return passes.length > 1
    ...
    && passes.every((pass) => pass.name === "bloom" || pass.name === "tone-mapping" || pass.name === "color-grade" || pass.name === "depth-of-field" || pass.name === "ssao" || pass.name === "ssr" || pass.name === "outline" || pass.name === "fxaa")
...
function ldrFusionPassRank(name: RendererPostProcessPassName): number {
  ...
  if (name === "ssao") return 3;
  if (name === "ssr") return 4;
  if (name === "outline") return 5;
  if (name === "fxaa") return 6;
