...
export async function runV6Example(definition: V6ExampleDefinition): Promise<void> {
  ...
  try {
    ...
    const hdr = await fetchArrayBuffer(`/fixtures/environment-corpus/hdri/${definition.environment.file}`);
    ...
    const pipeline = await loadV6GLTFRenderPipeline({
      url: `/fixtures/asset-corpus/${primary.file}`,
      assetId: primary.id,
    ...
    const proof = renderer.renderImportedAsset({
      ...
      metadata: {
        ...
        hdrEnvironmentUri: `/fixtures/environment-corpus/hdri/${definition.environment.file}`
      }
    });
...
async function loadSecondaryMetadata(
  ...
  for (const asset of definition.assets.filter((item) => item.id !== primaryAssetId)) {
    const pipeline = await loadV6GLTFRenderPipeline({
      url: `/fixtures/asset-corpus/${asset.file}`,
      assetId: asset.id,
