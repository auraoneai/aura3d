export type V6WorkflowId = "product" | "asset" | "material" | "architecture" | "cinematic";

export interface V6WorkflowDefinition {
  readonly id: V6WorkflowId;
...
export interface V6AssetPreflightInput {
  readonly id: string;
...
export interface V6AssetPreflightResult {
...
export interface V6VisualQAInput {
...
export interface V6VisualQAResult {
  readonly screenshotPath: string;
  readonly pass: boolean;
...
export interface V6ProductionRendererDefaults {
  readonly workflowId: V6WorkflowId;
...
export interface V6WorkflowPlan {
  readonly workflow: V6WorkflowDefinition;
  readonly defaults: V6ProductionRendererDefaults;
  readonly preflightRequired: true;
  readonly visualQARequired: true;
}

export const V6_WORKFLOWS: readonly V6WorkflowDefinition[] = [
  {
    id: "product",
...
export function listV6WorkflowDefinitions(): readonly V6WorkflowDefinition[] {
  return V6_WORKFLOWS;
}

export function createV6AssetPreflight(input: V6AssetPreflightInput): V6AssetPreflightResult {
...
export function createV6VisualQAResult(input: V6VisualQAInput): V6VisualQAResult {
...
export function createV6ProductionRendererDefaults(workflowId: V6WorkflowId): V6ProductionRendererDefaults {
  const cameraByWorkflow: Record<V6WorkflowId, { readonly yawRadians: number; readonly pitchRadians: number; readonly paddingRatio: number }> = {
...
export function createV6WorkflowPlan(workflowId: V6WorkflowId): V6WorkflowPlan {
  const workflow = V6_WORKFLOWS.find((item) => item.id === workflowId);
  if (!workflow) throw new Error(`Unknown V6 workflow: ${workflowId}`);
  return {
    workflow,
    defaults: createV6ProductionRendererDefaults(workflowId),
