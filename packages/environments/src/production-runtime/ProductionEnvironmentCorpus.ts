...
export type ProductionEnvironmentProbeType = "diffuseIrradiance" | "specularPrefilter" | "brdfLut" | "background";

export interface ProductionEnvironmentRequirements {
  readonly minimumRealHdriSources: number;
  readonly minimumResolution: readonly [number, number];
  readonly minimumTotalBytes: number;
  readonly requiredProbeTypes: readonly ProductionEnvironmentProbeType[];
...
export interface ProductionPMREMPreset {
  readonly faceSize: number;
  readonly mipCount: number;
  readonly cacheKey: string;
}

export interface ProductionHDREnvironment {
  readonly id: string;
  ...
  readonly pmrem: ProductionPMREMPreset;
  readonly probes: readonly ProductionEnvironmentProbeType[];
}

export interface ProductionEnvironmentManifest {
  readonly schema: "a3d-production-runtime-hdr-environment-corpus";
  readonly requirements: ProductionEnvironmentRequirements;
  readonly claimBoundary: string;
  readonly flagshipBindings: Readonly<Record<string, string>>;
  readonly environments: readonly ProductionHDREnvironment[];
}

export interface ProductionHDRInspection {
...
export interface ProductionEnvironmentReadinessEntry {
  readonly id: string;
  readonly localPath: string;
  readonly exists: boolean;
  readonly sha256Matches: boolean;
  readonly bytesMatch: boolean;
  readonly inspection: ProductionHDRInspection | null;
}

export interface ProductionEnvironmentCorpusSummary {
  readonly schema: "a3d-production-runtime-environment-readiness";
  ...
  readonly entries: readonly ProductionEnvironmentReadinessEntry[];
}

export function loadProductionEnvironmentManifest(path = "fixtures/environment-corpus/manifest.json"): ProductionEnvironmentManifest {
  return JSON.parse(readFileSync(resolve(path), "utf8")) as ProductionEnvironmentManifest;
}

export function inspectProductionHDR(path: string, expectedResolution: readonly [number, number]): ProductionHDRInspection {
...
export function createProductionEnvironmentCorpusSummary(manifest = loadProductionEnvironmentManifest()): ProductionEnvironmentCorpusSummary {
  const entries = manifest.environments.map((environment): ProductionEnvironmentReadinessEntry => {
    ...
    const inspection = exists ? inspectProductionHDR(path, environment.resolution) : null;
    return {
      id: environment.id,
  ...
  return {
    schema: "a3d-production-runtime-environment-readiness",
