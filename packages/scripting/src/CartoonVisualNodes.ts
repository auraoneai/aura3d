...
export const cartoonVisualNodeDefinitions: readonly VisualNodeDefinition[] = [
  node("setScene", "scene", "Set Scene", "Switch to a named cartoon scene.", [flowInput(), stringInput("sceneId")]),
  node("transitionTo", "scene", "Transition To", "Transition to a cartoon scene with a named transition.", [flowInput(), stringInput("sceneId"), stringInput("transition", true), numberInput("duration", 0.5)]),
  node("loadSet", "scene", "Load Set", "Load a cartoon set asset into the episode scene.", [flowInput(), stringInput("setId")]),
  ...
  node("cartoonCutTo", "camera", "Cartoon Cut To", "Cut to a named cartoon camera preset.", [flowInput(), stringInput("presetId"), stringInput("targetId", true)]),
  ...
  node("shakeCamera", "camera", "Shake Camera", "Add cartoon camera shake.", [flowInput(), numberInput("intensity", 0.2), numberInput("duration", 0.25)]),
  ...
  node("delay", "timing", "Delay", "Delay a cartoon action by seconds.", [flowInput(), numberInput("duration", 1)]),
  node("schedule", "timing", "Schedule", "Schedule a cartoon command at a timeline time.", [flowInput(), numberInput("time", 0), stringInput("label", true)]),
...
function node(kind: string, category: VisualNodeDefinition["category"], title: string, description: string, inputs: VisualNodeDefinition["ports"]): VisualNodeDefinition {
  return {
    ...
    oldBranchSource: ["packages/scripting/src/CartoonVisualNodes.ts"]
  };
}
